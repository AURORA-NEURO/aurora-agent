...
export class InMemoryAutonomousWorkflowPortfolioEvidenceWorkQueue {
  ...
  admit(input: {
    ...
    if (existing) {
      ...
      if (existing.job_id !== jobId || existing.item_id !== itemId || existing.domain !== input.domain || existing.wave_index !== input.waveIndex || canonicalJson(existing.dependency_item_ids) !== canonicalJson(dependencies) || existing.provider_status !== input.providerStatus || existing.portfolio_plan_digest !== input.portfolioPlanDigest || existing.admission_digest !== admissionDigest || existing.provider_execution_digest !== input.providerExecutionDigest || existing.evidence_plan_digest !== input.evidencePlanDigest || existing.request_digest !== input.requestDigest || existing.max_attempts !== maxAttempts) throw new ArgumentError("portfolio evidence work identity conflicts with an existing item");
