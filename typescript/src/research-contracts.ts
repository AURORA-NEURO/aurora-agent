...
export const ADAPTER_MULTIMODAL_RETRIEVAL_SYNTHESIS_CONTRACT_MODEL_FEATURE_ID = "AFA-adapter-P02-F06" as const;
export const ADAPTER_MULTIMODAL_RETRIEVAL_SYNTHESIS_CONTRACT_MODEL_CONTRACT_VERSION = "adapter-multimodal-retrieval-synthesis-contract-model/1.0" as const;
export interface AdapterMultimodalRetrievalSynthesisContractModelReceipt { schema_version:string; contract_version:string; feature_id:string; request_id:string; query_id:string; contract_id:string; schema_profile:string; canonicalization:string; consumer:string; algorithm_version:string; requested_output:string; disposition:"passed"|"blocked"|"unknown"; candidate_order:string[]; selected_order:string[]; omitted_order:string[]; uncertainty_order:string[]; negative_order:string[]; contradictory_order:string[]; replay_identity:string; synthesis_digest:string; contract_digest:string; effect_receipts:string[]; artifact:Record<string,unknown>; raw_data_local:boolean; boundary:string; }
export function validateAdapterMultimodalRetrievalSynthesisContractModelReceipt(receipt:AdapterMultimodalRetrievalSynthesisContractModelReceipt):void { if(receipt.schema_version!==RESEARCH_CONTRACT_SCHEMA_VERSION||receipt.contract_version!==ADAPTER_MULTIMODAL_RETRIEVAL_SYNTHESIS_CONTRACT_MODEL_CONTRACT_VERSION||receipt.feature_id!==ADAPTER_MULTIMODAL_RETRIEVAL_SYNTHESIS_CONTRACT_MODEL_FEATURE_ID)throw new Error("multimodal retrieval contract model schema, feature, or version mismatch"); if(receipt.boundary!==PRECLINICAL_BOUNDARY||!receipt.raw_data_local||!receipt.request_id.trim()||!receipt.query_id.trim()||!receipt.contract_id.trim()||receipt.schema_profile!=="ScopedRetrievalQuery@1"||receipt.canonicalization!=="aurora-json-canonical-v1"||!receipt.consumer.trim()||!receipt.algorithm_version.trim()||receipt.requested_output!=="EvidenceSynthesis1@1"||!receipt.candidate_order.length||!receipt.effect_receipts.length)throw new Error("multimodal retrieval contract model identity, output, locality, schema, candidates, or effects are incomplete"); for(const values of[receipt.candidate_order,receipt.selected_order,receipt.omitted_order,receipt.uncertainty_order,receipt.negative_order,receipt.contradictory_order,receipt.effect_receipts])if(JSON.stringify([...new Set(values)].sort())!==JSON.stringify(values))throw new Error("multimodal retrieval contract model ordering is invalid"); if(JSON.stringify([...receipt.selected_order,...receipt.omitted_order].sort())!==JSON.stringify([...receipt.candidate_order].sort()))throw new Error("multimodal retrieval contract model state partition is incomplete"); for(const value of[receipt.replay_identity,receipt.synthesis_digest,receipt.contract_digest,receipt.artifact.content_hash])if(typeof value!=="string"||!/^[0-9a-f]{64}$/.test(value))throw new Error("multimodal retrieval contract model digest is invalid"); if(receipt.effect_receipts.some(effect=>!effect.startsWith("compute:multimodal-retrieval-contract-model:")&&effect!=="block:unsafe-release"))throw new Error("multimodal retrieval contract model effect is outside local computation gate"); }
export function adapterMultimodalRetrievalSynthesisContractModelReceiptDigest(receipt:AdapterMultimodalRetrievalSynthesisContractModelReceipt):string{validateAdapterMultimodalRetrievalSynthesisContractModelReceipt(receipt);return digestJsonSync(receipt);}
