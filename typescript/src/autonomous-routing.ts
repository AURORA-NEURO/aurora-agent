...
export interface AutonomousSemanticRouteResult extends JsonObject {
  ...
  status: "completed" | "approval_required" | "provider_abstained" | "provider_invalid" | "provider_disagreement" | "policy_review_required" | "policy_blocked";
  ...
  /** Strict-mode provider-free admission; absent for ordinary audit-mode routes. */
  domain_policy_admission?: AutonomousDomainPolicyAdmission;
...
function routeReviewResult(deterministic: AutonomousRouteProposal, status: AutonomousSemanticRouteResult["status"], promptDigest: string, selectionDigest: string | null, selectedModel: { provider: string; model: string } | null, semanticCandidates: AutonomousSemanticRouteCandidate[] = [], selectedDomains: AutonomousDomainName[] = [], confidence = 0, outcomeDigest: string | null = null, costBudget?: AutonomousCostBudget, domainPolicyAdmission?: AutonomousDomainPolicyAdmission): AutonomousSemanticRouteResult {
  const result = { schema: AUTONOMOUS_SEMANTIC_ROUTE_SCHEMA, status, route: deterministic, deterministic_route: deterministic, semantic_candidates: semanticCandidates, semantic_selected_domains: selectedDomains, semantic_confidence: confidence, selected_model: selectedModel, selection_digest: selectionDigest, prompt_digest: promptDigest, outcome_digest: outcomeDigest, cost_budget: costBudget?.snapshot() ?? null, retention: RETENTION, authorization: AUTHORIZATION } as AutonomousSemanticRouteResult;
  return domainPolicyAdmission ? { ...result, domain_policy_admission: domainPolicyAdmission } : result;
}
...
export async function semanticRouteAutonomousTask(agent: AutonomousAgent, task: string, options: AutonomousSemanticRouteOptions = {}): Promise<AutonomousSemanticRouteResult> {
  ...
  const domainPolicyMode = options.domainPolicyMode ?? "audit";
  if (!AUTONOMOUS_DOMAIN_POLICY_MODES.includes(domainPolicyMode)) throw new ArgumentError("domainPolicyMode must be audit or strict");
  ...
  const estimatedInputTokens = Math.max(1, Math.ceil((taskText.length + catalogText.length + 2_048) / 4));
  const domainPolicyAdmission = domainPolicyMode === "strict"
    ? evaluateAutonomousDomainPolicy(autonomousDomainPolicy("cross_domain"), {
      estimated_input_tokens: estimatedInputTokens,
      requested_output_tokens: options.maxOutputTokens ?? 512,
    ...
    : undefined;
  if (domainPolicyAdmission && domainPolicyAdmission.decision !== "admitted") {
    return routeReviewResult(deterministic, domainPolicyAdmission.decision === "blocked" ? "policy_blocked" : "policy_review_required", promptDigest, null, null, [], [], 0, null, costBudget, domainPolicyAdmission);
  }
  if (options.approveProviderCall !== true) return routeReviewResult(deterministic, "approval_required", promptDigest, null, null, [], [], 0, null, costBudget, domainPolicyAdmission);
  ...
  if (payload === null && execution.response.text.trim()) {
    try { payload = JSON.parse(execution.response.text); } catch { return routeReviewResult(deterministic, "provider_invalid", promptDigest, selectionDigest, selectedModel, [], [], 0, outcomeDigest, costBudget, domainPolicyAdmission); }
  }
  if (!isObject(payload) || !Array.isArray(payload.selected_domains)) return routeReviewResult(deterministic, "provider_invalid", promptDigest, selectionDigest, selectedModel, [], [], 0, outcomeDigest, costBudget, domainPolicyAdmission);
  let confidence: number;
  try { confidence = boundedProbability("semantic route confidence", payload.confidence); } catch { return routeReviewResult(deterministic, "provider_invalid", promptDigest, selectionDigest, selectedModel, [], [], 0, outcomeDigest, costBudget, domainPolicyAdmission); }
  ...
  for (const row of payload.selected_domains.slice(0, 8)) {
    if (!isObject(row) || typeof row.domain !== "string" || !AUTONOMOUS_DOMAIN_NAMES.includes(row.domain as AutonomousDomainName) || seen.has(row.domain as AutonomousDomainName) || typeof row.rationale !== "string") return routeReviewResult(deterministic, "provider_invalid", promptDigest, selectionDigest, selectedModel, [], [], confidence, outcomeDigest, costBudget, domainPolicyAdmission);
    let score: number;
    try { score = boundedProbability("semantic route score", row.score); } catch { return routeReviewResult(deterministic, "provider_invalid", promptDigest, selectionDigest, selectedModel, [], [], confidence, outcomeDigest, costBudget, domainPolicyAdmission); }
    try { boundedText("semantic route rationale", row.rationale, 512); } catch { return routeReviewResult(deterministic, "provider_invalid", promptDigest, selectionDigest, selectedModel, [], [], confidence, outcomeDigest, costBudget, domainPolicyAdmission); }
    const domain = row.domain as AutonomousDomainName;
    const profile = profileByDomain.get(domain);
    if (!profile) return routeReviewResult(deterministic, "provider_invalid", promptDigest, selectionDigest, selectedModel, [], [], confidence, outcomeDigest, costBudget, domainPolicyAdmission);
    seen.add(domain);
  ...
  if (abstained) return routeReviewResult(deterministic, "provider_abstained", promptDigest, selectionDigest, selectedModel, semanticCandidates, [], confidence, outcomeDigest, costBudget, domainPolicyAdmission);
  ...
  if (!agrees) return routeReviewResult(deterministic, "provider_disagreement", promptDigest, selectionDigest, selectedModel, semanticCandidates, selectedDomains, confidence, outcomeDigest, costBudget, domainPolicyAdmission);
