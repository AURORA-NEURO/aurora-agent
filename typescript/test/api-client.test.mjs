...
test("client exposes typed discovery, tool calls, and refusal preservation", async () => {
  ...
  const evaluatorReview = await client.missionEvaluatorReview({
    discovery: evaluators.mcp.result.structuredContent,
    selections: [{ id: "assay-evaluator", claim_id: "fidelity-claim", adapter_id: "oncoworlds.assay_fidelity", domain: "oncology", step_id: "assay", output_pointer: "/fidelity" }],
  });
...
test("client parses cursor SSE and validates webhook mutations", async () => {
    ...
    fetch: async (input, init) => {
      ...
      if (path === "/v1/operations/handoff") {
        return jsonResponse({
          ...
          execution_prerequisites: { kind: "operations_gate_acceptance", required: true, group_ids: ["biological_domains"], required_gates: ["catalogue", "observed_activity", "transport_completion", "evaluation_evidence", "safety_evidence", "release_evidence"], gate_endpoint: "/v1/operations/gates?after=0&limit=256", acceptance_field: "operations_gate_acceptance", review_required_before_dispatch: true, readiness_claimed: false },
  ...
  assert.equal(handoff.execution_prerequisites.required, true);
  ...
  const gates = await client.operationsDomainGates(0, 2);
  assert.equal(gates.groups[0].gate_state, "insufficient_evidence");
  assert.equal(gates.gate_digest.length, 64);
  assert.equal(gates.summary.readiness_claimed, false);
  const review = await client.createOperationsGateReview({ gate_digest: "d".repeat(64), reviewer: "operator", rationale: "reviewed", group_ids: ["biological_domains"], accepted_gates: { biological_domains: ["catalogue", "observed_activity", "transport_completion", "evaluation_evidence", "safety_evidence", "release_evidence"] } });
  assert.equal(review.review_id.length, 64);
  const reviews = await client.operationsGateReviews(0, 2, review.review_id);
  assert.equal(reviews.reviews[0].review_id, review.review_id);
  ...
  const client = new ApiClient({
    ...
    fetch: async (input, init) => {
      ...
      if (path === "/v1/missions/queue/persistence" && init.method === "GET") {
        return jsonResponse({ ok: true, enabled: true, file_present: true, file_bytes: 128, schema_version: 1, state_digest: "q".repeat(64), integrity_verified: true, max_file_bytes: 64 * 1024 * 1024, registry_size: 0, jobs: [], startup_recoveries: [], automatic_resume: false, execution_scope: "single-process-api-worker", recovery_policy: "expired leases are classified by idempotency at startup; no recovered job is automatically dispatched", does_not_claim: ["distributed scheduling or lease fencing"], flush: "/v1/missions/queue/persistence/flush" });
      }
      ...
      if (path === "/v1/missions/queue/persistence/flush" && init.method === "POST") {
        return jsonResponse({ ok: true, bytes: 128, queue: { ok: true, enabled: true, file_present: true, file_bytes: 128, schema_version: 1, state_digest: "q".repeat(64), integrity_verified: true, max_file_bytes: 64 * 1024 * 1024, registry_size: 0, jobs: [], startup_recoveries: [], automatic_resume: false, execution_scope: "single-process-api-worker", recovery_policy: "expired leases are classified by idempotency at startup; no recovered job is automatically dispatched", does_not_claim: ["external effect completion"], flush: "/v1/missions/queue/persistence/flush" }, request_id: "queue-flush-1", guarantees: ["the checkpoint is content-addressed and atomically replaced"] });
      }
