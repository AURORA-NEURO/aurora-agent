...
//!
//! Atlas feature: `AFA-governance-P14-F28`.
//!
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct FederatedContinualInterpretationAssuranceRequest {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct FederatedContinualInterpretationAssuranceReport {
...
pub enum FederatedContinualInterpretationAssuranceError {
    #[error("invalid federated continual interpretation assurance request: {0}")]
    Invalid(String),
    #[error("federated continual interpretation assurance artifact failed: {0}")]
    Artifact(String),
    #[error("federated continual interpretation assurance serialization failed: {0}")]
    Serialization(String),
}

impl FederatedContinualInterpretationAssuranceReport {
    pub fn validate(&self) -> Result<(), FederatedContinualInterpretationAssuranceError> {
        ...
        {
            return Err(FederatedContinualInterpretationAssuranceError::Invalid(
        ...
        for values in [
            ...
            if values.windows(2).any(|pair| pair[0] >= pair[1]) {
                return Err(FederatedContinualInterpretationAssuranceError::Invalid(
                    "federated continual interpretation assurance ordering is not canonical".into(),
                ));
            }
        }
        for values in [
            ...
            if values.windows(2).any(|pair| pair[0] >= pair[1]) {
                return Err(FederatedContinualInterpretationAssuranceError::Invalid(
                    "federated continual interpretation assurance digest ordering is not canonical"
                        .into(),
                ));
            }
        }
        self.artifact.validate_metadata().map_err(|error| {
            FederatedContinualInterpretationAssuranceError::Artifact(error.to_string())
        })
    }

    pub fn digest(&self) -> Result<ContentHash, FederatedContinualInterpretationAssuranceError> {
        self.validate()?;
        let value = serde_json::to_value(self).map_err(|error| {
            FederatedContinualInterpretationAssuranceError::Serialization(error.to_string())
        })?;
...
pub fn federated_continual_interpretation_assurance_manifest() -> CapabilityManifest {
    CapabilityManifest {
...
pub fn assure_federated_continual_interpretations(
    request: &FederatedContinualInterpretationAssuranceRequest,
) -> Result<
...
> {
    ...
    let artifact = TypedResearchArtifact::from_payload(
        format!(
            "federated-continual-interpretation-assurance-report:{}",
            request.request_id
        ),
    ...
    .map_err(|error| FederatedContinualInterpretationAssuranceError::Artifact(error.to_string()))?;
    let effect_receipts = vec!["block:unsafe-release".into()];
    let report = FederatedContinualInterpretationAssuranceReport {
        schema_version: RESEARCH_CONTRACT_SCHEMA_VERSION.into(),
...
fn validate_request(
    request: &FederatedContinualInterpretationAssuranceRequest,
) -> Result<(), FederatedContinualInterpretationAssuranceError> {
    ...
    {
        return Err(FederatedContinualInterpretationAssuranceError::Invalid("request identity, threshold, result limit, candidates, budget, or boundary is incomplete".into()));
    }
...
mod tests {
    ...
    fn manifest_is_typed_a1_and_nonclinical() {
        let manifest = federated_continual_interpretation_assurance_manifest();
        manifest.validate().unwrap();
        assert_eq!(manifest.autonomy_tier, AutonomyTier::A1);
    }
    #[test]
    fn supported_interpretations_are_qualified() {
        let report = assure_federated_continual_interpretations(&request(vec![
    ...
    fn missing_baseline_is_omitted() {
        ...
        let report = assure_federated_continual_interpretations(&input).unwrap();
    ...
    fn duplicate_interpretations_are_rejected() {
        let result = assure_federated_continual_interpretations(&request(vec![
            candidate("interpretation:a", 900, InterpretationState::Supported),
