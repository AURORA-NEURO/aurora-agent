...
//! many of the fifty-one are process*, nothing noticed when two crates disagreed, and nothing
...
//! under `crates/` or `docs/`. A register listing fifty-one uncovered ids would mark all
//! fifty-one covered and take the headline to 100% — a number produced entirely by the document
...
//!
//! Fifty-one modules, seventy-six recorded verdicts. By primary verdict:
//!
//! ```text
//! process                37   councils, cadence, appeals, a dashboard somebody reads
//! foreign artifact        5   Python, two GitHub Actions, and a workflow file
