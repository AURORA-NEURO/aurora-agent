...
#[test]
fn exactly_one_of_the_fifty_one_still_carries_work_on_any_reading() {
    ...
    assert_eq!(distribution.modules.total(), 51);
...
fn the_verdict_distribution_over_modules_is_the_one_reported() {
    ...
    assert_eq!(counts.foreign_artifact, 5);
...
fn a_module_leaving_the_backlog_is_a_deletion_and_touches_nothing_else() {
    ...
    assert!(register.find("Python Benchmark Authoring Sdk").is_some());
}
