...
use bioprism_mutation::{
    apply, generate, measure, standard_suite, verdict_of, Mechanism, Mutation, MutationKind,
    PostconditionResult, Relation,
};
...
fn a_violated_postcondition_is_caught() {
    ...
    assert!(family.rejected[0].reason.contains("postcondition violated"));
}
