//! Bounded-evolution control plane (`AFA-ids-P32-F31`).
//!
...
pub const CONTRACT_VERSION: &str = "ids-bounded-evolution-federated-control-plane/1.0";
pub const INPUT_SCHEMA: &str = "IdsEvolutionRequest8@1";
pub const OUTPUT_SCHEMA: &str = "IdsEvolutionReceipt10@1";
pub const CONTENT_TYPE: &str = "application/vnd.aurora.ids-evolution-receipt-10+json";
...
pub enum EvolutionEvidenceState {
    Proven,
    Supported,
    Unknown,
    Unmeasured,
...
pub struct IdsEvolutionProposal7 {
    ...
    pub evidence_state: EvolutionEvidenceState,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct IdsEvolutionRequest8 {
    pub request_id: String,
    pub purpose: String,
    pub capability_id: String,
    pub current_version: String,
    pub proposals: Vec<IdsEvolutionProposal7>,
    pub replay_identity: ContentHash,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct IdsEvolutionReceipt10Artifact {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct IdsEvolutionReceipt10 {
    ...
    pub artifact: IdsEvolutionReceipt10Artifact,
...
pub enum BoundedEvolutionError {
    #[error("invalid IDS bounded-evolution request: {0}")]
    Invalid(String),
    #[error("IDS bounded-evolution report failed validation: {0}")]
    Report(String),
}
...
pub fn bounded_evolution_manifest() -> serde_json::Value {
    json!({"schema_version":"aurora-research-contract/1.0","capability_id":FEATURE_ID,"version":CONTRACT_VERSION,"owner_crate":"ids","consumers":["researcher","release operator","evolution steward"],"behavior":"evaluate signed capability-evolution proposals with deterministic compatibility, benchmark, safety, evidence, replay, policy, locality, and protected-closure gates","value":"makes bounded evolution auditable and reversible before any implementation or release mutation","input_schema":INPUT_SCHEMA,"output_schema":OUTPUT_SCHEMA,"effects":["preview:bounded-evolution","manage:local-capability"],"permissions":["read:local-evolution-summaries","request:bounded-evolution-preview"],"autonomy_tier":"A2","boundary":PRECLINICAL_BOUNDARY})
}
impl IdsEvolutionReceipt10 {
    pub fn validate(&self) -> Result<(), BoundedEvolutionError> {
        ...
        {
            return Err(BoundedEvolutionError::Report(
                "evolution identity, proposals, effects, locality, or disposition is incomplete"
                    .into(),
            ));
        }
        for values in [
...
fn validate_request(request: &IdsEvolutionRequest8) -> Result<(), BoundedEvolutionError> {
        ...
        || !request.aggregate_only
    {
        return Err(BoundedEvolutionError::Invalid(
            "evolution identity, proposal bound, replay, locality, or boundary is invalid".into(),
        ));
    }
...
pub fn preview_bounded_evolution(
    request: &IdsEvolutionRequest8,
) -> Result<IdsEvolutionReceipt10, BoundedEvolutionError> {
    ...
    for proposal in &proposals {
        ...
        } else if proposal.evidence_state == EvolutionEvidenceState::Contradicted {
            blocked.insert(proposal.proposal_id.clone());
            negative.insert(format!("{}:contradicted", proposal.proposal_id));
        } else if !matches!(
            proposal.evidence_state,
            EvolutionEvidenceState::Proven | EvolutionEvidenceState::Supported
        ) {
            unresolved.insert(proposal.proposal_id.clone());
    ...
    let digest = ContentHash::of_value(&payload)
        .map_err(|error| BoundedEvolutionError::Report(error.to_string()))?;
    let report = IdsEvolutionReceipt10 {
        ...
        artifact: IdsEvolutionReceipt10Artifact {
            artifact_id: format!("ids-evolution-receipt-10:{}", request.request_id),
...
mod tests {
    ...
    fn proposal(id: &str) -> IdsEvolutionProposal7 {
        IdsEvolutionProposal7 {
            proposal_id: id.into(),
            ...
            evidence_state: EvolutionEvidenceState::Supported,
            benchmark_pass: true,
            safety_pass: true,
            compatible: true,
    ...
    fn request() -> IdsEvolutionRequest8 {
        IdsEvolutionRequest8 {
    ...
    #[test]
    fn manifest_is_a2() {
        assert_eq!(bounded_evolution_manifest()["autonomy_tier"], "A2");
    }
    #[test]
    fn nominal_is_qualified() {
        assert_eq!(
            preview_bounded_evolution(&request()).unwrap().disposition,
            "qualified"
        );
    }
    #[test]
    fn incompatible_is_unresolved() {
        ...
        assert_eq!(
            preview_bounded_evolution(&q).unwrap().disposition,
            "unresolved"
        );
    }
    #[test]
    ...
    fn digest_is_deterministic() {
        let a = preview_bounded_evolution(&request()).unwrap();
        let b = preview_bounded_evolution(&request()).unwrap();
