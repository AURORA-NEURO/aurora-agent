...
pub const CONTRACT_VERSION: &str =
    "ids-prospective-throughput-evidence-surveillance-contract-model/1.0";
...
impl TypedArtifact {
    fn validate(&self) -> Result<(), ContractModelError> {
        ...
        {
            return Err(ContractModelError::Artifact(
                "typed evidence artifact metadata is incomplete".into(),
            ));
        }
        Ok(())
    }
...
pub enum ContractModelError {
    #[error("invalid ids evidence contract request: {0}")]
    Invalid(String),
    #[error("ids evidence contract artifact failed: {0}")]
    Artifact(String),
}
...
impl EvidenceSurveillanceContractReceipt {
    pub fn validate(&self) -> Result<(), ContractModelError> {
        ...
        for values in [
            ...
            if !canonical(values) {
                return Err(ContractModelError::Invalid(
                    "ids evidence contract ordering is not canonical".into(),
                ));
            }
        }
        let classified = self
        ...
        if classified
            != self
                ...
                .cloned()
                .collect::<BTreeSet<_>>()
            || classified.len() != self.candidate_order.len()
        {
            return Err(ContractModelError::Invalid(
                "ids evidence contract states do not partition candidates".into(),
            ));
        }
        for value in [
            &self.queue_digest,
            ...
            if !digest(value) {
                return Err(ContractModelError::Invalid(
                    "ids evidence contract digest is invalid".into(),
                ));
            }
        }
        if self.effect_receipts.iter().any(|effect| {
            !effect.starts_with("read:local-evidence-contract:") && effect != "block:unsafe-release"
        }) {
            return Err(ContractModelError::Invalid(
                "ids evidence contract effect is outside local-read gate".into(),
            ));
        }
        ...
        {
            return Err(ContractModelError::Invalid(
                "blocked ids evidence contract must explicitly block release".into(),
            ));
        }
...
pub fn model_throughput_evidence_surveillance_contract(
...
) -> Result<EvidenceSurveillanceContractReceipt, ContractModelError> {
    ...
    if claims.iter().any(|claim| {
        claim.claim_id.trim().is_empty()
            || claim.semantic_type.trim().is_empty()
            || !digest(&claim.value_digest)
    }) || claims
        .windows(2)
        .any(|pair| pair[0].claim_id == pair[1].claim_id)
    {
        return Err(ContractModelError::Invalid(
            "ids evidence claim identities or value digests are malformed or duplicated".into(),
        ));
    }
    let compatibility =
        request.input_schema == INPUT_SCHEMA && request.output_schema == OUTPUT_SCHEMA;
    let admission = request
        .max_claims
        .min(request.budget_units)
        .min(claims.len());
    let (admitted, overflow) = claims.split_at(admission);
    let mut candidate_order = claims
        .iter()
    ...
    for claim in admitted {
        if !compatibility {
            ...
            || matches!(
                ...
                EvidenceState::Unknown | EvidenceState::Speculative | EvidenceState::Unmeasured
            )
        {
            unknown.insert(claim.claim_id.clone());
            loss.insert(format!("claim:{}:evidence-unresolved", claim.claim_id));
    ...
    if !request.policy_allow {
        omission.insert("control:policy-denied".into());
    }
    if !request.protected_closure {
        omission.insert("control:protected-closure-incomplete".into());
    }
    let disposition =
        if !request.policy_allow || !request.protected_closure || !request.raw_data_local {
            ...
            ContractDisposition::Compatible
        };
    let retained_order = retained.into_iter().collect::<Vec<_>>();
    ...
    let semantic_loss = loss.into_iter().collect::<Vec<_>>();
    ...
    let artifact_hash =
        ContentHash::of_value(&payload).map_err(|e| ContractModelError::Artifact(e.to_string()))?;
    let artifact = TypedArtifact {
        schema_version: RESEARCH_CONTRACT_SCHEMA_VERSION.into(),
        artifact_id: format!("ids-throughput-evidence-contract:{}", request.request_id),
        content_type: CONTENT_TYPE.into(),
        content_hash: artifact_hash,
        semantic_loss: Vec::new(),
        provenance: vec![provenance_digest.to_string()],
        boundary: PRECLINICAL_BOUNDARY.into(),
    };
    let effect_receipts = if disposition == ContractDisposition::Blocked {
        vec!["block:unsafe-release".into()]
    } else {
        vec![format!(
            "read:local-evidence-contract:{}",
            request.request_id
        )]
    };
    let receipt = EvidenceSurveillanceContractReceipt {
        schema_version: RESEARCH_CONTRACT_SCHEMA_VERSION.into(),
...
mod tests {
    ...
    fn request() -> EvidenceFeedRequest {
        ...
        let claim = |id: &str, seq: u64, state: EvidenceState| ContractClaim {
            claim_id: id.into(),
            sequence: seq,
        ...
        EvidenceFeedRequest {
            request_id: "request:ids-throughput".into(),
            input_schema: INPUT_SCHEMA.into(),
            output_schema: OUTPUT_SCHEMA.into(),
    ...
    fn overflow_is_partial() {
        ...
        r.max_claims = 1;
        assert_eq!(
            model_throughput_evidence_surveillance_contract(&r)
                .unwrap()
                .disposition,
            ContractDisposition::Partial
        );
    ...
    fn unknown_is_retained() {
        let mut r = request();
        r.claims[0].evidence_state = EvidenceState::Unknown;
        let out = model_throughput_evidence_surveillance_contract(&r).unwrap();
        assert!(!out.unknown_order.is_empty());
    }
    #[test]
    fn contradiction_is_denied() {
        let mut r = request();
        r.claims[0].evidence_state = EvidenceState::Contradicted;
        let out = model_throughput_evidence_surveillance_contract(&r).unwrap();
        assert!(!out.denied_order.is_empty());
    }
    #[test]
    fn policy_blocks() {
        let mut r = request();
    ...
    #[test]
    fn replay_is_stable() {
        let a = model_throughput_evidence_surveillance_contract(&request()).unwrap();
        let b = model_throughput_evidence_surveillance_contract(&request()).unwrap();
