...
use bioprism_foundation::{
    AutonomyTier, CapabilityManifest, Determinism, EvidenceReference, EvidenceState, LossSeverity,
...
pub const CONTRACT_VERSION: &str =
    "onco-federated-continual-laboratory-integration-research-workbench/1.0";
...
fn invalid(message: impl Into<String>) -> OncoInstrumentError {
    OncoInstrumentError::Invalid(message.into())
}
...
impl OncoInstrumentReceipt5 {
    pub fn validate(&self) -> Result<(), OncoInstrumentError> {
        if self.schema_version != RESEARCH_CONTRACT_SCHEMA_VERSION
            ...
            || self.worldline_order.is_empty()
            || self.specimen_order.is_empty()
            || self.effect_receipts.is_empty()
        {
            return Err(invalid("OncoWorld instrument identity, axes, locality, boundary, or effects are incomplete"));
        }
        for values in [
            &self.action_order,
            ...
            &self.worldline_order,
            &self.specimen_order,
            &self.modality_order,
            &self.selected_worldline_order,
            &self.selected_specimen_order,
            &self.omission_order,
            ...
            if !canonical(values) {
                return Err(invalid("OncoWorld instrument ordering is not canonical"));
            }
        }
        let all = self.action_order.iter().cloned().collect::<BTreeSet<_>>();
        let present_parts = self
        ...
        {
            return Err(invalid(
                "instrument action states do not form a complete partition",
            ));
        }
        ...
        {
            return Err(invalid("instrument replay or action digest is invalid"));
        }
        self.artifact
            .validate_metadata()
            .map_err(|e| OncoInstrumentError::Artifact(e.to_string()))?;
        if self.artifact.content_type != CONTENT_TYPE {
            return Err(invalid("OncoWorld instrument artifact type is invalid"));
        }
        if self.effect_receipts.iter().any(|effect| {
            !effect.starts_with("view:authorized-research-state:")
                && effect != "block:unsafe-release"
        }) {
            return Err(invalid("effect is outside the researcher interaction gate"));
        }
        if self.disposition == OncoInstrumentDisposition::Qualified
            && self.effect_receipts
                != [format!(
        ...
        {
            return Err(invalid("qualified instrument effect is invalid"));
        }
        if self.disposition != OncoInstrumentDisposition::Qualified
            && self.effect_receipts != ["block:unsafe-release"]
        {
            return Err(invalid("non-qualified instrument interaction must block"));
        }
        Ok(())
    }
    pub fn digest(&self) -> Result<ContentHash, OncoInstrumentError> {
        self.validate()?;
...
pub fn qualify_instrument_actions(
    request: &OncoInstrumentRequest6,
) -> Result<OncoInstrumentReceipt5, OncoInstrumentError> {
    ...
    let mut selected_worldlines = BTreeSet::new();
    let mut selected_specimens = BTreeSet::new();
    let mut selected_endpoints = BTreeSet::new();
    let mut selected_capabilities = BTreeSet::new();
    for action in &actions {
        if !action.world_scope
            || !action.permitted
            || !action.instrument_ready
            || !action.consented_preclinical
            || !action.local_only
        {
            blocked.insert(action.action_id.clone());
            omission.insert(format!("{}:scope-permission-readiness", action.action_id));
        } else if action.replay_identity != request.replay_identity
            || !action.scope_compatible
            || !matches!(
                action.evidence_state,
                EvidenceState::Proven | EvidenceState::Supported
            )
            || action.evidence_digest.is_none()
            || action.provenance_digest.is_none()
        {
            unresolved.insert(action.action_id.clone());
            if action.replay_identity != request.replay_identity {
                uncertainty.insert(format!("{}:replay-mismatch", action.action_id));
            }
            if !action.scope_compatible {
                uncertainty.insert(format!("{}:scope-incompatible", action.action_id));
            }
            if !matches!(
                action.evidence_state,
                EvidenceState::Proven | EvidenceState::Supported
            ) {
                uncertainty.insert(format!("{}:evidence-state", action.action_id));
            }
            if action.evidence_digest.is_none() || action.provenance_digest.is_none() {
                omission.insert(format!(
                    "{}:evidence-or-provenance-missing",
                    action.action_id
                ));
            }
        } else {
            selected.insert(action.action_id.clone());
            selected_worldlines.insert(action.worldline_id.clone());
            selected_specimens.insert(action.specimen_id.clone());
            selected_endpoints.insert(action.endpoint_id.clone());
            selected_capabilities.insert(action.capability_id.clone());
        }
        omission.extend(
            action
                .omission_order
                .iter()
                .map(|item| format!("{}:{item}", action.action_id)),
        );
    ...
    let capability_order = actions
        ...
        .collect::<BTreeSet<_>>();
    let worldline_order = actions
        .iter()
        .map(|a| a.worldline_id.clone())
        .collect::<BTreeSet<_>>();
    let specimen_order = actions
        .iter()
        .map(|a| a.specimen_id.clone())
        .collect::<BTreeSet<_>>();
    let modality_order = actions
        .iter()
        .map(|a| a.modality.clone())
        .collect::<BTreeSet<_>>();
    ...
    let missing_actions = required_actions
        .difference(&action_order.iter().cloned().collect())
        .cloned()
        .collect::<BTreeSet<_>>();
    let missing_endpoints = request
        .required_endpoint_order
        .iter()
        .cloned()
        .collect::<BTreeSet<_>>()
        .difference(&endpoint_order)
        .cloned()
        .collect::<BTreeSet<_>>();
    let missing_capabilities = request
        ...
        .cloned()
        .collect::<BTreeSet<_>>()
        .difference(&capability_order)
        .cloned()
        .collect::<BTreeSet<_>>();
    omission.extend(
        missing_actions
            .iter()
            .map(|id| format!("action:{id}:missing")),
    );
    omission.extend(
        missing_endpoints
            .iter()
    ...
    if global_block {
        blocked.extend(action_order.iter().cloned());
        selected.clear();
        unresolved.clear();
        omission.insert("request:instrument-release-gate-blocked".into());
    }
    let disposition = if global_block {
        OncoInstrumentDisposition::Blocked
    ...
    {
        OncoInstrumentDisposition::Unresolved
    } else {
        OncoInstrumentDisposition::Qualified
    };
    if disposition != OncoInstrumentDisposition::Qualified {
        omission.insert("request:instrument-interaction-not-release-ready".into());
    }
    let selected_action_order = selected.into_iter().collect::<Vec<_>>();
    let unresolved_action_order = unresolved.into_iter().collect::<Vec<_>>();
    let blocked_action_order = blocked.into_iter().collect::<Vec<_>>();
    let effect_receipts = if disposition == OncoInstrumentDisposition::Qualified {
        vec![format!(
            "view:authorized-research-state:{}",
            request.request_id
        )]
    ...
    let artifact = TypedResearchArtifact::from_payload(
        format!("onco-instrument-receipt:{}", request.request_id),
        CONTENT_TYPE,
        &payload,
        vec![SemanticLoss {
            field: "omission_order".into(),
            reason: "unmeasured and incomplete instrument evidence remains visible".into(),
            severity: LossSeverity::DecisionRelevant,
        }],
        vec![ProvenanceLink {
            source_id: request.request_id.clone(),
            relation: "qualified-oncoworld-instrument-attestations".into(),
            digest: action_digest.clone(),
        }],
    )
    .map_err(|e| OncoInstrumentError::Artifact(e.to_string()))?;
    let receipt = OncoInstrumentReceipt5 {
        ...
        capability_order: payload["capability_order"]
            ...
            .map(|v| v.as_str().unwrap().into())
            .collect(),
        selected_endpoint_order: payload["selected_endpoint_order"]
            .as_array()
            .unwrap()
            .iter()
            .map(|v| v.as_str().unwrap().into())
        ...
        selected_capability_order: payload["selected_capability_order"]
            ...
            .map(|v| v.as_str().unwrap().into())
            .collect(),
        missing_endpoint_order: payload["missing_endpoint_order"]
            .as_array()
            .unwrap()
            .iter()
            .map(|v| v.as_str().unwrap().into())
        ...
        missing_capability_order: payload["missing_capability_order"]
            ...
            .map(|v| v.as_str().unwrap().into())
            .collect(),
        worldline_order: payload["worldline_order"]
            .as_array()
            .unwrap()
            .iter()
            .map(|v| v.as_str().unwrap().into())
            .collect(),
        specimen_order: payload["specimen_order"]
            .as_array()
            .unwrap()
            .iter()
            .map(|v| v.as_str().unwrap().into())
        ...
        modality_order: payload["modality_order"]
            ...
            .map(|v| v.as_str().unwrap().into())
            .collect(),
        selected_worldline_order: payload["selected_worldline_order"]
            .as_array()
            .unwrap()
            .iter()
            .map(|v| v.as_str().unwrap().into())
            .collect(),
        selected_specimen_order: payload["selected_specimen_order"]
            .as_array()
            .unwrap()
            .iter()
            .map(|v| v.as_str().unwrap().into())
...
fn validate_request(request: &OncoInstrumentRequest6) -> Result<(), OncoInstrumentError> {
    if request.schema_version != INPUT_SCHEMA
        ...
        || !canonical(&request.required_action_order)
    ...
    {
        return Err(invalid("OncoWorld instrument request identity, closure, replay, locality, or boundary is invalid"));
    }
    let mut ids = BTreeSet::new();
    for action in &request.actions {
        if action.action_id.trim().is_empty()
            || action.endpoint_id.trim().is_empty()
            || action.capability_id.trim().is_empty()
            || action.worldline_id.trim().is_empty()
            || action.specimen_id.trim().is_empty()
            || action.modality.trim().is_empty()
            || action.protocol_version.trim().is_empty()
            || !ids.insert(action.action_id.clone())
            || !valid_digest(&action.parameters_digest)
            || !valid_digest(&action.replay_identity)
            || !action.evidence_digest.as_ref().is_none_or(valid_digest)
            || !action.provenance_digest.as_ref().is_none_or(valid_digest)
            || !canonical(&action.omission_order)
        {
            return Err(invalid(format!(
                "OncoWorld instrument action {} is malformed or duplicated",
                action.action_id
            )));
        }
    }
...
mod tests {
    ...
    fn request() -> OncoInstrumentRequest6 {
        let action = |id: &str| OncoInstrumentAction5 {
            action_id: id.into(),
            endpoint_id: format!("endpoint:{id}"),
            capability_id: format!("assay:{id}"),
            worldline_id: format!("worldline:{id}"),
            specimen_id: format!("specimen:{id}"),
            modality: "methylation".into(),
            protocol_version: "v1".into(),
            parameters_digest: hash(id),
            evidence_digest: Some(hash("evidence")),
            provenance_digest: Some(hash("provenance")),
            ...
            consented_preclinical: true,
            local_only: true,
            omission_order: Vec::new(),
            negative_result: false,
        };
        OncoInstrumentRequest6 {
            schema_version: INPUT_SCHEMA.into(),
            request_id: "onco-instrument-request".into(),
            requester: "benchmark-curator".into(),
            purpose: "preclinical assay benchmark".into(),
            semantic_profile: "onco-instrument:v1".into(),
            required_action_order: vec!["action:a".into(), "action:b".into()],
            required_endpoint_order: vec!["endpoint:action:a".into(), "endpoint:action:b".into()],
            required_capability_order: vec!["assay:action:a".into(), "assay:action:b".into()],
            actions: vec![action("action:a"), action("action:b")],
            replay_identity: hash("replay"),
            policy_allow: true,
    ...
    fn manifest_is_a1() {
        assert_eq!(
            instrument_research_workbench_manifest().autonomy_tier,
            AutonomyTier::A1
        );
    }
    #[test]
    fn qualified_actions_are_deterministic() {
        let receipt = qualify_instrument_actions(&request()).unwrap();
        assert_eq!(receipt.disposition, OncoInstrumentDisposition::Qualified);
        assert_eq!(receipt.selected_action_order.len(), 2);
        assert_eq!(receipt.digest().unwrap(), receipt.digest().unwrap());
    }
    #[test]
    fn missing_action_is_unresolved() {
        let mut req = request();
        req.actions.pop();
        let receipt = qualify_instrument_actions(&req).unwrap();
        assert_eq!(receipt.disposition, OncoInstrumentDisposition::Unresolved);
        assert!(receipt.missing_action_order.contains(&"action:b".into()));
    }
    #[test]
    fn denied_action_is_blocked_but_batch_unresolved() {
        let mut req = request();
        req.actions[0].permitted = false;
        let receipt = qualify_instrument_actions(&req).unwrap();
        assert!(receipt.blocked_action_order.contains(&"action:a".into()));
        assert_eq!(receipt.disposition, OncoInstrumentDisposition::Unresolved);
    }
    #[test]
    fn adversarial_request_blocks_all() {
        ...
        let receipt = qualify_instrument_actions(&req).unwrap();
        assert_eq!(receipt.disposition, OncoInstrumentDisposition::Blocked);
        assert!(receipt.selected_action_order.is_empty());
    }
    #[test]
    fn negative_result_is_preserved() {
        let mut req = request();
        req.actions[0].negative_result = true;
        let receipt = qualify_instrument_actions(&req).unwrap();
        assert!(receipt
            .negative_evidence_order
            .contains(&"action:a:negative-result".into()));
    }
    #[test]
    fn replay_mismatch_is_unresolved() {
        let mut req = request();
        req.actions[0].replay_identity = hash("other");
        let receipt = qualify_instrument_actions(&req).unwrap();
        assert!(receipt.unresolved_action_order.contains(&"action:a".into()));
        assert!(receipt
            .uncertainty_order
            .contains(&"action:a:replay-mismatch".into()));
    }
}
