//! Federated continual statistical/causal/ML analysis researcher workbench.
//!
//! Atlas feature: `AFA-oncoworlds-P13-F20`.
//!
//! The workbench admits typed analysis-result attestations for an institution-local portfolio and
//! produces a deterministic, digest-only researcher view. It never executes a model or exports
//! raw data; it makes the distinction between qualified, unresolved, blocked, missing, and
//! negative results explicit before a researcher interprets a result.
...
pub const FEATURE_ID: &str = "AFA-oncoworlds-P13-F20";
pub const CONTRACT_VERSION: &str =
    "oncoworlds-federated-continual-statistical-causal-ml-analysis-research-workbench/1.0";
pub const INPUT_SCHEMA: &str = "AnalysisQuestion4@1";
pub const OUTPUT_SCHEMA: &str = "QualifiedAnalysisResult8@1";
pub const CONTENT_TYPE: &str = "application/vnd.aurora.oncoworlds-analysis-workbench-result-8+json";

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct OncoworldsAnalysisCandidate8 {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct OncoworldsAnalysisWorkbenchRequest {
    ...
    pub candidates: Vec<OncoworldsAnalysisCandidate8>,
    pub replay_identity: ContentHash,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct OncoworldsAnalysisWorkbenchReceipt {
    pub schema_version: String,
...
pub enum OncoworldsAnalysisWorkbenchError {
...
impl OncoworldsAnalysisWorkbenchReceipt {
    pub fn validate(&self) -> Result<(), OncoworldsAnalysisWorkbenchError> {
        ...
        {
            return Err(OncoworldsAnalysisWorkbenchError::Invalid(
        ...
        if self.effect_receipts.iter().any(|effect| {
            !effect.starts_with("view:analysis-workbench:")
        ...
        self.artifact
            .validate_metadata()
            .map_err(|error| OncoworldsAnalysisWorkbenchError::Artifact(error.to_string()))
    }

    pub fn digest(&self) -> Result<ContentHash, OncoworldsAnalysisWorkbenchError> {
        self.validate()?;
...
pub fn oncoworlds_analysis_workbench_manifest() -> CapabilityManifest {
    CapabilityManifest {
        schema_version: RESEARCH_CONTRACT_SCHEMA_VERSION.into(), capability_id: FEATURE_ID.into(), version: CONTRACT_VERSION.into(), owner_crate: "oncoworlds".into(),
        consumers: BTreeSet::from(["preclinical neuroscientist".into(), "analysis workbench operator".into(), "federated research verifier".into()]),
        behavior: "presents typed federated statistical, causal, and ML analysis attestations in a deterministic researcher workbench without executing models or moving raw data".into(),
        value: "lets preclinical neuroscientists compare qualified analytical results while preserving replay, provenance, negative evidence, and zero-versus-unknown distinctions".into(),
        ...
        permissions: BTreeSet::from(["view:analysis-workbench".into()]),
        ...
        authority_requirements: vec![AuthorityRequirement { role: "institution analysis reviewer".into(), reason: "researcher-facing interpretation requires scoped read authority over local attestations".into() }],
        autonomy_tier: AutonomyTier::A1,
        surfaces: BTreeSet::from([ResearchSurface::Ui, ResearchSurface::Cli, ResearchSurface::Api, ResearchSurface::Sdk, ResearchSurface::McpTool, ResearchSurface::Protocol, ResearchSurface::Policy, ResearchSurface::Operator]),
        boundary: PRECLINICAL_BOUNDARY.into(),
    }
}

fn validate_request(
    request: &OncoworldsAnalysisWorkbenchRequest,
) -> Result<(), OncoworldsAnalysisWorkbenchError> {
    ...
    {
        return Err(OncoworldsAnalysisWorkbenchError::Invalid("analysis identity, axes, candidates, budget, locality, aggregate boundary, or schema is invalid".into()));
    }
    let ids = request
        .candidates
        .iter()
...
pub fn qualify_oncoworlds_analysis_workbench(
    request: &OncoworldsAnalysisWorkbenchRequest,
) -> Result<OncoworldsAnalysisWorkbenchReceipt, OncoworldsAnalysisWorkbenchError> {
    ...
    let mut effects = if disposition == "qualified" {
        vec![
            format!("view:analysis-workbench:{}", request.request_id),
        ]
    } else {
    ...
    let artifact = TypedResearchArtifact::from_payload(
        ...
        vec![ProvenanceLink {
            ...
            relation: "oncoworlds-analysis-workbench".into(),
            digest: result_digest.clone(),
        }],
    )
    .map_err(|error| OncoworldsAnalysisWorkbenchError::Artifact(error.to_string()))?;
    let receipt = OncoworldsAnalysisWorkbenchReceipt {
...
mod tests {
    ...
    fn candidate(id: &str, model: &str) -> OncoworldsAnalysisCandidate8 {
        OncoworldsAnalysisCandidate8 {
    ...
    fn request() -> OncoworldsAnalysisWorkbenchRequest {
    ...
    fn qualified_analysis_emits_bounded_effects() {
        let receipt = qualify_oncoworlds_analysis_workbench(&request()).unwrap();
        assert_eq!(receipt.disposition, "qualified");
        assert!(receipt
            .effect_receipts
            .iter()
            .any(|effect| effect.starts_with("view:analysis-workbench:")));
    }
    #[test]
    fn missing_artifact_is_unresolved() {
        let mut value = request();
        value.candidates[0].artifact_digest = None;
        let receipt = qualify_oncoworlds_analysis_workbench(&value).unwrap();
    ...
    fn policy_federation_and_adversarial_gates_block() {
        ...
        assert_eq!(
            qualify_oncoworlds_analysis_workbench(&value).unwrap().disposition,
            "blocked"
        );
    ...
    #[test]
    fn manifest_is_a1_byte_stable_and_preclinical() {
        let manifest = oncoworlds_analysis_workbench_manifest();
