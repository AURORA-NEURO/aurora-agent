...
//!
//! Atlas feature `AFA-oncoworlds-P01-F11`: a bounded A2 agent surface for
...
pub const FEATURE_ID: &str = "AFA-oncoworlds-P01-F11";
pub const CONTRACT_VERSION: &str = "oncoworlds-prospective-high-throughput-evidence-surveillance-research-copilot/1.0";
pub const INPUT_SCHEMA: &str = "EvidenceFeed3@1";
pub const OUTPUT_SCHEMA: &str = "QualifiedEvidenceSet3@1";

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct OncoworldsEvidenceObservation {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct OncoworldsEvidenceSurveillanceCopilotRequest {
    ...
    pub observations: Vec<OncoworldsEvidenceObservation>,
...
pub enum OncoworldsEvidenceCopilotDisposition {
    Completed,
    Partial,
    Unknown,
    Blocked,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct OncoworldsQualifiedEvidenceSet {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct OncoworldsEvidenceSurveillanceCopilotReceipt {
    pub schema_version: String,
    ...
    pub checkpoint_seq: u64,
    pub capacity: usize,
    pub disposition: OncoworldsEvidenceCopilotDisposition,
    ...
    pub qualified_set: OncoworldsQualifiedEvidenceSet,
...
pub enum OncoworldsEvidenceSurveillanceCopilotError {
...
impl OncoworldsEvidenceSurveillanceCopilotReceipt {
    pub fn validate(&self) -> Result<(), OncoworldsEvidenceSurveillanceCopilotError> {
        ...
        {
            return Err(OncoworldsEvidenceSurveillanceCopilotError::Invalid("identity, checkpoint, locality, candidates, effects, or qualified-set linkage is incomplete".into()));
        }
        for values in [
            ...
            if !ordered(values) {
                return Err(OncoworldsEvidenceSurveillanceCopilotError::Invalid(
        ...
        if self.disposition == OncoworldsEvidenceCopilotDisposition::Blocked
...
pub fn oncoworlds_evidence_surveillance_copilot_manifest() -> CapabilityManifest {
    CapabilityManifest {
        schema_version: RESEARCH_CONTRACT_SCHEMA_VERSION.into(),
        capability_id: FEATURE_ID.into(),
        version: CONTRACT_VERSION.into(),
        owner_crate: "oncoworlds".into(),
        consumers: [
...
pub fn run_oncoworlds_evidence_surveillance_copilot(
    request: &OncoworldsEvidenceSurveillanceCopilotRequest,
) -> Result<
...
> {
    ...
    let disposition = if !request.policy_allow
        ...
        || approval_missing
    {
        OncoworldsEvidenceCopilotDisposition::Blocked
    } else if selected.is_empty() {
        OncoworldsEvidenceCopilotDisposition::Unknown
    } else if !unresolved.is_empty() || !denied.is_empty() || !overflow.is_empty() {
        OncoworldsEvidenceCopilotDisposition::Partial
    } else {
        OncoworldsEvidenceCopilotDisposition::Completed
    };
    ...
    let tool_receipts = if disposition == OncoworldsEvidenceCopilotDisposition::Blocked {
    ...
    let queue_digest = ContentHash::of_value(&json!({"batch_id": request.batch_id, "capacity": request.capacity, "candidate_order": candidate_order, "checkpoint_seq": request.checkpoint_seq})).map_err(|e| OncoworldsEvidenceSurveillanceCopilotError::Artifact(e.to_string()))?;
    let checkpoint_digest = ContentHash::of_value(&json!({"batch_id": request.batch_id, "checkpoint_seq": request.checkpoint_seq, "replay_identity": request.replay_identity})).map_err(|e| OncoworldsEvidenceSurveillanceCopilotError::Artifact(e.to_string()))?;
    let evidence_digest = ContentHash::of_value(&json!({"selected_order": selected_order, "unresolved_order": unresolved_order, "denied_order": denied_order, "overflow_order": overflow_order})).map_err(|e| OncoworldsEvidenceSurveillanceCopilotError::Artifact(e.to_string()))?;
    let provenance_digest = ContentHash::of_value(&json!({"request_id": request.request_id, "agent_id": request.agent_id, "queue_digest": queue_digest, "checkpoint_digest": checkpoint_digest, "evidence_digest": evidence_digest})).map_err(|e| OncoworldsEvidenceSurveillanceCopilotError::Artifact(e.to_string()))?;
    let run_digest = ContentHash::of_value(&json!({"request_id": request.request_id, "dry_run": request.dry_run, "tool_receipts": tool_receipts, "provenance_digest": provenance_digest})).map_err(|e| OncoworldsEvidenceSurveillanceCopilotError::Artifact(e.to_string()))?;
    let qualified_set = OncoworldsQualifiedEvidenceSet {
        schema_version: RESEARCH_CONTRACT_SCHEMA_VERSION.into(),
        set_id: format!(
        ...
        evidence_state: if disposition == OncoworldsEvidenceCopilotDisposition::Completed {
    ...
    let artifact_payload = serde_json::to_value(&qualified_set)
        .map_err(|e| OncoworldsEvidenceSurveillanceCopilotError::Artifact(e.to_string()))?;
    ...
    let receipt = OncoworldsEvidenceSurveillanceCopilotReceipt {
        ...
        negative_evidence,
        tool_receipts,
        effect_receipts: if disposition == OncoworldsEvidenceCopilotDisposition::Blocked {
...
mod tests {
    use super::*;
    fn request(dry_run: bool) -> OncoworldsEvidenceSurveillanceCopilotRequest {
        OncoworldsEvidenceSurveillanceCopilotRequest {
            ...
            observations: (0..3)
                .map(|sequence| OncoworldsEvidenceObservation {
    ...
    fn manifest_is_a2() {
        assert_eq!(
            oncoworlds_evidence_surveillance_copilot_manifest().autonomy_tier,
            AutonomyTier::A2
        );
    }
    #[test]
    fn complete_batch_dry_runs() {
        let receipt =
            run_oncoworlds_evidence_surveillance_copilot(&request(true)).unwrap();
        assert_eq!(
            receipt.disposition,
            OncoworldsEvidenceCopilotDisposition::Completed
        );
    ...
    fn capacity_overflow_is_explicit() {
        ...
        input.capacity = 2;
        let receipt = run_oncoworlds_evidence_surveillance_copilot(&input).unwrap();
    ...
    fn approved_invocation_is_declared() {
        let receipt =
            run_oncoworlds_evidence_surveillance_copilot(&request(false)).unwrap();
    ...
    fn replay_is_stable() {
        ...
        let first = run_oncoworlds_evidence_surveillance_copilot(&input).unwrap();
        let second = run_oncoworlds_evidence_surveillance_copilot(&input).unwrap();
