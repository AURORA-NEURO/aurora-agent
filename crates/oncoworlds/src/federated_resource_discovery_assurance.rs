//! Federated continual resource-discovery assurance for `AFA-oncoworlds-P05-F28`.
//!
//! This module is deliberately below the foundation crate: `oncoworlds` owns the domain contract and
...
pub const FEATURE_ID: &str = "AFA-oncoworlds-P05-F28";
pub const CONTRACT_VERSION: &str =
    "oncoworlds-federated-continual-resource-discovery-assurance-harness/1.0";
pub const INPUT_SCHEMA: &str = "OncoworldsResourceNeed4@1";
pub const OUTPUT_SCHEMA: &str = "OncoworldsQualifiedResourceSet7@1";
pub const CONTENT_TYPE: &str = "application/vnd.aurora.qualified-resource-set-7+json";
...
pub struct OncoworldsResourceNeed4 {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct OncoworldsResourceEndpoint4 {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct OncoworldsPeerResourceSummary4 {
    pub peer_id: String,
    pub origin: String,
    pub semantic_profile: String,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct OncoworldsResourceManifest {
...
#[serde(rename_all = "snake_case")]
pub enum OncoworldsResourceDisposition {
    Qualified,
    Unresolved,
    Blocked,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct OncoworldsQualifiedResource7 {
    pub resource_id: String,
    pub endpoint_id: String,
    pub origin: String,
    pub protocol_version: String,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct OncoworldsResourceArtifact7 {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct OncoworldsQualifiedResourceSet7 {
    pub schema_version: String,
    ...
    pub disposition: OncoworldsResourceDisposition,
    ...
    pub resources: Vec<OncoworldsQualifiedResource7>,
    ...
    pub artifact: OncoworldsResourceArtifact7,
...
pub enum OncoworldsResourceDiscoveryError {
...
pub fn oncoworlds_resource_discovery_manifest() -> OncoworldsResourceManifest {
    OncoworldsResourceManifest {
        schema_version: "aurora-research-contract/1.0".into(),
        capability_id: FEATURE_ID.into(),
        version: CONTRACT_VERSION.into(),
        input_schema: INPUT_SCHEMA.into(),
        output_schema: OUTPUT_SCHEMA.into(),
        effects: vec!["verify:resource-registry".into()],
        permissions: vec!["evaluate:capability-runs".into()],
        autonomy_tier: "A1".into(),
        boundary: PRECLINICAL_BOUNDARY.into(),
    }
}

impl OncoworldsQualifiedResourceSet7 {
    pub fn validate(&self) -> Result<(), OncoworldsResourceDiscoveryError> {
        ...
        {
            return Err(OncoworldsResourceDiscoveryError::Invalid(
        ...
        {
            return Err(OncoworldsResourceDiscoveryError::Artifact(
                "artifact content type or digest is inconsistent".into(),
            ));
        }
        if self.effect_receipts.iter().any(|effect| {
            !effect.starts_with("verify:resource-registry:") && effect != "block:unsafe-release"
        }) {
            return Err(OncoworldsResourceDiscoveryError::Invalid(
    ...
    pub fn digest(&self) -> Result<ContentHash, OncoworldsResourceDiscoveryError> {
        self.validate()?;
        ContentHash::of_value(
            &serde_json::to_value(self)
                .map_err(|error| OncoworldsResourceDiscoveryError::Artifact(error.to_string()))?,
        )
        .map_err(|error| OncoworldsResourceDiscoveryError::Artifact(error.to_string()))
    }
}

pub fn assure_oncoworlds_resources(
    request: &OncoworldsResourceNeed4,
    endpoints: &[OncoworldsResourceEndpoint4],
    peers: &[OncoworldsPeerResourceSummary4],
) -> Result<OncoworldsQualifiedResourceSet7, OncoworldsResourceDiscoveryError> {
    ...
    for endpoint in &endpoint_rows {
        ...
        if reasons.iter().any(|reason| {
            ...
            qualified.push(OncoworldsQualifiedResource7 {
                resource_id: id,
    ...
    let disposition = if global_block || !blocked.is_empty() {
        OncoworldsResourceDisposition::Blocked
    } else if qualified_peers.len() < request.minimum_peer_quorum || qualified.is_empty() {
        OncoworldsResourceDisposition::Unresolved
    } else {
        OncoworldsResourceDisposition::Qualified
    };
    if disposition != OncoworldsResourceDisposition::Qualified && qualified.is_empty() {
    ...
    let provenance_digests = endpoint_rows
        ...
        .into_iter()
        .collect::<Vec<_>>();
    let artifact = OncoworldsResourceArtifact7 {
        artifact_id: format!("qualified-resource-set-7:{}", request.request_id),
    ...
    let effects = if disposition == OncoworldsResourceDisposition::Qualified {
        vec![format!(
    ...
    let receipt = OncoworldsQualifiedResourceSet7 {
        schema_version: "aurora-research-contract/1.0".into(),
...
fn validate_request(
    ...
    endpoints: &[OncoworldsResourceEndpoint4],
    peers: &[OncoworldsPeerResourceSummary4],
) -> Result<(), OncoworldsResourceDiscoveryError> {
...
mod tests {
    ...
    fn endpoint(id: &str, fitness: i64, state: EvidenceState) -> OncoworldsResourceEndpoint4 {
        OncoworldsResourceEndpoint4 {
            resource_id: id.into(),
    ...
    fn peer(id: &str, state: EvidenceState) -> OncoworldsPeerResourceSummary4 {
        OncoworldsPeerResourceSummary4 {
            peer_id: id.into(),
            origin: id.into(),
            semantic_profile: "resource:v1".into(),
    ...
    fn request() -> OncoworldsResourceNeed4 {
        OncoworldsResourceNeed4 {
    ...
    #[test]
    fn manifest_is_typed_a1_and_verify_only() {
        let manifest = oncoworlds_resource_discovery_manifest();
        assert_eq!(manifest.autonomy_tier, "A1");
        assert_eq!(manifest.effects, vec!["verify:resource-registry"]);
    }
    #[test]
    fn deterministic_fitness_order_and_qualified_verification() {
        let result = assure_oncoworlds_resources(
            &request(),
            &[
        ...
        .unwrap();
        assert_eq!(result.disposition, OncoworldsResourceDisposition::Qualified);
        assert_eq!(result.qualified_order, vec!["resource:a", "resource:b"]);
        assert_eq!(
            result.effect_receipts,
            vec!["verify:resource-registry:request:resource"]
        );
    ...
    fn unknown_evidence_is_unresolved_and_blocked_from_exchange() {
        ...
        .unwrap();
        assert_eq!(result.disposition, OncoworldsResourceDisposition::Unresolved);
        assert!(result
            .uncertainty
            .iter()
    ...
    fn contradiction_is_negative_and_blocked() {
        ...
        .unwrap();
        assert_eq!(result.disposition, OncoworldsResourceDisposition::Blocked);
        assert!(result
            .negative_evidence
            .iter()
    ...
    #[test]
    fn adversarial_event_fails_closed_and_is_retained() {
        let mut value = request();
        value.adversarial_event_order = vec!["poisoned-manifest".into()];
        let result = assure_oncoworlds_resources(
            &value,
            &[endpoint("resource:a", 900, EvidenceState::Supported)],
            &[peer("peer:a", EvidenceState::Supported)],
        ...
        assert!(result
            .negative_evidence
            .iter()
            .any(|item| item.contains("adversarial:poisoned-manifest")));
    }
    #[test]
