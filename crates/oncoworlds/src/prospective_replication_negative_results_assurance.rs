...
//!
//! Atlas feature: `AFA-oncoworlds-P15-F28`.  The harness turns caller-attested
...
pub const FEATURE_ID: &str = "AFA-oncoworlds-P15-F28";
pub const CONTRACT_VERSION: &str =
    "oncoworlds-federated-continual-replication-negative-results-assurance-harness/1.0";
pub const INPUT_SCHEMA: &str = "OncoworldsClaimAndProtocol4@1";
pub const OUTPUT_SCHEMA: &str = "OncoworldsReplicationRecord7@1";
const CONTENT_TYPE: &str = "application/vnd.aurora.replication-record-7+json";

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum OncoworldsReplicationOutcome {
    Reproduced,
    Partial,
    Null,
    Failed,
...
pub struct OncoworldsReplicationClaim {
    pub claim_id: String,
    pub study_id: String,
    ...
    pub outcome: OncoworldsReplicationOutcome,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct OncoworldsClaimAndProtocol {
    ...
    pub protocol_digest: ContentHash,
    pub replay_identity: ContentHash,
    pub claims: Vec<OncoworldsReplicationClaim>,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum OncoworldsReplicationDisposition {
    Qualified,
    Unresolved,
    Blocked,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct OncoworldsReplicationRecord {
    pub schema_version: String,
    ...
    pub federation_id: String,
    pub semantic_profile: String,
    pub disposition: OncoworldsReplicationDisposition,
...
pub enum OncoworldsReplicationAssuranceError {
    #[error("invalid replication claim and protocol: {0}")]
    Invalid(String),
    #[error("replication record artifact failed: {0}")]
    Artifact(String),
}
fn invalid(value: impl Into<String>) -> OncoworldsReplicationAssuranceError {
    OncoworldsReplicationAssuranceError::Invalid(value.into())
}
...
impl OncoworldsReplicationRecord {
    pub fn validate(&self) -> Result<(), OncoworldsReplicationAssuranceError> {
        ...
        self.artifact
            .validate_metadata()
            .map_err(|error| OncoworldsReplicationAssuranceError::Artifact(error.to_string()))?;
        if self.artifact.content_type != CONTENT_TYPE {
            return Err(invalid("replication artifact type is invalid"));
        }
        if self.disposition == OncoworldsReplicationDisposition::Qualified
...
pub fn assure_oncoworlds_replication(
    ...
    for row in &claims {
        if matches!(
            row.outcome,
            OncoworldsReplicationOutcome::Null | OncoworldsReplicationOutcome::Failed
        ) {
            ...
            )
        {
            ...
            if row.outcome == OncoworldsReplicationOutcome::Reproduced {
    ...
    let disposition = if global_block {
        OncoworldsReplicationDisposition::Blocked
    } else if required.is_subset(&admitted) && unresolved.is_empty() && blocked.is_empty() {
        OncoworldsReplicationDisposition::Qualified
    } else {
        OncoworldsReplicationDisposition::Unresolved
    };
    ...
    let effect_receipts = if disposition == OncoworldsReplicationDisposition::Qualified {
    ...
    .map_err(|error| OncoworldsReplicationAssuranceError::Artifact(error.to_string()))?;
    let record = OncoworldsReplicationRecord {
...
fn validate_input(input: &OncoworldsClaimAndProtocol) -> Result<(), OncoworldsReplicationAssuranceError> {
...
mod tests {
    ...
    fn input() -> OncoworldsClaimAndProtocol {
        let d = hash("replication");
        let claim = |id: &str, outcome: OncoworldsReplicationOutcome| OncoworldsReplicationClaim {
            claim_id: id.into(),
        ...
        OncoworldsClaimAndProtocol {
            ...
            replay_identity: d.clone(),
            claims: vec![
                claim("claim:a", OncoworldsReplicationOutcome::Reproduced),
                claim("claim:b", OncoworldsReplicationOutcome::Null),
            ],
            policy_allow: true,
    ...
    fn manifest_is_a1() {
        assert_eq!(
            oncoworlds_replication_negative_results_manifest().autonomy_tier,
            AutonomyTier::A1
        );
    }
    #[test]
    fn qualified_replication_keeps_null() {
        let out = assure_oncoworlds_replication(&input()).unwrap();
        assert_eq!(out.disposition, OncoworldsReplicationDisposition::Qualified);
        assert!(out.negative_result_order.contains(&"claim:b".into()));
    }
    #[test]
    fn deterministic_record() {
        let a = assure_oncoworlds_replication(&input()).unwrap();
        let b = assure_oncoworlds_replication(&input()).unwrap();
    ...
    fn missing_claim_unresolved() {
        ...
        assert_eq!(
            assure_oncoworlds_replication(&value).unwrap().disposition,
            OncoworldsReplicationDisposition::Unresolved
        );
    }
    #[test]
    fn contradictory_claim_blocked() {
        ...
        let out = assure_oncoworlds_replication(&value).unwrap();
