...
//! Atlas feature `AFA-worldgen-P01-F19`: a bounded A1 interaction surface over the
...
pub const FEATURE_ID: &str = "AFA-worldgen-P01-F19";
pub const CONTRACT_VERSION: &str =
    "worldgen-throughput-evidence-surveillance-research-workbench/1.0";
...
impl ThroughputEvidenceSurveillanceResearchWorkbenchReceipt {
    pub fn validate(&self) -> Result<(), ThroughputEvidenceSurveillanceResearchWorkbenchError> {
        ...
        if self.effect_receipts.iter().any(|e| {
            !e.starts_with("view:throughput-evidence-workbench:") && e != "block:unsafe-release"
        }) {
            return Err(
                ThroughputEvidenceSurveillanceResearchWorkbenchError::Invalid(
...
> {
    ...
    let workbench_digest=ContentHash::of_value(&json!({"workbench_id":request.workbench_id,"scope":request.scope,"batch_id":request.copilot_request.batch_id,"checkpoint_seq":request.copilot_request.checkpoint_seq,"capacity":request.copilot_request.capacity,"views":views,"panels":panels,"candidate":candidate,"qualified":qualified,"unknown":unknown,"blocked":blocked,"overflow":overflow,"replay_identity":request.replay_identity,"copilot_run_digest":copilot_run_digest})).map_err(|e|ThroughputEvidenceSurveillanceResearchWorkbenchError::Artifact(e.to_string()))?;
    let mut omissions = c.omissions.clone();
    omissions.push("workbench:read-only-throughput-view".into());
    omissions.sort();
    omissions.dedup();
    let payload = json!({"schema_version":RESEARCH_CONTRACT_SCHEMA_VERSION,"contract_version":CONTRACT_VERSION,"feature_id":FEATURE_ID,"request_id":request.copilot_request.request_id,"workbench_id":request.workbench_id,"scope":request.scope,"batch_id":request.copilot_request.batch_id,"checkpoint_seq":request.copilot_request.checkpoint_seq,"capacity":request.copilot_request.capacity,"disposition":c.disposition,"view_order":views,"panel_order":panels,"candidate_order":candidate,"qualified_order":qualified,"unknown_order":unknown,"blocked_order":blocked,"overflow_order":overflow,"replay_identity":request.replay_identity,"copilot_run_digest":copilot_run_digest,"queue_digest":queue_digest,"checkpoint_digest":checkpoint_digest,"omissions":omissions,"uncertainty":c.uncertainty,"negative_evidence":c.negative_evidence,"boundary":PRECLINICAL_BOUNDARY,"raw_data_local":true});
    let artifact = TypedResearchArtifact::from_payload(
        format!(
            "worldgen-throughput-evidence-workbench:{}",
...
mod tests {
    ...
    fn policy_denial_visible() {
        ...
        assert!(render_throughput_evidence_surveillance_research_workbench(&r).is_ok())
    }
    #[test]
