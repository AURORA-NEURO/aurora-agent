//! Version-negotiated federated-continual evidence-surveillance interoperability gateway.
//! Atlas feature `AFA-worldgen-P01-F24`.
//!
...
use crate::federated_continual_evidence_surveillance_research_workbench::{
...
pub const FEATURE_ID: &str = "AFA-worldgen-P01-F24";
pub const CONTRACT_VERSION: &str =
    "worldgen-federated-continual-evidence-surveillance-interoperability-gateway/1.0";
...
pub struct FederatedContinualEvidenceSurveillanceInteroperabilityGatewayRequest {
    pub interop_request: InteroperabilityRequest,
    pub workbench_request: FederatedContinualEvidenceSurveillanceResearchWorkbenchRequest,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct FederatedContinualEvidenceSurveillanceInteroperabilityGatewayReceipt {
...
pub enum FederatedContinualEvidenceSurveillanceInteroperabilityGatewayError {
...
impl FederatedContinualEvidenceSurveillanceInteroperabilityGatewayReceipt {
    pub fn validate(&self) -> Result<(), FederatedContinualEvidenceSurveillanceInteroperabilityGatewayError> {
        ...
        self.artifact.validate_metadata().map_err(|error| {
            FederatedContinualEvidenceSurveillanceInteroperabilityGatewayError::Artifact(error.to_string())
        })
    }

    fn invalid(message: &str) -> FederatedContinualEvidenceSurveillanceInteroperabilityGatewayError {
        FederatedContinualEvidenceSurveillanceInteroperabilityGatewayError::Invalid(message.into())
    }
}

pub fn federated_continual_evidence_surveillance_interoperability_gateway_manifest() -> CapabilityManifest {
    CapabilityManifest {
        ...
        behavior: "negotiates a version-pinned federated-continual evidence-surveillance protocol and projects a typed workbench artifact".into(),
        value: "makes API/protocol compatibility, semantic loss, omissions, and local-only effects machine-checkable".into(),
        inputs: vec![
            TypedPort { name: "federated_continual_evidence_feed".into(), schema: INPUT_SCHEMA.into(), required: true },
...
pub fn render_federated_continual_evidence_surveillance_interoperability_gateway(
    request: &FederatedContinualEvidenceSurveillanceInteroperabilityGatewayRequest,
) -> Result<
    FederatedContinualEvidenceSurveillanceInteroperabilityGatewayReceipt,
...
> {
    ...
    let integration = negotiate_interoperability(&request.interop_request).map_err(|error| {
        FederatedContinualEvidenceSurveillanceInteroperabilityGatewayError::Invalid(error.to_string())
    })?;
    let workbench = render_federated_continual_evidence_surveillance_research_workbench(
        &request.workbench_request,
    )
    .map_err(|error| {
        FederatedContinualEvidenceSurveillanceInteroperabilityGatewayError::Workbench(error.to_string())
    })?;
    ...
    if loss.len() as u32 > request.semantic_loss_budget {
        return Err(
            FederatedContinualEvidenceSurveillanceInteroperabilityGatewayError::Invalid(
    ...
    let artifact = TypedResearchArtifact::from_payload(
        ...
        "application/vnd.aurora.worldgen-federated-continual-evidence-surveillance-interoperability+json",
        &protocol_payload,
        Vec::new(),
        Vec::new(),
    )
    ...
    let receipt = FederatedContinualEvidenceSurveillanceInteroperabilityGatewayReceipt {
        schema_version: RESEARCH_CONTRACT_SCHEMA_VERSION.into(),
...
fn validate_request(
    request: &FederatedContinualEvidenceSurveillanceInteroperabilityGatewayRequest,
) -> Result<(), FederatedContinualEvidenceSurveillanceInteroperabilityGatewayError> {
    ...
    {
        return Err(FederatedContinualEvidenceSurveillanceInteroperabilityGatewayError::Invalid("protocol schemas, migration policy, loss budget, locality, and boundary are invalid".into()));
    }
    Ok(())
}
...
mod tests {
    ...
    fn manifest_is_a1_and_pinned() {
        let manifest = federated_continual_evidence_surveillance_interoperability_gateway_manifest();
        assert_eq!(manifest.autonomy_tier, AutonomyTier::A2);
        assert_eq!(TARGET_PROTOCOL_VERSION, "1.0.0");
        assert_eq!(INPUT_SCHEMA, "EvidenceFeed4@1");
    }
}
