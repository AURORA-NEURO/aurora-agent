//! Version-negotiated local evidence-surveillance interoperability gateway.
//! Atlas feature `AFA-worldgen-P01-F21`.
//!
...
use crate::local_evidence_surveillance_research_workbench::{
    render_local_evidence_surveillance_research_workbench,
    LocalEvidenceSurveillanceResearchWorkbenchRequest,
};

pub const FEATURE_ID: &str = "AFA-worldgen-P01-F21";
pub const CONTRACT_VERSION: &str =
    "worldgen-local-evidence-surveillance-interoperability-gateway/1.0";
...
pub struct LocalEvidenceSurveillanceInteroperabilityGatewayRequest {
    pub interop_request: InteroperabilityRequest,
    pub workbench_request: LocalEvidenceSurveillanceResearchWorkbenchRequest,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct LocalEvidenceSurveillanceInteroperabilityGatewayReceipt {
...
pub enum LocalEvidenceSurveillanceInteroperabilityGatewayError {
...
impl LocalEvidenceSurveillanceInteroperabilityGatewayReceipt {
    pub fn validate(&self) -> Result<(), LocalEvidenceSurveillanceInteroperabilityGatewayError> {
        ...
        self.artifact.validate_metadata().map_err(|error| {
            LocalEvidenceSurveillanceInteroperabilityGatewayError::Artifact(error.to_string())
        })
    }

    fn invalid(message: &str) -> LocalEvidenceSurveillanceInteroperabilityGatewayError {
        LocalEvidenceSurveillanceInteroperabilityGatewayError::Invalid(message.into())
    }
}

pub fn local_evidence_surveillance_interoperability_gateway_manifest() -> CapabilityManifest {
    CapabilityManifest {
        ...
        behavior: "negotiates a version-pinned local evidence-surveillance protocol and projects a typed workbench artifact".into(),
        value: "makes API/protocol compatibility, semantic loss, omissions, and local-only effects machine-checkable".into(),
        inputs: vec![
            TypedPort { name: "local_evidence_feed".into(), schema: INPUT_SCHEMA.into(), required: true },
...
pub fn render_local_evidence_surveillance_interoperability_gateway(
    request: &LocalEvidenceSurveillanceInteroperabilityGatewayRequest,
) -> Result<
    LocalEvidenceSurveillanceInteroperabilityGatewayReceipt,
...
> {
    ...
    let integration = negotiate_interoperability(&request.interop_request).map_err(|error| {
        LocalEvidenceSurveillanceInteroperabilityGatewayError::Invalid(error.to_string())
    })?;
    let workbench = render_local_evidence_surveillance_research_workbench(
        &request.workbench_request,
    )
    .map_err(|error| {
        LocalEvidenceSurveillanceInteroperabilityGatewayError::Workbench(error.to_string())
    })?;
    ...
    if loss.len() as u32 > request.semantic_loss_budget {
        return Err(
            LocalEvidenceSurveillanceInteroperabilityGatewayError::Invalid(
    ...
    let artifact = TypedResearchArtifact::from_payload(
        ...
        "application/vnd.aurora.local-evidence-surveillance-interoperability+json",
        &protocol_payload,
        Vec::new(),
        Vec::new(),
    )
    ...
    let receipt = LocalEvidenceSurveillanceInteroperabilityGatewayReceipt {
        schema_version: RESEARCH_CONTRACT_SCHEMA_VERSION.into(),
...
fn validate_request(
    request: &LocalEvidenceSurveillanceInteroperabilityGatewayRequest,
) -> Result<(), LocalEvidenceSurveillanceInteroperabilityGatewayError> {
    ...
    {
        return Err(LocalEvidenceSurveillanceInteroperabilityGatewayError::Invalid("protocol schemas, migration policy, loss budget, locality, and boundary are invalid".into()));
    }
    Ok(())
}
...
mod tests {
    ...
    fn manifest_is_a1_and_pinned() {
        let manifest = local_evidence_surveillance_interoperability_gateway_manifest();
        assert_eq!(manifest.autonomy_tier, AutonomyTier::A1);
        assert_eq!(TARGET_PROTOCOL_VERSION, "1.0.0");
        assert_eq!(INPUT_SCHEMA, "EvidenceFeed1@1");
    }
}
