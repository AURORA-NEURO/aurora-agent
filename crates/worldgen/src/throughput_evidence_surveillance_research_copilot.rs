...
//!
//! Atlas feature `AFA-worldgen-P01-F11`: a bounded A2 agent surface for
...
pub const FEATURE_ID: &str = "AFA-worldgen-P01-F11";
pub const CONTRACT_VERSION: &str = "worldgen-throughput-evidence-surveillance-research-copilot/1.0";
...
pub fn throughput_evidence_surveillance_research_copilot_manifest() -> CapabilityManifest {
    CapabilityManifest {
        ...
        consumers: [
            ...
            "world-generation engineer".into(),
            "queue schema steward".into(),
        ]
        .into(),
        behavior: "runs bounded EvidenceFeed3 batches over structural benchmark worlds with checkpointed queue identity, explicit overflow, omission, negative evidence, and signed tool effects".into(),
        value: "turns high-throughput benchmark evidence surveillance into resumable, policy-auditable research work without hiding capacity loss or protected-closure gaps".into(),
...
> {
    ...
    let qualified_set = ThroughputCopilotQualifiedEvidenceSet {
        schema_version: RESEARCH_CONTRACT_SCHEMA_VERSION.into(),
        set_id: format!(
            "worldgen-qualified-evidence-throughput-copilot:{}",
            request.request_id
        ),
