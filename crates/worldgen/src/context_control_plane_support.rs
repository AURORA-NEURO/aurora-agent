...
pub const CONTENT_TYPE: &str = "application/vnd.aurora.worldgen.context-control-plane-receipt+json";
...
impl ContextControlPlaneReceipt {
    pub fn validate(&self) -> Result<(), ContextControlPlaneError> {
        if self.schema_version != RESEARCH_CONTRACT_SCHEMA_VERSION
            ...
            || ![&self.replay_identity, &self.control_digest]
                .into_iter()
                .all(digest)
        {
        ...
        for values in [
            ...
            if !ordered(values) {
                return Err(ContextControlPlaneError::Invalid(
        ...
        let action_parts = self
            .admitted_action_order
            .iter()
            .chain(&self.denied_action_order)
            .cloned()
            .collect::<Vec<_>>();
        if actions.len() != self.action_order.len()
            || action_parts.len() != actions.len()
            || action_parts.iter().cloned().collect::<BTreeSet<_>>() != actions
        {
            return Err(ContextControlPlaneError::Invalid(
        ...
        if self.effect_receipts.iter().any(|effect| {
            effect != "block:unsafe-release" && !effect.starts_with("control:worldgen-context:")
        }) {
            return Err(ContextControlPlaneError::Invalid(
                "control-plane effect is outside governance gate".into(),
            ));
        }
        if self
            .artifact
            .get("content_hash")
            .and_then(|value| value.as_str())
            != Some(self.control_digest.as_str())
        {
            return Err(ContextControlPlaneError::Invalid(
                "control artifact digest is inconsistent".into(),
            ));
        }
        Ok(())
    }
...
pub fn control(
    ...
    if request.program_id.trim().is_empty()
        ...
        || request.requested_action_order.is_empty()
        || request.action_budget == 0
        || request.minimum_site_quorum == 0
        || request.boundary != PRECLINICAL_BOUNDARY
        || sorted(&request.requested_action_order) != request.requested_action_order
    ...
    if candidates.iter().any(|attestation| {
        attestation.attestation_id.trim().is_empty()
            || attestation.site_id.trim().is_empty()
            || !digest(&attestation.context_digest)
            || !digest(&attestation.provenance_digest)
            || !digest(&attestation.replay_identity)
            || !attestation.raw_data_local
            || !attestation.aggregate_only
            || attestation.boundary != PRECLINICAL_BOUNDARY
            || attestation.replay_identity != request.replay_identity
    }) {
        return Err(ContextControlPlaneError::Invalid(
            "attestation identity, replay, provenance, locality, or boundary is invalid".into(),
        ));
    }
    ...
    for attestation in &candidates {
        if !attestation.permitted {
            blocked.insert(attestation.attestation_id.clone());
        } else if attestation.evidence_state != "supported"
            || attestation.support_milli < request.minimum_support_milli
            || attestation.freshness_milli < request.minimum_freshness_milli
        {
            unresolved.insert(attestation.attestation_id.clone());
        } else {
            admitted.insert(attestation.attestation_id.clone());
        }
    }
    let admitted_sites = admitted
        .iter()
        .filter_map(|id| {
            candidates
                .iter()
                .find(|attestation| &attestation.attestation_id == id)
                .map(|attestation| attestation.site_id.clone())
        })
        .collect::<BTreeSet<_>>();
    // `site_order` describes the observed attestation universe, not only admitted sites. An
    // unsupported or stale attestation must still name its origin in a partial result.
    let sites = candidates
        .iter()
        .map(|attestation| attestation.site_id.clone())
        .collect::<BTreeSet<_>>();
    let approvals_ok = (!require_approval || request.signed_approval)
        && (!require_federation || request.federation_approved);
    let quorum_ok = admitted_sites.len() >= request.minimum_site_quorum as usize;
    let actions_ok = request.requested_action_order.len() as u64 <= request.action_budget;
    let safe = approvals_ok
        && quorum_ok
        && actions_ok
        && !admitted.is_empty()
        && unresolved.is_empty()
        && blocked.is_empty();
    // A missing quorum caused only by unresolved attestations is partial, not a hard block: a
    // later refresh may satisfy it.  Quorum failure with no unresolved path remains blocked.
    let disposition = if !approvals_ok || (!quorum_ok && unresolved.is_empty()) {
        "blocked"
    } else if safe {
        "qualified"
    } else {
    ...
    if !approvals_ok {
        omissions.push("control:approval-missing".into());
    }
    if !quorum_ok {
        omissions.push("control:site-quorum-missing".into());
    }
    if !actions_ok {
        omissions.push("control:action-budget-exceeded".into());
    }
    if !unresolved.is_empty() {
        omissions.push("control:unsupported-or-stale-attestation".into());
    }
    if !blocked.is_empty() {
        omissions.push("control:permitted-attestation-missing".into());
    }
    omissions.sort();
    let mut denied_actions = if safe {
        Vec::new()
    } else {
        request.requested_action_order.clone()
    };
    denied_actions.sort();
    let admitted_actions = if safe {
        request.requested_action_order.clone()
    } else {
        Vec::new()
    };
    ...
    let effect_receipts = if disposition == "qualified" {
        vec![format!("control:worldgen-context:{}", request.program_id)]
    } else {
    ...
    let control_digest = ContentHash::of_value(&payload)
        .map_err(|error| ContextControlPlaneError::Artifact(error.to_string()))?;
    let admitted_order = sorted(&admitted.into_iter().collect::<Vec<_>>());
    let unresolved_order = sorted(&unresolved.into_iter().collect::<Vec<_>>());
    let blocked_order = sorted(&blocked.into_iter().collect::<Vec<_>>());
    let receipt = ContextControlPlaneReceipt {
        ...
        unresolved_order,
        blocked_order,
        site_order: sites_sorted,
        action_order: request.requested_action_order.clone(),
        admitted_action_order: admitted_actions,
        denied_action_order: denied_actions,
        ...
        uncertainty: if safe {
            ...
            vec!["control:qualification-requires-complete-attestations".into()]
        },
        negative_evidence: if !actions_ok {
            vec!["control:action-budget-negative".into()]
        } else {
            Vec::new()
        },
        effect_receipts: sorted(&effect_receipts),
        artifact: json!({"artifact_id":format!("worldgen-context-control:{}",request.program_id),"content_type":CONTENT_TYPE,"content_hash":control_digest,"raw_attestations":false,"aggregate_only":true,"boundary":PRECLINICAL_BOUNDARY}),
        raw_data_local: true,
        aggregate_only: true,
...
mod tests {
    ...
    fn request() -> ContextControlPlaneRequest {
        let replay = hash("replay");
        let attestation = ContextControlAttestation {
        ...
        ContextControlPlaneRequest {
            program_id: "program:context".into(),
            attestations: vec![attestation],
            minimum_support_milli: 500,
            minimum_freshness_milli: 500,
            minimum_site_quorum: 1,
            requested_action_order: vec!["retain:digest".into()],
            action_budget: 2,
    ...
    #[test]
    fn qualified_digest_control_is_deterministic() {
        let r = control(
            &request(),
            "AFA-worldgen-P03-F29",
            "worldgen-local-context-control-plane/1.0",
            "local single-study",
            false,
            false,
        )
        ...
        assert!(r.effect_receipts[0].starts_with("control:worldgen-context:"));
        assert!(r.validate().is_ok());
    }
    #[test]
    fn unsupported_attestation_is_retained() {
        let mut q = request();
        q.attestations[0].evidence_state = "unknown".into();
        let r = control(
            &q,
            "AFA-worldgen-P03-F30",
            "worldgen-multimodal-context-control-plane/1.0",
        ...
        assert_eq!(r.unresolved_order, vec!["attestation:a"]);
    }
    #[test]
    fn federation_gate_blocks() {
        let mut q = request();
        q.federation_approved = false;
        let r = control(
            &q,
            "AFA-worldgen-P03-F32",
            "worldgen-federated-continual-context-control-plane/1.0",
