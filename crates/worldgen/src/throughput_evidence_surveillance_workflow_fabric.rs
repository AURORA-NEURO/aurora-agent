...
//!
//! Atlas feature `AFA-worldgen-P01-F15`.  This is a product-level workflow
...
pub const FEATURE_ID: &str = "AFA-worldgen-P01-F15";
pub const CONTRACT_VERSION: &str = "worldgen-throughput-evidence-surveillance-workflow-fabric/1.0";
...
pub fn throughput_evidence_surveillance_workflow_fabric_manifest() -> CapabilityManifest {
    CapabilityManifest {
        ...
        consumers: ["bioinformatician".into(), "queue operator".into()].into(),
...
> {
    ...
    let blocked_order = if disposition == ThroughputResearchCopilotDisposition::Blocked {
    ...
    let artifact = TypedResearchArtifact::from_payload(
        format!(
            "worldgen-throughput-evidence-workflow:{}",
            request.workflow_id
        ),
        "application/vnd.aurora.worldgen.throughput-research-workflow+json",
        &payload,
        vec![],
        vec![],
    )
