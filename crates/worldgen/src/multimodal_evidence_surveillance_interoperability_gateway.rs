//! Version-negotiated multimodal evidence-surveillance interoperability gateway.
//! Atlas feature `AFA-worldgen-P01-F22`.
//!
...
use crate::multimodal_evidence_surveillance_research_workbench::{
    render_multimodal_evidence_surveillance_research_workbench,
    MultimodalEvidenceSurveillanceResearchWorkbenchRequest,
};

pub const FEATURE_ID: &str = "AFA-worldgen-P01-F22";
pub const CONTRACT_VERSION: &str =
    "worldgen-multimodal-evidence-surveillance-interoperability-gateway/1.0";
...
pub struct MultimodalEvidenceSurveillanceInteroperabilityGatewayRequest {
    pub interop_request: InteroperabilityRequest,
    pub workbench_request: MultimodalEvidenceSurveillanceResearchWorkbenchRequest,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct MultimodalEvidenceSurveillanceInteroperabilityGatewayReceipt {
...
pub enum MultimodalEvidenceSurveillanceInteroperabilityGatewayError {
...
impl MultimodalEvidenceSurveillanceInteroperabilityGatewayReceipt {
    pub fn validate(&self) -> Result<(), MultimodalEvidenceSurveillanceInteroperabilityGatewayError> {
        ...
        self.artifact.validate_metadata().map_err(|error| {
            MultimodalEvidenceSurveillanceInteroperabilityGatewayError::Artifact(error.to_string())
        })
    }

    fn invalid(message: &str) -> MultimodalEvidenceSurveillanceInteroperabilityGatewayError {
        MultimodalEvidenceSurveillanceInteroperabilityGatewayError::Invalid(message.into())
    }
}

pub fn multimodal_evidence_surveillance_interoperability_gateway_manifest() -> CapabilityManifest {
    CapabilityManifest {
        ...
        behavior: "negotiates a version-pinned multimodal evidence-surveillance protocol and projects a typed workbench artifact".into(),
        value: "makes API/protocol compatibility, semantic loss, omissions, and local-only effects machine-checkable".into(),
        inputs: vec![
            TypedPort { name: "multimodal_evidence_feed".into(), schema: INPUT_SCHEMA.into(), required: true },
...
pub fn render_multimodal_evidence_surveillance_interoperability_gateway(
    request: &MultimodalEvidenceSurveillanceInteroperabilityGatewayRequest,
) -> Result<
...
> {
    ...
    let integration = negotiate_interoperability(&request.interop_request).map_err(|error| {
        MultimodalEvidenceSurveillanceInteroperabilityGatewayError::Invalid(error.to_string())
    })?;
    let workbench = render_multimodal_evidence_surveillance_research_workbench(
        &request.workbench_request,
    )
    .map_err(|error| {
        MultimodalEvidenceSurveillanceInteroperabilityGatewayError::Workbench(error.to_string())
    })?;
    ...
    if loss.len() as u32 > request.semantic_loss_budget {
        return Err(
            MultimodalEvidenceSurveillanceInteroperabilityGatewayError::Invalid(
    ...
    let artifact = TypedResearchArtifact::from_payload(
        ...
        "application/vnd.aurora.worldgen-multimodal-evidence-surveillance-interoperability+json",
        &protocol_payload,
        Vec::new(),
        Vec::new(),
    )
    ...
    let receipt = MultimodalEvidenceSurveillanceInteroperabilityGatewayReceipt {
        schema_version: RESEARCH_CONTRACT_SCHEMA_VERSION.into(),
...
fn validate_request(
    request: &MultimodalEvidenceSurveillanceInteroperabilityGatewayRequest,
) -> Result<(), MultimodalEvidenceSurveillanceInteroperabilityGatewayError> {
    ...
    {
        return Err(MultimodalEvidenceSurveillanceInteroperabilityGatewayError::Invalid("protocol schemas, migration policy, loss budget, locality, and boundary are invalid".into()));
    }
    Ok(())
}
...
mod tests {
    ...
    fn manifest_is_a1_and_pinned() {
        let manifest = multimodal_evidence_surveillance_interoperability_gateway_manifest();
        assert_eq!(manifest.autonomy_tier, AutonomyTier::A2);
        assert_eq!(TARGET_PROTOCOL_VERSION, "1.0.0");
        assert_eq!(INPUT_SCHEMA, "EvidenceFeed2@1");
    }
}
