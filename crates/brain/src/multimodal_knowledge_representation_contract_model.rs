...
impl MultimodalKnowledgeContractReceipt {
    pub fn validate(&self) -> Result<(), MultimodalKnowledgeContractError> {
        ...
        for values in [
            ...
            if values.windows(2).any(|pair| pair[0] >= pair[1]) {
                return Err(MultimodalKnowledgeContractError::Invalid(
        ...
        if classified.len() != self.candidate_order.len()
            || classified
                .iter()
                .any(|cell| !self.candidate_order.contains(cell))
            || self
                .missing_order
                .iter()
                .any(|cell| !self.candidate_order.contains(cell))
        ...
        {
            return Err(MultimodalKnowledgeContractError::Invalid(
                "multimodal contract states do not partition cells".into(),
            ));
        }
        for digest in [
        ...
        let expected_effect = if self.disposition == MultimodalKnowledgeContractDisposition::Blocked
        {
            "block:unsafe-release".into()
        } else {
            format!(
                "read:local-multimodal-knowledge-contract:{}",
                self.request_id
            )
        };
        if self.effect_receipts != [expected_effect] {
            return Err(MultimodalKnowledgeContractError::Invalid(
                "multimodal contract effect does not match disposition".into(),
            ));
        }
        ...
        .map_err(|error| MultimodalKnowledgeContractError::Artifact(error.to_string()))?;
        if self.contract_digest != expected_contract_digest {
            return Err(MultimodalKnowledgeContractError::Invalid(
                "multimodal contract digest is not bound to cell state".into(),
            ));
        }
        let expected_artifact_id =
            format!("brain-multimodal-knowledge-contract:{}", self.request_id);
    ...
    pub fn digest(&self) -> Result<ContentHash, MultimodalKnowledgeContractError> {
        self.validate()?;
...
fn receipt_payload(receipt: &MultimodalKnowledgeContractReceipt) -> serde_json::Value {
    json!({
...
pub fn model_multimodal_knowledge_representation_contract(
    request: &MultimodalKnowledgeContractRequest,
) -> Result<MultimodalKnowledgeContractReceipt, MultimodalKnowledgeContractError> {
    ...
    {
        return Err(MultimodalKnowledgeContractError::Invalid("multimodal contract identity, schemas, coverage, revisions, migration, replay, locality, or boundary is invalid".into()));
    }
    let mut studies = request.study_order.clone();
    studies.sort();
    let mut modalities = request.modality_order.clone();
    modalities.sort();
    if studies.windows(2).any(|p| p[0] == p[1]) || modalities.windows(2).any(|p| p[0] == p[1]) {
        return Err(MultimodalKnowledgeContractError::Invalid(
            "study and modality identifiers must be unique".into(),
        ));
    }
    ...
    let candidate = cells
        ...
        .collect::<Vec<_>>();
    if candidate.windows(2).any(|p| p[0] == p[1])
        || candidate.iter().any(|value| value.trim().is_empty())
    {
        return Err(MultimodalKnowledgeContractError::Invalid(
            "multimodal cell identifiers must be unique and non-empty".into(),
        ));
    }
    let profiles = cells
        .iter()
        .map(|cell| cell.semantic_profile.clone())
        .collect::<BTreeSet<_>>();
    let profile_conflict = request.comparability_required && profiles.len() > 1;
    ...
    let mut uncertainty = BTreeSet::from([
        "gate:study-modality-coverage".to_string(),
    ...
    for cell in &cells {
        if !request.study_order.contains(&cell.study_id)
            || !request.modality_order.contains(&cell.modality)
            || !request.policy_allow
            || !request.protected_closure
            || cell.boundary != PRECLINICAL_BOUNDARY
        {
            denied.insert(cell.cell_id.clone());
            negative.insert(format!("cell:{}:scope-policy-closure", cell.cell_id));
        } else if cell.evidence_digest.is_none() || cell.provenance_digest.is_none() {
            unresolved.insert(cell.cell_id.clone());
            missing.insert(cell.cell_id.clone());
            omissions.insert(format!(
                "cell:{}:evidence-or-provenance-missing",
                cell.cell_id
            ));
        } else if matches!(
            cell.state,
            ...
            unresolved.insert(cell.cell_id.clone());
            uncertainty.insert(format!("cell:{}:unknown-not-asserted", cell.cell_id));
        } else if matches!(cell.state, EvidenceState::Contradicted) {
            denied.insert(cell.cell_id.clone());
            negative.insert(format!("cell:{}:contradicted", cell.cell_id));
        } else if profile_conflict {
            unresolved.insert(cell.cell_id.clone());
            incomparable.insert(cell.cell_id.clone());
            uncertainty.insert(format!("cell:{}:semantic-profile-conflict", cell.cell_id));
        } else {
            admitted.insert(cell.cell_id.clone());
            if request.target_revision > request.source_revision {
                semantic_loss.insert(cell.cell_id.clone());
            }
        }
    }
    for required in request.required_cell_ids.iter().collect::<BTreeSet<_>>() {
        if !candidate.contains(required) {
            omissions.insert(format!("cell:{}:required-missing", required));
            uncertainty.insert(format!("cell:{}:required-unresolved", required));
        } else if !admitted.contains(required) {
            uncertainty.insert(format!("cell:{}:required-not-admitted", required));
        }
    }
    if request.target_revision > request.source_revision {
    ...
    if profile_conflict {
        omissions.insert("control:semantic-profile-incompatibility".into());
    }
    ...
    let comparability_digest = ContentHash::of_value(&json!({"study_order": studies, "modality_order": modalities, "semantic_profiles": profiles, "required": request.comparability_required})).map_err(|error| MultimodalKnowledgeContractError::Artifact(error.to_string()))?;
    let contract_digest = ContentHash::of_value(&json!({"workspace_id": request.workspace_id, "candidate_order": candidate, "admitted_order": admitted, "unresolved_order": unresolved, "denied_order": denied, "incomparable_order": incomparable, "source_revision": request.source_revision, "target_revision": request.target_revision, "comparability_digest": comparability_digest})).map_err(|error| MultimodalKnowledgeContractError::Artifact(error.to_string()))?;
    let migration_digest = ContentHash::of_value(&json!({"source_revision": request.source_revision, "target_revision": request.target_revision, "migration_requested": request.migration_requested, "semantic_loss_order": semantic_loss})).map_err(|error| MultimodalKnowledgeContractError::Artifact(error.to_string()))?;
    let disposition =
        if !request.policy_allow || !request.protected_closure || !request.raw_data_local {
            MultimodalKnowledgeContractDisposition::Blocked
        } else if admitted.is_empty()
            || !unresolved.is_empty()
            || !denied.is_empty()
            || profile_conflict
        {
            MultimodalKnowledgeContractDisposition::Partial
        } else if request.target_revision > request.source_revision {
            MultimodalKnowledgeContractDisposition::Migrated
        } else {
            MultimodalKnowledgeContractDisposition::Compatible
        };
    let payload = json!({"schema_version": RESEARCH_CONTRACT_SCHEMA_VERSION, "contract_version": CONTRACT_VERSION, "feature_id": FEATURE_ID, "request_id": request.request_id, "workspace_id": request.workspace_id, "disposition": disposition, "input_schema": INPUT_SCHEMA, "output_schema": OUTPUT_SCHEMA, "source_revision": request.source_revision, "target_revision": request.target_revision, "comparability_digest": comparability_digest, "contract_digest": contract_digest, "migration_digest": migration_digest, "replay_identity": request.replay_identity, "boundary": PRECLINICAL_BOUNDARY});
    let artifact = TypedResearchArtifact::from_payload(
        format!("brain-multimodal-knowledge-contract:{}", request.request_id),
    ...
    .map_err(|error| MultimodalKnowledgeContractError::Artifact(error.to_string()))?;
    let receipt = MultimodalKnowledgeContractReceipt {
        ...
        effect_receipts: if disposition == MultimodalKnowledgeContractDisposition::Blocked {
...
mod tests {
    ...
    fn request() -> MultimodalKnowledgeContractRequest {
        let h = hash("multimodal-contract");
        let cell = |id: &str, study: &str, modality: &str, state: EvidenceState| {
            MultimodalKnowledgeContractCell {
                cell_id: id.into(),
                study_id: study.into(),
        ...
        MultimodalKnowledgeContractRequest {
            ...
            cells: vec![
                cell("cell:a", "study:a", "imaging", EvidenceState::Supported),
                cell("cell:b", "study:b", "omics", EvidenceState::Supported),
            ],
            required_cell_ids: vec!["cell:a".into()],
            input_schema: INPUT_SCHEMA.into(),
            output_schema: OUTPUT_SCHEMA.into(),
            source_revision: 1,
            target_revision: 1,
    ...
    fn manifest_is_a1() {
        assert_eq!(
            multimodal_knowledge_representation_contract_model_manifest().autonomy_tier,
            AutonomyTier::A1
        );
    }
    #[test]
    fn comparable_contract_is_compatible() {
        assert_eq!(
            model_multimodal_knowledge_representation_contract(&request())
                .unwrap()
                .disposition,
            MultimodalKnowledgeContractDisposition::Compatible
        );
    ...
    fn migration_is_explicit() {
        ...
        assert_eq!(
            model_multimodal_knowledge_representation_contract(&v)
                .unwrap()
                .disposition,
            MultimodalKnowledgeContractDisposition::Migrated
        );
    ...
    #[test]
    fn semantic_profile_conflict_is_partial() {
        let mut v = request();
        v.cells[1].semantic_profile = "profile:v2".into();
        let r = model_multimodal_knowledge_representation_contract(&v).unwrap();
        assert_eq!(
            r.disposition,
            MultimodalKnowledgeContractDisposition::Partial
        );
        assert!(!r.incomparable_order.is_empty());
    }
    #[test]
    fn missing_modality_evidence_is_partial() {
        let mut v = request();
        v.cells[0].evidence_digest = None;
        let r = model_multimodal_knowledge_representation_contract(&v).unwrap();
        assert_eq!(
            r.disposition,
            MultimodalKnowledgeContractDisposition::Partial
        );
        assert_eq!(r.missing_order, vec!["cell:a".to_string()]);
    }
    #[test]
    fn unknown_is_unresolved() {
        let mut v = request();
        v.cells[0].state = EvidenceState::Unknown;
        let r = model_multimodal_knowledge_representation_contract(&v).unwrap();
        assert_eq!(
            r.disposition,
            MultimodalKnowledgeContractDisposition::Partial
    ...
    #[test]
    fn digest_is_stable() {
        let r = model_multimodal_knowledge_representation_contract(&request()).unwrap();
