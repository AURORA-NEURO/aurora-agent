...
impl RetrievalWorkflowReceipt {
    pub fn validate(&self) -> Result<(), RetrievalWorkflowError> {
        ...
        {
            return Err(RetrievalWorkflowError::Invalid(
                "retrieval workflow state is not covered by candidates".into(),
            ));
        }
        for values in [
            &self.stage_order,
            ...
            if values.windows(2).any(|pair| pair[0] >= pair[1]) {
                return Err(RetrievalWorkflowError::Invalid(
                    "retrieval workflow ordering is not canonical".into(),
                ));
            }
        }
        for digest in [
        ...
        if self.effect_receipts.iter().any(|effect| {
            !effect.starts_with("schedule:retrieval-work:")
                && !effect.starts_with("compensate:retrieval-work:")
                && effect != "block:unsafe-release"
        }) {
            return Err(RetrievalWorkflowError::Invalid(
                "retrieval workflow effect is outside schedule/compensation gate".into(),
            ));
        }
        if self.disposition == SynthesisDisposition::Qualified
            && !self
                .effect_receipts
                .iter()
                .any(|effect| effect.starts_with("schedule:retrieval-work:"))
        {
            return Err(RetrievalWorkflowError::Invalid(
                "qualified retrieval workflow requires schedule receipt".into(),
            ));
        }
        self.artifact
            .validate_metadata()
...
pub fn compile_retrieval_workflow(
    ...
    let checkpoint_digest = ContentHash::of_value(&json!({"workflow_id": request.workflow_id, "checkpoint_id": request.checkpoint_id, "stage_order": stage_order, "replay_identity": request.replay_identity}))
        .map_err(|error| RetrievalWorkflowError::Artifact(error.to_string()))?;
    let workflow_digest = ContentHash::of_value(&json!({"workflow_id": request.workflow_id, "plan_order": plan_order, "completed_order": completed_order, "checkpoint_digest": checkpoint_digest, "synthesis_digest": synthesis_digest, "budget_units": request.budget_units, "replay_identity": request.replay_identity}))
        .map_err(|error| RetrievalWorkflowError::Artifact(error.to_string()))?;
    let payload = json!({"schema_version": RESEARCH_CONTRACT_SCHEMA_VERSION, "contract_version": CONTRACT_VERSION, "feature_id": FEATURE_ID, "request_id": request.request.request_id, "workflow_id": request.workflow_id, "study_id": request.request.study_id, "scope": request.request.scope, "disposition": disposition, "stage_order": stage_order, "plan_order": plan_order, "completed_order": completed_order, "blocked_order": blocked_order, "compensation_order": compensation_order, "candidate_order": synthesis.candidate_order, "ranked_order": synthesis.ranked_order, "qualified_order": synthesis.qualified_order, "unknown_order": synthesis.unknown_order, "synthesis_digest": synthesis_digest, "checkpoint_digest": checkpoint_digest, "workflow_digest": workflow_digest, "replay_identity": request.replay_identity, "budget_units": request.budget_units, "omissions": omissions, "uncertainty": uncertainty, "negative_evidence": negative, "boundary": PRECLINICAL_BOUNDARY});
    let artifact = TypedResearchArtifact::from_payload(
        format!("brain-retrieval-workflow:{}", request.workflow_id),
        "application/vnd.aurora.retrieval-workflow-receipt+json",
        &payload,
        Vec::new(),
        Vec::new(),
    )
