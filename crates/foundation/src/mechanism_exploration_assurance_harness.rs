...
//! Atlas feature `AFA-foundation-P08-F27`.
...
pub const FEATURE_ID: &str = "AFA-foundation-P08-F27";
pub const CONTRACT_VERSION: &str = "foundation-prospective-mechanism-exploration-assurance/1.0";
...
impl MechanismExplorationAssuranceReceipt {
    pub fn validate(&self) -> Result<(), MechanismExplorationAssuranceError> {
        ...
        if self.effect_receipts.iter().any(|effect| {
            effect != "assure:foundation-mechanism-exploration" && effect != "block:unsafe-release"
        }) {
            return Err(Self::invalid(
...
pub fn assure_mechanism_exploration(
    ...
    let effect = if verdict == "qualified" {
        "assure:foundation-mechanism-exploration"
    } else {
    ...
    let artifact = TypedResearchArtifact::from_payload(
        format!("foundation-mechanism-assurance:{}", request.batch_id),
        "application/vnd.aurora.foundation-mechanism-exploration-assurance+json",
        &payload,
        Vec::new(),
        Vec::new(),
    )
