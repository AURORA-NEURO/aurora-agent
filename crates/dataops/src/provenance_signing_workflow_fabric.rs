//! Prospective high-throughput provenance and signing workflow fabric assurance (`AFA-dataops-P18-F15`).
//!
...
pub const FEATURE_ID: &str = "AFA-dataops-P18-F15";
pub const CONTRACT_VERSION: &str = "dataops-prospective-high-throughput-provenance-signing-workflow-fabric/1.0";
pub const INPUT_SCHEMA: &str = "ArtifactAndDerivation3@1";
pub const OUTPUT_SCHEMA: &str = "SignedProvenanceEnvelope7@1";
pub const CONTENT_TYPE: &str = "application/vnd.aurora.dataops-provenance-signing-workflow+json";
...
pub fn prospective_provenance_assurance_manifest() -> serde_json::Value { json!({
    ...
    "owner_crate":"dataops", "consumers":["research-object steward","provenance auditor","release operator"],
...
fn validate_request(request: &ArtifactAndDerivationRequest3) -> Result<(), ProspectiveProvenanceError> {
    if request.schema_version != INPUT_SCHEMA || !nonempty(&request.request_id) || !nonempty(&request.semantic_profile) || request.artifacts.is_empty() || !nonempty(&request.workflow_id) || !nonempty(&request.batch_id) || request.capacity == 0 || request.active_jobs > request.capacity || !digest(&request.expected_root) || !digest(&request.replay_identity) || !ordered(&request.adversarial_events) || request.boundary != PRECLINICAL_BOUNDARY || !request.raw_data_local || !request.aggregate_only { return Err(ProspectiveProvenanceError::Invalid("request identity, lineage, digest, locality, or boundary is invalid".into())); }
...
pub fn assure_prospective_provenance(request: &ArtifactAndDerivationRequest3) -> Result<SignedProvenanceEnvelope7, ProspectiveProvenanceError> {
    ...
    for id in &missing { if !blocked.contains(id) { unresolved.insert(id.clone()); uncertainty.insert(format!("{id}:missing-parent")); } }
