...
pub const FEATURE_ID: &str = "AFA-influence-P08-F32";
pub const FEATURE_VERSION: &str = "influence-federated-continual-mechanism-control-plane/1.0";
...
impl FederatedMechanismReceipt {
    pub fn validate(&self) -> Result<(), FederatedMechanismError> {
        ...
        if self.effect_receipts.iter().any(|effect| {
            !effect.starts_with("operate:mechanism:")
                && !effect.starts_with("approval-required:mechanism")
...
pub fn capability_manifest() -> CapabilityManifest {
    CapabilityManifest {
        ...
        behavior: "ranks and governs digest-only federated mechanism attestations under continual A2 gates".into(),
        value: "unlocks consortium mechanism exploration without treating unknown influence or failed safety gates as qualified".into(),
