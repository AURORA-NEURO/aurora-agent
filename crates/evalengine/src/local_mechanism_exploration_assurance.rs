//! Local single-study mechanism-exploration assurance harness.
//!
//! Atlas feature: `AFA-evalengine-P08-F25`.
//!
...
pub const FEATURE_ID: &str = "AFA-evalengine-P08-F25";
pub const CONTRACT_VERSION: &str = "evalengine-local-single-study-mechanism-exploration-assurance/1.0";

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum EvalengineCandidateState {
    Supported,
    Unknown,
    Contradicted,
    Unmeasured,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum EvalengineAssuranceDisposition {
    Qualified,
    Conditional,
    Unknown,
    Blocked,
...
pub struct EvalengineMechanismCandidate {
    ...
    pub state: EvalengineCandidateState,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct EvalengineMechanismQuestion1 {
    pub question_id: String,
    pub workflow_id: String,
    pub scope: String,
    pub target_schema: String,
    pub candidates: Vec<EvalengineMechanismCandidate>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct EvalengineMechanismPortfolio7 {
    ...
    pub workflow_id: String,
    pub target_schema: String,
    pub disposition: EvalengineAssuranceDisposition,
...
pub enum EvalengineMechanismExplorationAssuranceError {
...
impl EvalengineMechanismPortfolio7 {
    pub fn validate(&self) -> Result<(), EvalengineMechanismExplorationAssuranceError> {
        if self.schema_version != RESEARCH_CONTRACT_SCHEMA_VERSION
            ...
            || (self.effect_receipts.is_empty()
                && self.disposition != EvalengineAssuranceDisposition::Qualified)
        {
            return Err(EvalengineMechanismExplorationAssuranceError::Contract(
        ...
        self.artifact
            .validate_metadata()
            .map_err(|error| EvalengineMechanismExplorationAssuranceError::Contract(error.to_string()))?;
        Ok(())
    }
}

pub fn evalengine_local_mechanism_exploration_assurance_manifest() -> CapabilityManifest {
    CapabilityManifest {
        ...
        behavior: "verifies and ranks caller-supplied local single-study mechanism candidates without inventing a mechanism or hiding unresolved evidence".into(),
        ...
        inputs: vec![TypedPort {
            ...
            schema: "MechanismQuestion1@1".into(),
            required: true,
        }],
...
pub fn assure_evalengine_local_mechanism_exploration(question: &EvalengineMechanismQuestion1) -> Result<EvalengineMechanismPortfolio7, EvalengineMechanismExplorationAssuranceError> {
    ...
    for candidate in &candidates {
        ...
        match candidate.state {
            EvalengineCandidateState::Contradicted => {
                blocked.insert(candidate.candidate_id.clone());
            ...
            EvalengineCandidateState::Unknown | EvalengineCandidateState::Unmeasured => {
            ...
            EvalengineCandidateState::Supported => {}
        }
    ...
    let disposition = if !question.policy_allow || !question.raw_data_local {
        EvalengineAssuranceDisposition::Blocked
    } else if admitted_order.is_empty() {
        EvalengineAssuranceDisposition::Unknown
    ...
    {
        EvalengineAssuranceDisposition::Conditional
    } else {
        EvalengineAssuranceDisposition::Qualified
    };
    let effect_receipts = if disposition == EvalengineAssuranceDisposition::Qualified {
        Vec::new()
    } else {
    ...
    .map_err(|error| EvalengineMechanismExplorationAssuranceError::Contract(error.to_string()))?;
    let portfolio = EvalengineMechanismPortfolio7 {
        schema_version: RESEARCH_CONTRACT_SCHEMA_VERSION.into(),
...
fn validate_question(question: &EvalengineMechanismQuestion1) -> Result<(), EvalengineMechanismExplorationAssuranceError> {
    ...
    {
        return Err(EvalengineMechanismExplorationAssuranceError::Invalid(
    ...
    for candidate in &question.candidates {
        if candidate.candidate_id.trim().is_empty()
            ...
            || candidate
                ...
                .any(|pair| pair[0] >= pair[1])
        {
            return Err(EvalengineMechanismExplorationAssuranceError::Invalid(format!(
...
mod tests {
    ...
    fn candidate(id: &str, state: EvalengineCandidateState, negative_result: bool) -> EvalengineMechanismCandidate {
        EvalengineMechanismCandidate {
            candidate_id: id.into(),
            mechanism_id: format!("mechanism:{id}"),
            scope: "organoid:neural".into(),
            study_ids: vec!["study:imaging".into()],
    ...
    fn question(candidates: Vec<EvalengineMechanismCandidate>) -> EvalengineMechanismQuestion1 {
    ...
    #[test]
    fn manifest_is_typed_and_a0_deterministic() {
        let manifest = evalengine_local_mechanism_exploration_assurance_manifest();
    ...
    fn supported_candidates_are_ranked_and_admitted() {
        let portfolio = assure_evalengine_local_mechanism_exploration(&question(vec![
            candidate("candidate:a", EvalengineCandidateState::Supported, false),
            candidate("candidate:b", EvalengineCandidateState::Supported, true),
        ]))
        .unwrap();
        assert_eq!(portfolio.disposition, EvalengineAssuranceDisposition::Qualified);
    ...
    fn unknown_candidate_is_retained_and_release_blocked() {
        let portfolio = assure_evalengine_local_mechanism_exploration(&question(vec![
            candidate("candidate:a", EvalengineCandidateState::Supported, false),
            candidate("candidate:b", EvalengineCandidateState::Unknown, false),
        ]))
        .unwrap();
        assert_eq!(portfolio.disposition, EvalengineAssuranceDisposition::Conditional);
    ...
    fn contradiction_is_blocked_with_negative_evidence() {
        let portfolio = assure_evalengine_local_mechanism_exploration(&question(vec![
            candidate("candidate:a", EvalengineCandidateState::Supported, false),
            candidate("candidate:b", EvalengineCandidateState::Contradicted, false),
        ]))
        .unwrap();
    ...
    fn policy_denial_blocks_without_release() {
        let mut question = question(vec![
            candidate("candidate:a", EvalengineCandidateState::Supported, false),
            candidate("candidate:b", EvalengineCandidateState::Supported, false),
        ]);
        question.policy_allow = false;
        let portfolio = assure_evalengine_local_mechanism_exploration(&question).unwrap();
        assert_eq!(portfolio.disposition, EvalengineAssuranceDisposition::Blocked);
        assert_eq!(portfolio.effect_receipts, vec!["block:unsafe-release"]);
    }

    #[test]
    fn duplicate_candidates_are_rejected() {
        let result = assure_evalengine_local_mechanism_exploration(&question(vec![
            candidate("candidate:a", EvalengineCandidateState::Supported, false),
            candidate("candidate:a", EvalengineCandidateState::Supported, false),
        ]));
