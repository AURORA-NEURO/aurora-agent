...
//!
//! Atlas feature `AFA-adapter-P02-F05`: a deterministic A0 product surface
...
pub const FEATURE_ID: &str = "AFA-adapter-P02-F05";
pub const CONTRACT_VERSION: &str = "adapter-local-retrieval-synthesis-contract-model/1.0";
pub const INPUT_SCHEMA: &str = "ScopedRetrievalQuery@1";
pub const OUTPUT_SCHEMA: &str = "EvidenceSynthesis1@1";

const EFFECT_PREFIX: &str = "compute:local-retrieval-contract-model:";

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct LocalRetrievalSynthesisContractModelRequest {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct LocalRetrievalSynthesisContractModelReceipt {
...
pub enum LocalRetrievalSynthesisContractModelError {
    #[error("invalid local retrieval contract model request: {0}")]
    Invalid(String),
    #[error("local retrieval contract model artifact failed: {0}")]
    Artifact(String),
    #[error("local retrieval contract model synthesis failed: {0}")]
    Synthesis(String),
}
...
impl LocalRetrievalSynthesisContractModelReceipt {
    pub fn validate(&self) -> Result<(), LocalRetrievalSynthesisContractModelError> {
        ...
        {
            return Err(LocalRetrievalSynthesisContractModelError::Invalid(
                "local retrieval contract model identity, output, locality, candidates, or effects are incomplete".into(),
            ));
        }
        for values in [
            ...
            if !canonical(values) {
                return Err(LocalRetrievalSynthesisContractModelError::Invalid(
                    "local retrieval contract model ordering is not canonical".into(),
                ));
            }
        }
        let classified = self
        ...
        if classified != self.candidate_order.iter().cloned().collect() {
            return Err(LocalRetrievalSynthesisContractModelError::Invalid(
                "local retrieval contract model states do not partition candidates".into(),
            ));
        }
        for digest in [
            ...
            if digest.as_str().len() != 64 {
                return Err(LocalRetrievalSynthesisContractModelError::Invalid(
                    "local retrieval contract model digest is invalid".into(),
                ));
            }
        }
        if self
        ...
        {
            return Err(LocalRetrievalSynthesisContractModelError::Invalid(
                "local retrieval contract model effect is outside local computation gate".into(),
            ));
        }
        self.artifact
            .validate_metadata()
            .map_err(|error| LocalRetrievalSynthesisContractModelError::Artifact(error.to_string()))
    }
}

pub fn local_retrieval_synthesis_contract_model_manifest() -> CapabilityManifest {
    CapabilityManifest {
        ...
        behavior: "validates and materializes a deterministic institution-local retrieval corpus as a typed data primitive, retaining omissions, uncertainty, contradiction, and negative results".into(),
        value: "provides a separately versioned canonical contract model for auditable single-study evidence retrieval without silently substituting defaults".into(),
...
pub fn run_local_retrieval_synthesis_contract_model(
    request: &LocalRetrievalSynthesisContractModelRequest,
) -> Result<LocalRetrievalSynthesisContractModelReceipt, LocalRetrievalSynthesisContractModelError>
{
    validate_request(request)?;
    let synthesis = compile_evidence_synthesis(&request.synthesis_request)
        .map_err(|error| LocalRetrievalSynthesisContractModelError::Synthesis(error.to_string()))?;
    ...
    let synthesis_value = serde_json::to_value(&synthesis.synthesis)
        .map_err(|error| LocalRetrievalSynthesisContractModelError::Artifact(error.to_string()))?;
    let synthesis_digest = ContentHash::of_value(&synthesis_value)
        .map_err(|error| LocalRetrievalSynthesisContractModelError::Artifact(error.to_string()))?;
    ...
    let artifact = TypedResearchArtifact::from_payload(
        format!(
            "adapter-local-retrieval-contract-model:{}",
            request.contract_id
        ),
...
fn validate_request(
    request: &LocalRetrievalSynthesisContractModelRequest,
) -> Result<(), LocalRetrievalSynthesisContractModelError> {
...
mod tests {
    ...
    fn request() -> LocalRetrievalSynthesisContractModelRequest {
    ...
    fn manifest_is_a0() {
        assert_eq!(
            local_retrieval_synthesis_contract_model_manifest().autonomy_tier,
            AutonomyTier::A0
        );
    }
    ...
    fn computes_local_synthesis() {
        let receipt = run_local_retrieval_synthesis_contract_model(&request()).unwrap();
    ...
    fn policy_block_is_observable() {
        ...
        let receipt = run_local_retrieval_synthesis_contract_model(&value).unwrap();
    ...
    fn rejects_wrong_output() {
        ...
        assert!(run_local_retrieval_synthesis_contract_model(&value).is_err());
    }
    ...
    fn replay_is_stable() {
        let value = request();
        assert_eq!(
            run_local_retrieval_synthesis_contract_model(&value).unwrap(),
            run_local_retrieval_synthesis_contract_model(&value).unwrap()
        );
    }
}
