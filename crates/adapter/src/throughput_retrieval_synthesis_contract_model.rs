...
//!
//! Atlas feature `AFA-adapter-P02-F07`: a deterministic A0 product surface
...
pub const FEATURE_ID: &str = "AFA-adapter-P02-F07";
pub const CONTRACT_VERSION: &str = "adapter-throughput-retrieval-synthesis-contract-model/1.0";
pub const INPUT_SCHEMA: &str = "ScopedRetrievalQuery@1";
pub const OUTPUT_SCHEMA: &str = "EvidenceSynthesis1@1";

const EFFECT_PREFIX: &str = "compute:throughput-retrieval-contract-model:";

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ThroughputRetrievalSynthesisContractModelRequest {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ThroughputRetrievalSynthesisContractModelReceipt {
...
pub enum ThroughputRetrievalSynthesisContractModelError {
    #[error("invalid throughput retrieval contract model request: {0}")]
    Invalid(String),
    #[error("throughput retrieval contract model artifact failed: {0}")]
    Artifact(String),
    #[error("throughput retrieval contract model synthesis failed: {0}")]
    Synthesis(String),
}
...
impl ThroughputRetrievalSynthesisContractModelReceipt {
    pub fn validate(&self) -> Result<(), ThroughputRetrievalSynthesisContractModelError> {
        ...
        {
            return Err(ThroughputRetrievalSynthesisContractModelError::Invalid(
                "throughput retrieval contract model identity, output, locality, candidates, or effects are incomplete".into(),
            ));
        }
        for values in [
            ...
            if !canonical(values) {
                return Err(ThroughputRetrievalSynthesisContractModelError::Invalid(
                    "throughput retrieval contract model ordering is not canonical".into(),
                ));
            }
        }
        let classified = self
        ...
        if classified != self.candidate_order.iter().cloned().collect() {
            return Err(ThroughputRetrievalSynthesisContractModelError::Invalid(
                "throughput retrieval contract model states do not partition candidates".into(),
            ));
        }
        if self
            .overflow_order
        ...
        for digest in [
            ...
            if digest.as_str().len() != 64 {
                return Err(ThroughputRetrievalSynthesisContractModelError::Invalid(
                    "throughput retrieval contract model digest is invalid".into(),
                ));
            }
        }
        if self
        ...
        {
            return Err(ThroughputRetrievalSynthesisContractModelError::Invalid(
                "throughput retrieval contract model effect is outside local computation gate"
                    .into(),
            ));
        }
        self.artifact.validate_metadata().map_err(|error| {
            ThroughputRetrievalSynthesisContractModelError::Artifact(error.to_string())
        })
    }
}

pub fn throughput_retrieval_synthesis_contract_model_manifest() -> CapabilityManifest {
    CapabilityManifest {
        ...
        behavior: "validates and materializes a deterministic bounded high-throughput retrieval corpus as a typed data primitive with batch admission, capacity overflow, checkpoints, and retained omissions, uncertainty, contradiction, and negative results".into(),
        value: "provides a separately versioned canonical contract model for auditable prospective evidence retrieval without silently dropping queue overflow or substituting defaults".into(),
...
pub fn run_throughput_retrieval_synthesis_contract_model(
    request: &ThroughputRetrievalSynthesisContractModelRequest,
) -> Result<
...
> {
    ...
    let synthesis = compile_evidence_synthesis(&request.synthesis_request).map_err(|error| {
        ThroughputRetrievalSynthesisContractModelError::Synthesis(error.to_string())
    })?;
    ...
    .map_err(|error| ThroughputRetrievalSynthesisContractModelError::Artifact(error.to_string()))?;
    let payload = json!({
    ...
    let artifact = TypedResearchArtifact::from_payload(
        format!(
            "adapter-throughput-retrieval-contract-model:{}",
            request.contract_id
        ),
        "application/vnd.aurora.throughput-retrieval-synthesis-contract-model+json",
        &payload,
        Vec::new(),
        Vec::new(),
    )
...
fn validate_request(
    request: &ThroughputRetrievalSynthesisContractModelRequest,
) -> Result<(), ThroughputRetrievalSynthesisContractModelError> {
...
mod tests {
    ...
    fn request() -> ThroughputRetrievalSynthesisContractModelRequest {
    ...
    fn manifest_is_a1() {
        assert_eq!(
            throughput_retrieval_synthesis_contract_model_manifest().autonomy_tier,
            AutonomyTier::A1
        );
    }
    ...
    fn computes_local_synthesis() {
        let receipt = run_throughput_retrieval_synthesis_contract_model(&request()).unwrap();
    ...
    fn policy_block_is_observable() {
        ...
        let receipt = run_throughput_retrieval_synthesis_contract_model(&value).unwrap();
    ...
    fn rejects_wrong_output() {
        ...
        assert!(run_throughput_retrieval_synthesis_contract_model(&value).is_err());
    }
    ...
    fn replay_is_stable() {
        let value = request();
        assert_eq!(
            run_throughput_retrieval_synthesis_contract_model(&value).unwrap(),
            run_throughput_retrieval_synthesis_contract_model(&value).unwrap()
        );
    }
