...
//!
//! Atlas feature `AFA-adapter-P02-F03`: a deterministic A0 product surface
...
pub const FEATURE_ID: &str = "AFA-adapter-P02-F03";
pub const CONTRACT_VERSION: &str = "adapter-throughput-retrieval-synthesis-inference-engine/1.0";
pub const INPUT_SCHEMA: &str = "ScopedRetrievalQuery@1";
pub const OUTPUT_SCHEMA: &str = "EvidenceSynthesis1@1";

const EFFECT_PREFIX: &str = "compute:throughput-retrieval-engine:";

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ThroughputRetrievalSynthesisInferenceEngineRequest {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ThroughputRetrievalSynthesisInferenceEngineReceipt {
...
pub enum ThroughputRetrievalSynthesisInferenceEngineError {
    #[error("invalid throughput retrieval engine request: {0}")]
    Invalid(String),
    #[error("throughput retrieval engine artifact failed: {0}")]
    Artifact(String),
    #[error("throughput retrieval engine synthesis failed: {0}")]
    Synthesis(String),
}
...
impl ThroughputRetrievalSynthesisInferenceEngineReceipt {
    pub fn validate(&self) -> Result<(), ThroughputRetrievalSynthesisInferenceEngineError> {
        ...
        {
            return Err(ThroughputRetrievalSynthesisInferenceEngineError::Invalid(
                "throughput retrieval engine identity, output, locality, candidates, or effects are incomplete".into(),
            ));
        }
        for values in [
            ...
            if !canonical(values) {
                return Err(ThroughputRetrievalSynthesisInferenceEngineError::Invalid(
                    "throughput retrieval engine ordering is not canonical".into(),
                ));
            }
        }
        let classified = self
        ...
        if classified != self.candidate_order.iter().cloned().collect() {
            return Err(ThroughputRetrievalSynthesisInferenceEngineError::Invalid(
                "throughput retrieval engine states do not partition candidates".into(),
            ));
        }
        if self
            .overflow_order
        ...
        for digest in [
            ...
            if digest.as_str().len() != 64 {
                return Err(ThroughputRetrievalSynthesisInferenceEngineError::Invalid(
                    "throughput retrieval engine digest is invalid".into(),
                ));
            }
        }
        if self
        ...
        {
            return Err(ThroughputRetrievalSynthesisInferenceEngineError::Invalid(
                "throughput retrieval engine effect is outside local computation gate".into(),
            ));
        }
        self.artifact.validate_metadata().map_err(|error| {
            ThroughputRetrievalSynthesisInferenceEngineError::Artifact(error.to_string())
        })
    }
}

pub fn throughput_retrieval_synthesis_inference_engine_manifest() -> CapabilityManifest {
    CapabilityManifest {
        ...
        behavior: "computes a deterministic bounded high-throughput retrieval corpus with batch admission, capacity overflow, checkpoints, and typed evidence synthesis while retaining omissions, uncertainty, contradiction, and negative results".into(),
        value: "provides a separately versioned, replayable inference-engine surface for auditable prospective evidence retrieval without silently dropping queue overflow or substituting defaults".into(),
        ...
        permissions: ["read:institution-local-research-state".into(), "admit:bounded-retrieval-batch".into()].into(),
...
pub fn run_throughput_retrieval_synthesis_inference_engine(
    request: &ThroughputRetrievalSynthesisInferenceEngineRequest,
) -> Result<
...
> {
    ...
    let synthesis = compile_evidence_synthesis(&request.synthesis_request).map_err(|error| {
        ThroughputRetrievalSynthesisInferenceEngineError::Synthesis(error.to_string())
    })?;
...
fn validate_request(
    request: &ThroughputRetrievalSynthesisInferenceEngineRequest,
) -> Result<(), ThroughputRetrievalSynthesisInferenceEngineError> {
...
mod tests {
    ...
    fn request() -> ThroughputRetrievalSynthesisInferenceEngineRequest {
    ...
    fn manifest_is_a1() {
        assert_eq!(
            throughput_retrieval_synthesis_inference_engine_manifest().autonomy_tier,
            AutonomyTier::A1
        );
    }
    ...
    fn computes_local_synthesis() {
        let receipt = run_throughput_retrieval_synthesis_inference_engine(&request()).unwrap();
    ...
    fn policy_block_is_observable() {
        ...
        let receipt = run_throughput_retrieval_synthesis_inference_engine(&value).unwrap();
    ...
    fn rejects_wrong_output() {
        ...
        assert!(run_throughput_retrieval_synthesis_inference_engine(&value).is_err());
    }
    ...
    fn replay_is_stable() {
        let value = request();
        assert_eq!(
            run_throughput_retrieval_synthesis_inference_engine(&value).unwrap(),
            run_throughput_retrieval_synthesis_inference_engine(&value).unwrap()
        );
    }
