//! Federated continual retrieval-and-synthesis research copilot.
//!
//! Atlas feature `AFA-adapter-P02-F12`: a deterministic A2 product surface
...
//! batch/checkpoint identity and purpose-bound peer quorum, emits an
//! aggregate-only approval-aware tool intent, computes only over
//! institution-local candidate metadata, and preserves every omission,
//! uncertainty, contradiction, overflow, and negative result.
...
pub const FEATURE_ID: &str = "AFA-adapter-P02-F12";
pub const CONTRACT_VERSION: &str =
    "adapter-federated-continual-retrieval-synthesis-research-copilot/1.0";
pub const INPUT_SCHEMA: &str = "ScopedRetrievalQuery@1";
pub const OUTPUT_SCHEMA: &str = "EvidenceSynthesis1@1";

const EFFECT_PREFIX: &str = "compute:federated-continual-retrieval-research-copilot:";

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct FederatedContinualRetrievalSynthesisResearchCopilotRequest {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct FederatedContinualRetrievalSynthesisResearchCopilotReceipt {
...
pub enum FederatedContinualRetrievalSynthesisResearchCopilotError {
    #[error("invalid federated continual retrieval research copilot request: {0}")]
    Invalid(String),
    #[error("federated continual retrieval research copilot artifact failed: {0}")]
    Artifact(String),
    #[error("federated continual retrieval research copilot synthesis failed: {0}")]
    Synthesis(String),
}
...
impl FederatedContinualRetrievalSynthesisResearchCopilotReceipt {
    pub fn validate(&self) -> Result<(), FederatedContinualRetrievalSynthesisResearchCopilotError> {
        ...
        {
            return Err(FederatedContinualRetrievalSynthesisResearchCopilotError::Invalid(
                "federated continual retrieval research copilot identity, output, locality, candidates, or effects are incomplete".into(),
            ));
        }
        for values in [
            ...
            if !canonical(values) {
                return Err(
                    FederatedContinualRetrievalSynthesisResearchCopilotError::Invalid(
                        "federated continual retrieval research copilot ordering is not canonical"
                            .into(),
                    ),
                );
            }
        ...
        if classified != self.candidate_order.iter().cloned().collect() {
            return Err(FederatedContinualRetrievalSynthesisResearchCopilotError::Invalid(
                "federated continual retrieval research copilot states do not partition candidates".into(),
            ));
        }
        for digest in [
            ...
            if digest.as_str().len() != 64 {
                return Err(
                    FederatedContinualRetrievalSynthesisResearchCopilotError::Invalid(
                        "federated continual retrieval research copilot digest is invalid".into(),
                    ),
                );
            }
        }
        if self.effect_receipts.iter().any(|effect| {
            ...
            return Err(FederatedContinualRetrievalSynthesisResearchCopilotError::Invalid(
                "federated continual retrieval research copilot effect is outside local computation or approval gate".into(),
            ));
        }
        if self
            .overflow_order
        ...
        self.artifact.validate_metadata().map_err(|error| {
            FederatedContinualRetrievalSynthesisResearchCopilotError::Artifact(error.to_string())
        })
    }
}

pub fn federated_continual_retrieval_synthesis_research_copilot_manifest() -> CapabilityManifest {
    CapabilityManifest {
        ...
        behavior: "runs a bounded A2 federated continual research copilot that compares at least two institution-local modalities across studies, admits queue capacity with checkpoint identity and purpose-bound peer quorum, emits an aggregate-only approval-aware tool intent, and retains omissions, uncertainty, contradiction, overflow, and negative results".into(),
        value: "provides a separately versioned auditable copilot surface for continual consortium evidence synthesis without moving raw observations, silently dropping overflow, or making clinical decisions".into(),
...
pub fn run_federated_continual_retrieval_synthesis_research_copilot(
...
> {
    ...
    let synthesis = compile_evidence_synthesis(&bounded_request).map_err(|error| {
        FederatedContinualRetrievalSynthesisResearchCopilotError::Synthesis(error.to_string())
    })?;
    ...
    } else if request.peer_ids.len() < request.min_peer_quorum as usize {
        EvidenceSynthesisDisposition::Blocked
    } else {
    ...
    let copilot_digest = ContentHash::of_value(&json!({
        ...
        "federation_digest": request.federation_digest,
    ...
    let artifact = TypedResearchArtifact::from_payload(
        format!(
            "adapter-federated-continual-retrieval-research-copilot:{}",
            request.copilot_id
        ),
...
fn validate_request(
    request: &FederatedContinualRetrievalSynthesisResearchCopilotRequest,
) -> Result<(), FederatedContinualRetrievalSynthesisResearchCopilotError> {
        ...
        || request.replay_identity.as_str().len() != 64
    {
        return Err(
            FederatedContinualRetrievalSynthesisResearchCopilotError::Invalid(
                "contract identity, output, budget, locality, replay, or boundary is invalid"
                    .into(),
            ),
        );
    }
...
mod tests {
    ...
    fn request() -> FederatedContinualRetrievalSynthesisResearchCopilotRequest {
        FederatedContinualRetrievalSynthesisResearchCopilotRequest {
            ...
            purpose: "continual preclinical retrieval benchmark".into(),
            peer_ids: vec!["peer:a".into(), "peer:b".into()],
            min_peer_quorum: 2,
            aggregate_only: true,
            endpoint: "local://federation".into(),
            federation_digest: ContentHash::of_bytes(b"federation:f21"),
    ...
    fn manifest_is_a2() {
        assert_eq!(
            federated_continual_retrieval_synthesis_research_copilot_manifest().autonomy_tier,
            AutonomyTier::A2
        );
    }
    ...
    fn computes_local_synthesis() {
        let receipt =
            run_federated_continual_retrieval_synthesis_research_copilot(&request()).unwrap();
    ...
    fn policy_block_is_observable() {
        ...
        let receipt = run_federated_continual_retrieval_synthesis_research_copilot(&value).unwrap();
    ...
    fn rejects_wrong_output() {
        ...
        assert!(run_federated_continual_retrieval_synthesis_research_copilot(&value).is_err());
    }
    ...
    fn replay_is_stable() {
        let value = request();
        assert_eq!(
            run_federated_continual_retrieval_synthesis_research_copilot(&value).unwrap(),
            run_federated_continual_retrieval_synthesis_research_copilot(&value).unwrap()
        );
    }
    ...
    #[test]
    fn quorum_failure_is_blocked_without_raw_data_effect() {
        let mut value = request();
        value.peer_ids = vec!["peer:a".into()];
        let receipt = run_federated_continual_retrieval_synthesis_research_copilot(&value).unwrap();
        assert_eq!(receipt.disposition, EvidenceSynthesisDisposition::Blocked);
