...
//!
//! Atlas feature `AFA-adapter-P02-F02`: a deterministic A0 product surface
...
pub const FEATURE_ID: &str = "AFA-adapter-P02-F02";
pub const CONTRACT_VERSION: &str = "adapter-multimodal-retrieval-synthesis-inference-engine/1.0";
...
pub struct MultimodalRetrievalSynthesisInferenceEngineRequest {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct MultimodalRetrievalSynthesisInferenceEngineReceipt {
...
pub enum MultimodalRetrievalSynthesisInferenceEngineError {
    #[error("invalid multimodal retrieval engine request: {0}")]
    Invalid(String),
    #[error("multimodal retrieval engine artifact failed: {0}")]
    Artifact(String),
    #[error("multimodal retrieval engine synthesis failed: {0}")]
    Synthesis(String),
}
...
impl MultimodalRetrievalSynthesisInferenceEngineReceipt {
    pub fn validate(&self) -> Result<(), MultimodalRetrievalSynthesisInferenceEngineError> {
        ...
        {
            return Err(MultimodalRetrievalSynthesisInferenceEngineError::Invalid(
                "multimodal retrieval engine identity, output, locality, candidates, or effects are incomplete".into(),
            ));
        }
        for values in [
            ...
            if !canonical(values) {
                return Err(MultimodalRetrievalSynthesisInferenceEngineError::Invalid(
                    "multimodal retrieval engine ordering is not canonical".into(),
                ));
            }
        }
        let classified = self
        ...
        if classified != self.candidate_order.iter().cloned().collect() {
            return Err(MultimodalRetrievalSynthesisInferenceEngineError::Invalid(
                "multimodal retrieval engine states do not partition candidates".into(),
            ));
        }
        for digest in [
            ...
            if digest.as_str().len() != 64 {
                return Err(MultimodalRetrievalSynthesisInferenceEngineError::Invalid(
                    "multimodal retrieval engine digest is invalid".into(),
                ));
            }
        }
        if self
        ...
        {
            return Err(MultimodalRetrievalSynthesisInferenceEngineError::Invalid(
                "multimodal retrieval engine effect is outside local computation gate".into(),
            ));
        }
        self.artifact.validate_metadata().map_err(|error| {
            MultimodalRetrievalSynthesisInferenceEngineError::Artifact(error.to_string())
        })
    }
}

pub fn multimodal_retrieval_synthesis_inference_engine_manifest() -> CapabilityManifest {
    CapabilityManifest {
...
pub fn run_multimodal_retrieval_synthesis_inference_engine(
    request: &MultimodalRetrievalSynthesisInferenceEngineRequest,
) -> Result<
...
> {
    ...
    let synthesis = compile_evidence_synthesis(&request.synthesis_request).map_err(|error| {
        MultimodalRetrievalSynthesisInferenceEngineError::Synthesis(error.to_string())
    })?;
...
fn validate_request(
    request: &MultimodalRetrievalSynthesisInferenceEngineRequest,
) -> Result<(), MultimodalRetrievalSynthesisInferenceEngineError> {
...
mod tests {
    ...
    fn request() -> MultimodalRetrievalSynthesisInferenceEngineRequest {
    ...
    fn manifest_is_a0() {
        assert_eq!(
            multimodal_retrieval_synthesis_inference_engine_manifest().autonomy_tier,
            AutonomyTier::A1
        );
    }
    ...
    fn policy_block_is_observable() {
        ...
        let receipt = run_multimodal_retrieval_synthesis_inference_engine(&value).unwrap();
    ...
    fn rejects_wrong_output() {
        ...
        assert!(run_multimodal_retrieval_synthesis_inference_engine(&value).is_err());
    }
    ...
    fn replay_is_stable() {
        let value = request();
        assert_eq!(
            run_multimodal_retrieval_synthesis_inference_engine(&value).unwrap(),
            run_multimodal_retrieval_synthesis_inference_engine(&value).unwrap()
        );
    }
}
