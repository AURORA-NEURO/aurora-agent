//! Multimodal multi-study retrieval-and-synthesis research copilot.
//!
//! Atlas feature `AFA-adapter-P02-F10`: a deterministic A2 product surface
//! around the typed retrieval/synthesis contract.  The copilot compares at
//! least two modalities across studies, emits a bounded approval-aware tool
//! intent, computes only over institution-local candidate metadata, and
//! preserves every omission, uncertainty, contradiction, and negative result.
...
pub const FEATURE_ID: &str = "AFA-adapter-P02-F10";
pub const CONTRACT_VERSION: &str = "adapter-multimodal-retrieval-synthesis-research-copilot/1.0";
pub const INPUT_SCHEMA: &str = "ScopedRetrievalQuery@1";
pub const OUTPUT_SCHEMA: &str = "EvidenceSynthesis1@1";

const EFFECT_PREFIX: &str = "compute:multimodal-retrieval-research-copilot:";

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct MultimodalRetrievalSynthesisResearchCopilotRequest {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct MultimodalRetrievalSynthesisResearchCopilotReceipt {
...
pub enum MultimodalRetrievalSynthesisResearchCopilotError {
    #[error("invalid multimodal retrieval research copilot request: {0}")]
    Invalid(String),
    #[error("multimodal retrieval research copilot artifact failed: {0}")]
    Artifact(String),
    #[error("multimodal retrieval research copilot synthesis failed: {0}")]
    Synthesis(String),
}
...
impl MultimodalRetrievalSynthesisResearchCopilotReceipt {
    pub fn validate(&self) -> Result<(), MultimodalRetrievalSynthesisResearchCopilotError> {
        ...
        {
            return Err(MultimodalRetrievalSynthesisResearchCopilotError::Invalid(
                "multimodal retrieval research copilot identity, output, locality, candidates, or effects are incomplete".into(),
            ));
        }
        for values in [
            ...
            if !canonical(values) {
                return Err(MultimodalRetrievalSynthesisResearchCopilotError::Invalid(
                    "multimodal retrieval research copilot ordering is not canonical".into(),
                ));
            }
        }
        let classified = self
        ...
        if classified != self.candidate_order.iter().cloned().collect() {
            return Err(MultimodalRetrievalSynthesisResearchCopilotError::Invalid(
                "multimodal retrieval research copilot states do not partition candidates".into(),
            ));
        }
        for digest in [
            ...
            if digest.as_str().len() != 64 {
                return Err(MultimodalRetrievalSynthesisResearchCopilotError::Invalid(
                    "multimodal retrieval research copilot digest is invalid".into(),
                ));
            }
        }
        if self.effect_receipts.iter().any(|effect| {
            ...
            return Err(MultimodalRetrievalSynthesisResearchCopilotError::Invalid(
                "multimodal retrieval research copilot effect is outside local computation or approval gate".into(),
            ));
        }
        self.artifact.validate_metadata().map_err(|error| {
            MultimodalRetrievalSynthesisResearchCopilotError::Artifact(error.to_string())
        })
    }
}

pub fn multimodal_retrieval_synthesis_research_copilot_manifest() -> CapabilityManifest {
    CapabilityManifest {
        ...
        behavior: "runs a bounded A2 multimodal research copilot that compares at least two institution-local modalities across studies, emits an approval-aware tool intent, and retains omissions, uncertainty, contradiction, and negative results".into(),
        value: "provides a separately versioned auditable copilot surface for comparable imaging, omics, and assay evidence synthesis without silently substituting defaults or making clinical decisions".into(),
...
pub fn run_multimodal_retrieval_synthesis_research_copilot(
...
> {
    ...
    let synthesis = compile_evidence_synthesis(&request.synthesis_request).map_err(|error| {
        MultimodalRetrievalSynthesisResearchCopilotError::Synthesis(error.to_string())
    })?;
    ...
    let artifact = TypedResearchArtifact::from_payload(
        format!(
            "adapter-multimodal-retrieval-research-copilot:{}",
            request.copilot_id
        ),
...
fn validate_request(
    request: &MultimodalRetrievalSynthesisResearchCopilotRequest,
) -> Result<(), MultimodalRetrievalSynthesisResearchCopilotError> {
...
mod tests {
    ...
    fn request() -> MultimodalRetrievalSynthesisResearchCopilotRequest {
    ...
    fn manifest_is_a2() {
        assert_eq!(
            multimodal_retrieval_synthesis_research_copilot_manifest().autonomy_tier,
            AutonomyTier::A2
        );
    }
    ...
    fn computes_local_synthesis() {
        let receipt = run_multimodal_retrieval_synthesis_research_copilot(&request()).unwrap();
        assert_eq!(receipt.feature_id, FEATURE_ID);
        assert_eq!(
            receipt.selected_order,
            vec!["evidence:f21", "evidence:f21-transcriptomics"]
        );
    }
    ...
    fn policy_block_is_observable() {
        ...
        let receipt = run_multimodal_retrieval_synthesis_research_copilot(&value).unwrap();
    ...
    fn rejects_wrong_output() {
        ...
        assert!(run_multimodal_retrieval_synthesis_research_copilot(&value).is_err());
    }
    ...
    fn replay_is_stable() {
        let value = request();
        assert_eq!(
            run_multimodal_retrieval_synthesis_research_copilot(&value).unwrap(),
            run_multimodal_retrieval_synthesis_research_copilot(&value).unwrap()
        );
    }
