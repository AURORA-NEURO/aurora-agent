...
pub const CONTRACT_VERSION: &str = "adapter-federated-continual-evidence-surveillance-workflow-fabric/1.0";
    ...
    pub fn validate(&self) -> Result<(), FederatedContinualEvidenceSurveillanceWorkflowError> {
            ...
            || self.effect_receipts.is_empty()
        {
            return Err(FederatedContinualEvidenceSurveillanceWorkflowError::Invalid(
                "federation identity, workflow stages, locality, or effects are incomplete".into(),
            ));
        }
        ...
        for values in [
            &self.plan_order, &self.completed_order, &self.blocked_order,
            &self.compensation_order, &self.peer_order, &self.candidate_order,
            &self.selected_order, &self.unresolved_order, &self.denied_order,
            &self.aggregate_order, &self.omissions, &self.uncertainty,
        ...
        for value in [
            ...
            &self.federation_envelope_digest, &self.workflow_digest,
...
pub fn schedule_federated_continual_evidence_surveillance_workflow(
...
) -> Result<FederatedContinualEvidenceSurveillanceWorkflowReceipt, FederatedContinualEvidenceSurveillanceWorkflowError> {
    validate_request(request)?;
    let copilot = run_federated_continual_evidence_surveillance_research_copilot(&request.request)
        .map_err(|error| FederatedContinualEvidenceSurveillanceWorkflowError::Copilot(error.to_string()))?;
    ...
    let blocked_order = if disposition == FederatedContinualResearchCopilotDisposition::Blocked { vec!["stage:release".into()] } else { Vec::new() };
    ...
    let copilot_run_digest = ContentHash::of_value(&serde_json::to_value(&copilot).map_err(|error| FederatedContinualEvidenceSurveillanceWorkflowError::Artifact(error.to_string()))?).map_err(|error| FederatedContinualEvidenceSurveillanceWorkflowError::Artifact(error.to_string()))?;
    ...
    let artifact = TypedResearchArtifact::from_payload(format!("adapter-federated-continual-evidence-workflow:{}", request.workflow_id), "application/vnd.aurora.federated-continual-research-workflow+json", &payload, vec![], vec![]).map_err(|error| FederatedContinualEvidenceSurveillanceWorkflowError::Artifact(error.to_string()))?;
    let effect_receipts = if disposition == FederatedContinualResearchCopilotDisposition::Blocked { vec!["block:unsafe-release".into()] } else if !compensation_order.is_empty() { vec![format!("compensate:research-work:{}", request.workflow_id)] } else { vec![format!("schedule:research-work:{}", request.workflow_id)] };
    let receipt = FederatedContinualEvidenceSurveillanceWorkflowReceipt { schema_version: RESEARCH_CONTRACT_SCHEMA_VERSION.into(), contract_version: CONTRACT_VERSION.into(), feature_id: FEATURE_ID.into(), request_id: request.request.request_id.clone(), workflow_id: request.workflow_id.clone(), agent_id: request.request.agent_id.clone(), federation_id: request.request.federation_id.clone(), purpose: request.request.purpose.clone(), endpoint: request.request.endpoint.clone(), disposition, stage_order, plan_order, completed_order, blocked_order, compensation_order, peer_order: copilot.peer_order.clone(), candidate_order: copilot.candidate_order.clone(), selected_order: copilot.selected_order.clone(), unresolved_order: copilot.unresolved_order.clone(), denied_order: copilot.denied_order.clone(), aggregate_order: copilot.aggregate_order.clone(), replay_identity: request.replay_identity.clone(), copilot_run_digest, checkpoint_digest, federation_envelope_digest, workflow_digest, omissions, uncertainty: copilot.uncertainty.clone(), negative_evidence: copilot.negative_evidence.clone(), effect_receipts, artifact, raw_data_local: request.request.raw_data_local, boundary: request.boundary.clone() };
    receipt.validate()?;
    Ok(receipt)
}

fn validate_request(request: &FederatedContinualEvidenceSurveillanceWorkflowRequest) -> Result<(), FederatedContinualEvidenceSurveillanceWorkflowError> {
    if request.workflow_id.trim().is_empty() || request.checkpoint_id.trim().is_empty() || request.budget_units == 0 || request.boundary != PRECLINICAL_BOUNDARY || request.request.boundary != PRECLINICAL_BOUNDARY || !request.request.raw_data_local { return Err(FederatedContinualEvidenceSurveillanceWorkflowError::Invalid("workflow identity, checkpoint, budget, locality, or preclinical boundary is invalid".into())); }
    let expected = CANONICAL_STAGES.iter().map(|stage| (*stage).to_string()).collect::<Vec<_>>();
    if request.requested_stage_order != expected { return Err(FederatedContinualEvidenceSurveillanceWorkflowError::Invalid("workflow stage order is not canonical".into())); }
    if request.request.replay_identity.as_str().len() != 64 || request.replay_identity.as_str().len() != 64 { return Err(FederatedContinualEvidenceSurveillanceWorkflowError::Invalid("replay identity is invalid".into())); }
    Ok(())
}
...
mod tests {
    ...
    fn request() -> FederatedContinualEvidenceSurveillanceWorkflowRequest {
        FederatedContinualEvidenceSurveillanceWorkflowRequest { request: FederatedContinualEvidenceSurveillanceResearchCopilotRequest { request_id: "req-16".into(), agent_id: "agent-16".into(), federation_id: "fed-16".into(), purpose: "preclinical-evidence-surveillance".into(), endpoint: "https://local.example/federation".into(), semantic_profile: "profile-1".into(), allowed_artifacts: vec!["summary".into()], min_peer_quorum: 2, declared_tools: vec!["evidence.aggregate".into()], requested_tool: "evidence.aggregate".into(), max_tool_calls: 2, dry_run: true, approval_reference: None, approval_granted: false, contributions: vec![FederatedCopilotEvidenceContribution { peer_id: "peer-a".into(), institution_id: "inst-a".into(), source_id: "source-a".into(), semantic_profile: "profile-1".into(), artifact_kind: "summary".into(), digest: Some(ContentHash::of_bytes(b"source-a")), signed: true, permitted_artifact: true, aggregate_only: true, evidence_state: EvidenceState::Supported, negative_result: false }, FederatedCopilotEvidenceContribution { peer_id: "peer-b".into(), institution_id: "inst-b".into(), source_id: "source-b".into(), semantic_profile: "profile-1".into(), artifact_kind: "summary".into(), digest: Some(ContentHash::of_bytes(b"source-b")), signed: true, permitted_artifact: true, aggregate_only: true, evidence_state: EvidenceState::Supported, negative_result: false }], policy_allow: true, protected_closure: true, raw_data_local: true, replay_identity: ContentHash::of_bytes(b"copilot-16"), boundary: PRECLINICAL_BOUNDARY.into() }, workflow_id: "workflow-16".into(), requested_stage_order: CANONICAL_STAGES.iter().map(|stage| (*stage).to_string()).collect(), checkpoint_id: "checkpoint-16".into(), budget_units: 8, replay_identity: ContentHash::of_bytes(b"workflow-16"), boundary: PRECLINICAL_BOUNDARY.into() }
    }
    #[test] fn manifest_is_a2_and_aggregate_only() { let m = federated_continual_evidence_surveillance_workflow_fabric_manifest(); assert_eq!(m.autonomy_tier, AutonomyTier::A2); assert!(m.validate().is_ok()); assert!(m.behavior.contains("aggregate-only")); }
    #[test] fn schedules_quorum_workflow() { let receipt = schedule_federated_continual_evidence_surveillance_workflow(&request()).unwrap(); assert_eq!(receipt.feature_id, FEATURE_ID); assert_eq!(receipt.peer_order.len(), 2); assert!(!receipt.federation_envelope_digest.as_str().is_empty()); }
    #[test] fn policy_denial_blocks_release() { let mut r = request(); r.request.policy_allow = false; let receipt = schedule_federated_continual_evidence_surveillance_workflow(&r).unwrap(); assert_eq!(receipt.disposition, FederatedContinualResearchCopilotDisposition::Blocked); assert_eq!(receipt.effect_receipts, vec!["block:unsafe-release"]); }
    #[test] fn quorum_failure_is_blocked() { let mut r = request(); r.request.min_peer_quorum = 3; let receipt = schedule_federated_continual_evidence_surveillance_workflow(&r).unwrap(); assert_eq!(receipt.disposition, FederatedContinualResearchCopilotDisposition::Blocked); assert!(receipt.omissions.iter().any(|item| item.contains("peer-quorum"))); }
    #[test] fn budget_adds_compensation() { let mut r = request(); r.budget_units = 1; let receipt = schedule_federated_continual_evidence_surveillance_workflow(&r).unwrap(); assert!(receipt.compensation_order.iter().any(|item| item.contains("budget-exhausted"))); assert!(receipt.effect_receipts[0].starts_with("compensate:")); }
    #[test] fn stage_order_is_rejected() { let mut r = request(); r.requested_stage_order.reverse(); assert!(schedule_federated_continual_evidence_surveillance_workflow(&r).is_err()); }
    #[test] fn replay_is_stable() { let r = request(); assert_eq!(schedule_federated_continual_evidence_surveillance_workflow(&r).unwrap(), schedule_federated_continual_evidence_surveillance_workflow(&r).unwrap()); }
}
