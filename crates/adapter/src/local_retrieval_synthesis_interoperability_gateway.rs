//! Version-negotiated local retrieval/synthesis interoperability gateway.
//! Atlas feature `AFA-adapter-P02-F21`.
//!
...
pub const FEATURE_ID: &str = "AFA-adapter-P02-F21";
pub const CONTRACT_VERSION: &str = "adapter-local-retrieval-synthesis-interoperability-gateway/1.0";
...
pub struct LocalRetrievalSynthesisInteroperabilityGatewayRequest {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct LocalRetrievalSynthesisInteroperabilityGatewayReceipt {
...
pub enum LocalRetrievalSynthesisInteroperabilityGatewayError {
...
impl LocalRetrievalSynthesisInteroperabilityGatewayReceipt {
    pub fn validate(&self) -> Result<(), LocalRetrievalSynthesisInteroperabilityGatewayError> {
        ...
        self.artifact.validate_metadata().map_err(|error| {
            LocalRetrievalSynthesisInteroperabilityGatewayError::Artifact(error.to_string())
        })
    }

    fn invalid(message: &str) -> LocalRetrievalSynthesisInteroperabilityGatewayError {
        LocalRetrievalSynthesisInteroperabilityGatewayError::Invalid(message.into())
    }
}

pub fn local_retrieval_synthesis_interoperability_gateway_manifest() -> CapabilityManifest {
    CapabilityManifest {
        schema_version: RESEARCH_CONTRACT_SCHEMA_VERSION.into(),
        capability_id: FEATURE_ID.into(),
        version: CONTRACT_VERSION.into(),
        owner_crate: "adapter".into(),
        consumers: ["AURORA extension developer".into(), "preclinical researcher".into()].into(),
        behavior: "negotiates a version-pinned local retrieval/synthesis protocol and projects a typed workbench artifact".into(),
...
pub fn render_local_retrieval_synthesis_interoperability_gateway(
    request: &LocalRetrievalSynthesisInteroperabilityGatewayRequest,
) -> Result<
...
> {
    ...
    let integration = negotiate_interoperability(&request.interop_request).map_err(|error| {
        LocalRetrievalSynthesisInteroperabilityGatewayError::Invalid(error.to_string())
    })?;
    let workbench = render_local_retrieval_synthesis_research_workbench(&request.workbench_request)
        .map_err(|error| {
            LocalRetrievalSynthesisInteroperabilityGatewayError::Workbench(error.to_string())
        })?;
    ...
    if loss.len() as u32 > request.semantic_loss_budget {
        return Err(
            LocalRetrievalSynthesisInteroperabilityGatewayError::Invalid(
    ...
    let artifact = TypedResearchArtifact::from_payload(
        ...
        "application/vnd.aurora.local-retrieval-synthesis-interoperability+json",
        &protocol_payload,
        Vec::new(),
        Vec::new(),
    )
...
fn validate_request(
    request: &LocalRetrievalSynthesisInteroperabilityGatewayRequest,
) -> Result<(), LocalRetrievalSynthesisInteroperabilityGatewayError> {
    ...
    {
        return Err(LocalRetrievalSynthesisInteroperabilityGatewayError::Invalid("protocol schemas, migration policy, loss budget, locality, and boundary are invalid".into()));
    }
    Ok(())
}
...
mod tests {
    ...
    fn manifest_is_a1_and_pinned() {
        let manifest = local_retrieval_synthesis_interoperability_gateway_manifest();
