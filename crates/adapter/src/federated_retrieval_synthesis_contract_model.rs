...
//!
//! Atlas feature `AFA-adapter-P02-F08`: a deterministic A0 product surface
...
pub const FEATURE_ID: &str = "AFA-adapter-P02-F08";
pub const CONTRACT_VERSION: &str = "adapter-federated-retrieval-synthesis-contract-model/1.0";
pub const INPUT_SCHEMA: &str = "ScopedRetrievalQuery@1";
pub const OUTPUT_SCHEMA: &str = "EvidenceSynthesis1@1";

const EFFECT_PREFIX: &str = "compute:federated-retrieval-contract-model:";

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct FederatedRetrievalSynthesisContractModelRequest {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct FederatedRetrievalSynthesisContractModelReceipt {
...
pub enum FederatedRetrievalSynthesisContractModelError {
    #[error("invalid federated retrieval contract model request: {0}")]
    Invalid(String),
    #[error("federated retrieval contract model artifact failed: {0}")]
    Artifact(String),
    #[error("federated retrieval contract model synthesis failed: {0}")]
    Synthesis(String),
}
...
impl FederatedRetrievalSynthesisContractModelReceipt {
    pub fn validate(&self) -> Result<(), FederatedRetrievalSynthesisContractModelError> {
        ...
        {
            return Err(FederatedRetrievalSynthesisContractModelError::Invalid(
                "federated retrieval contract model identity, output, locality, candidates, or effects are incomplete".into(),
            ));
        }
        for values in [
            &self.peer_order,
            ...
            if !canonical(values) {
                return Err(FederatedRetrievalSynthesisContractModelError::Invalid(
                    "federated retrieval contract model ordering is not canonical".into(),
                ));
            }
        }
        ...
        if classified != self.candidate_order.iter().cloned().collect() {
            return Err(FederatedRetrievalSynthesisContractModelError::Invalid(
                "federated retrieval contract model states do not partition candidates".into(),
            ));
        }
        if self
            .overflow_order
        ...
        for digest in [
            ...
            if digest.as_str().len() != 64 {
                return Err(FederatedRetrievalSynthesisContractModelError::Invalid(
                    "federated retrieval contract model digest is invalid".into(),
                ));
            }
        }
        if self
        ...
        {
            return Err(FederatedRetrievalSynthesisContractModelError::Invalid(
                "federated retrieval contract model effect is outside local computation gate"
                    .into(),
            ));
        }
        self.artifact.validate_metadata().map_err(|error| {
            FederatedRetrievalSynthesisContractModelError::Artifact(error.to_string())
        })
    }
}

pub fn federated_retrieval_synthesis_contract_model_manifest() -> CapabilityManifest {
    CapabilityManifest {
        ...
        behavior: "validates and materializes a deterministic federated continual retrieval corpus as a typed data primitive from permitted aggregate contributions with purpose, signer, quorum, locality, and policy gates while retaining omissions, uncertainty, contradiction, and negative results".into(),
        value: "provides a separately versioned canonical contract model for policy-separated institutions without moving raw observations or silently substituting defaults".into(),
...
pub fn run_federated_retrieval_synthesis_contract_model(
    request: &FederatedRetrievalSynthesisContractModelRequest,
) -> Result<
...
> {
    ...
    let synthesis = compile_evidence_synthesis(&request.synthesis_request).map_err(|error| {
        FederatedRetrievalSynthesisContractModelError::Synthesis(error.to_string())
    })?;
    ...
    .map_err(|error| FederatedRetrievalSynthesisContractModelError::Artifact(error.to_string()))?;
    ...
    let artifact = TypedResearchArtifact::from_payload(
        format!(
            "adapter-throughput-retrieval-engine:{}",
            request.contract_id
        ),
        "application/vnd.aurora.federated-retrieval-synthesis-contract-model+json",
        &payload,
        Vec::new(),
        Vec::new(),
    )
...
fn validate_request(
    request: &FederatedRetrievalSynthesisContractModelRequest,
) -> Result<(), FederatedRetrievalSynthesisContractModelError> {
...
mod tests {
    ...
    fn request() -> FederatedRetrievalSynthesisContractModelRequest {
    ...
    fn manifest_is_a1() {
        assert_eq!(
            federated_retrieval_synthesis_contract_model_manifest().autonomy_tier,
            AutonomyTier::A1
        );
    }
    ...
    fn computes_local_synthesis() {
        let receipt = run_federated_retrieval_synthesis_contract_model(&request()).unwrap();
    ...
    fn policy_block_is_observable() {
        ...
        let receipt = run_federated_retrieval_synthesis_contract_model(&value).unwrap();
    ...
    fn rejects_wrong_output() {
        ...
        assert!(run_federated_retrieval_synthesis_contract_model(&value).is_err());
    }
    ...
    fn replay_is_stable() {
        let value = request();
        assert_eq!(
            run_federated_retrieval_synthesis_contract_model(&value).unwrap(),
            run_federated_retrieval_synthesis_contract_model(&value).unwrap()
        );
    }
