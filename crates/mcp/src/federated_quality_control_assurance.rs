//! Federated continual quality-control assurance for `AFA-mcp-P07-F28`.
//!
...
pub const FEATURE_ID: &str = "AFA-mcp-P07-F28";
pub const CONTRACT_VERSION: &str = "mcp-federated-continual-quality-control-assurance-harness/1.0";
pub const INPUT_SCHEMA: &str = "ResearchObject4@1";
pub const OUTPUT_SCHEMA: &str = "QualityVerdict7@1";
pub const CONTENT_TYPE: &str = "application/vnd.aurora.mcp-quality-verdict-7+json";
...
pub enum QualityAssuranceError {
    #[error("invalid federated quality-control request: {0}")]
    Invalid(String),
    #[error("federated quality-control artifact failed: {0}")]
    Artifact(String),
}
...
pub fn federated_quality_control_manifest() -> CapabilityManifest {
    CapabilityManifest {
        ...
        behavior: "verifies digest-bound aggregate quality declarations across policy-separated institutions".into(),
        value: "prevents stale, contradictory, missing, or unmeasured quality evidence from silently entering a research workflow".into(),
        surfaces: BTreeSet::from_iter(
            [ResearchSurface::Api, ResearchSurface::McpTool, ResearchSurface::Operator]
                .into_iter(),
        ),
        inputs: vec![TypedPort {
            name: "research_objects".into(),
...
pub fn assure_federated_quality(
    ...
    let artifact = TypedResearchArtifact::from_payload(
        format!("mcp-quality-verdict-7:{}", request.request_id),
        CONTENT_TYPE,
        &payload,
        Vec::new(),
        Vec::new(),
...
mod tests {
    ...
    fn manifest_is_typed() {
        assert_eq!(
            federated_quality_control_manifest().capability_id,
            FEATURE_ID
        );
    }
    ...
    fn complete_federation_qualifies() {
        let receipt = assure_federated_quality(&request()).unwrap();
    ...
    fn canonical_order_is_reproducible() {
        let first = assure_federated_quality(&request()).unwrap();
        let mut reversed = request();
        reversed.objects.reverse();
        let second = assure_federated_quality(&reversed).unwrap();
