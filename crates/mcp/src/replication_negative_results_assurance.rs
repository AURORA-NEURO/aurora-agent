...
pub const CONTRACT_VERSION: &str =
    "mcp-prospective-high-throughput-replication-negative-results-assurance/1.0";
...
pub enum ReplicationAssuranceError {
    ...
    Invalid(String),
    #[error("replication assurance artifact failed: {0}")]
    Artifact(String),
}
...
impl ReplicationRecord7 {
    pub fn validate(&self) -> Result<(), ReplicationAssuranceError> {
        ...
        {
            return Err(ReplicationAssuranceError::Invalid(
                "identity, locality, observations, or effects are incomplete".into(),
            ));
        }
        for values in [
        ...
        {
            return Err(ReplicationAssuranceError::Artifact(
        ...
        if self
            .effect_receipts
            .iter()
            .any(|e| e != "block:unsafe-release" && !e.starts_with("release:replication:"))
        {
            return Err(ReplicationAssuranceError::Invalid(
                "effect outside assurance boundary".into(),
            ));
        }
        Ok(())
    }
...
pub fn assure_replication(
    request: ClaimAndProtocol3,
) -> Result<ReplicationRecord7, ReplicationAssuranceError> {
    if request.request_id.trim().is_empty()
        || request.claim_id.trim().is_empty()
        || request.claim_text.trim().is_empty()
        || request.protocol_id.trim().is_empty()
        || request.semantic_profile.trim().is_empty()
        || request.expected_direction.trim().is_empty()
        || request.minimum_replicates == 0
        || request.batch_limit == 0
        || request.observations.is_empty()
        || request.observations.len() > request.batch_limit
        || request.observations.len() > MAX_OBSERVATIONS
        || request.boundary != PRECLINICAL_BOUNDARY
        || !request.raw_data_local
        || !request.aggregate_only
        || !digest(&request.protocol_digest)
        || !digest(&request.baseline_digest)
        || !digest(&request.replay_identity)
    {
        return Err(ReplicationAssuranceError::Invalid(
            "claim, batch, digest, locality, or boundary constraints are invalid".into(),
        ));
    }
    ...
    for row in &rows {
        ...
        match row.outcome {
            ...
            ReplicationOutcome::Null => {
                ...
                negative_evidence.insert(format!("{}:null", row.observation_id));
            }
            ReplicationOutcome::Negative => {
                negative.insert(row.observation_id.clone());
                negative_evidence.insert(format!("{}:negative", row.observation_id));
            }
            ReplicationOutcome::Inconclusive => {
                inconclusive.insert(row.observation_id.clone());
            }
        }
        let compatible = row.protocol_id == request.protocol_id
            && row.semantic_profile == request.semantic_profile
            && row.replay_identity == request.replay_identity
            && row.signed
            && row.comparable
            && row.raw_data_local
            && row.aggregate_only;
        if row.evidence_state == QualityEvidenceState::Contradicted {
    ...
    let disposition = if global_block || !blocked.is_empty() {
        "blocked"
    } else if qualified.len() < request.minimum_replicates
        || !negative.is_empty()
        || !nulls.is_empty()
    ...
    let effect_median_milli = if effect_values.is_empty() {
        0
    } else {
        effect_values.sort();
    ...
    let record_digest = ContentHash::of_value(&payload)
        .map_err(|e| ReplicationAssuranceError::Artifact(e.to_string()))?;
    let artifact = TypedResearchArtifact::from_payload(
        format!("replication-record-7:{}", request.request_id),
        CONTENT_TYPE,
        &payload,
        Vec::new(),
        Vec::new(),
    ...
    .map_err(|e| ReplicationAssuranceError::Artifact(e.to_string()))?;
    effects.push(if disposition == "qualified" {
        format!("release:replication:qualified:{}", request.request_id)
    } else {
        "block:unsafe-release".into()
    });
    let mut receipt = ReplicationRecord7 {
        ...
        missing_site_order: site_order
            .into_iter()
            .filter(|s| {
                !rows.iter().any(|r| {
                    r.site_id == *s && receipt_placeholder(&qualified_order, &r.observation_id)
                })
            })
            .collect(),
...
fn receipt_placeholder(qualified: &[String], id: &str) -> bool {
    qualified.iter().any(|q| q == id)
}
...
mod tests {
    ...
    fn request() -> ClaimAndProtocol3 {
        ClaimAndProtocol3 {
            ...
            observations: vec![obs(
                "a",
                ReplicationOutcome::Positive,
                QualityEvidenceState::Supported,
            )],
        }
    }
    fn obs(
    ...
    fn manifest_is_versioned() {
        assert_eq!(
            replication_assurance_manifest()["capability_id"],
            FEATURE_ID
        );
    }
    #[test]
    fn qualified_batch_is_released() {
        let r = assure_replication(request()).unwrap();
        assert_eq!(r.disposition, "qualified");
        assert!(r.effect_receipts[0].starts_with("release:replication:"));
    }
    #[test]
    fn null_is_unresolved() {
        let mut q = request();
        q.observations[0].outcome = ReplicationOutcome::Null;
        let r = assure_replication(q).unwrap();
        assert_eq!(r.disposition, "unresolved");
        assert!(r
            .negative_evidence_order
            .iter()
            .any(|v| v.ends_with(":null")));
    }
    #[test]
    ...
    fn contradiction_blocks() {
        let mut q = request();
        q.observations[0].evidence_state = QualityEvidenceState::Contradicted;
        let r = assure_replication(q).unwrap();
        assert_eq!(r.disposition, "blocked");
    }
    #[test]
    fn deterministic_digest() {
        let a = assure_replication(request()).unwrap();
        let mut q = request();
        q.observations.reverse();
        let b = assure_replication(q).unwrap();
