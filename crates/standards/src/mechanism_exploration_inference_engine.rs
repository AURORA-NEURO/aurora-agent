//! Federated continual mechanism-exploration assurance for `AFA-standards-P08-F04`.
//!
...
pub const FEATURE_ID: &str = "AFA-standards-P08-F04";
pub const CONTRACT_VERSION: &str =
    "standards-federated-continual-mechanism-exploration-inference-engine/1.0";
pub const INPUT_SCHEMA: &str = "StandardsMechanismQuestion6@1";
pub const OUTPUT_SCHEMA: &str = "StandardsMechanismInferenceReceipt8@1";
pub const CONTENT_TYPE: &str =
    "application/vnd.aurora.standards-mechanism-inference-receipt-8+json";

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct StandardsMechanismCandidate6 {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct StandardsMechanismPeer5 {
    pub peer_id: String,
    pub origin: String,
    pub mechanism_id: String,
    pub semantic_profile: String,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct StandardsMechanismQuestion6 {
    ...
    pub candidates: Vec<StandardsMechanismCandidate6>,
    pub peers: Vec<StandardsMechanismPeer5>,
    pub checkpoint: u64,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct StandardsMechanismInferenceArtifact8 {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct StandardsMechanismInferenceReceipt8 {
    ...
    pub artifact: StandardsMechanismInferenceArtifact8,
...
pub enum StandardsMechanismInferenceError {
    #[error("invalid standards mechanism inference request: {0}")]
    Invalid(String),
    #[error("standards mechanism inference artifact failed: {0}")]
    Artifact(String),
}
pub fn standards_mechanism_exploration_inference_manifest() -> serde_json::Value {
    json!({"schema_version":RESEARCH_CONTRACT_SCHEMA_VERSION,"capability_id":FEATURE_ID,"version":CONTRACT_VERSION,"owner_crate":"standards","consumers":["mechanism scientist","adaptive-panel operator","federation steward"],"behavior":"ranks typed mechanism candidates and peer attestations under reproducibility and policy gates","value":"exposes competing explanations and missing evidence before an adaptive research decision","input_schema":INPUT_SCHEMA,"output_schema":OUTPUT_SCHEMA,"effects":["retain:standards-mechanism-inference","exchange:aggregate-mechanism-summary"],"permissions":["retain:mechanism-evidence","exchange:aggregate-mechanism"],"autonomy_tier":"A1","boundary":PRECLINICAL_BOUNDARY})
}
impl StandardsMechanismInferenceReceipt8 {
    pub fn validate(&self) -> Result<(), StandardsMechanismInferenceError> {
        ...
        {
            return Err(StandardsMechanismInferenceError::Invalid("mechanism identity, checkpoint, locality, candidates, peers, or effects are incomplete".into()));
        }
        for v in [
            ...
            if v.windows(2).any(|w| w[0] >= w[1]) {
                return Err(StandardsMechanismInferenceError::Invalid(
        ...
        {
            return Err(StandardsMechanismInferenceError::Artifact(
...
pub fn infer_standards_mechanisms(
    q: &StandardsMechanismQuestion6,
) -> Result<StandardsMechanismInferenceReceipt8, StandardsMechanismInferenceError> {
    validate_request(q)?;
    ...
    let digest = ContentHash::of_value(&payload)
        .map_err(|e| StandardsMechanismInferenceError::Artifact(e.to_string()))?;
    let artifact = StandardsMechanismInferenceArtifact8 {
        artifact_id: format!("standards-mechanism-inference-receipt-8:{}", q.request_id),
    ...
    let r = StandardsMechanismInferenceReceipt8 {
...
fn validate_request(
    q: &StandardsMechanismQuestion6,
) -> Result<(), StandardsMechanismInferenceError> {
    if ![
        &q.request_id,
        &q.federation_id,
        &q.researcher,
    ...
    {
        return Err(StandardsMechanismInferenceError::Invalid("mechanism identity, bounds, candidates, peers, replay, locality, or boundary is invalid".into()));
    }
...
mod tests {
    ...
    fn q() -> StandardsMechanismQuestion6 {
        let r = h("replay");
        StandardsMechanismQuestion6 {
            ...
            candidates: vec![StandardsMechanismCandidate6 {
            ...
            peers: vec![StandardsMechanismPeer5 {
                peer_id: "peer:a".into(),
                origin: "site:a".into(),
                mechanism_id: "mech:a".into(),
                semantic_profile: "neuro:v1".into(),
    ...
    fn qualified_and_ranked() {
        let r = infer_standards_mechanisms(&q()).unwrap();
    ...
    fn contradiction_blocks() {
        ...
        assert_eq!(
            infer_standards_mechanisms(&x).unwrap().disposition,
            "blocked"
        )
    }
}
