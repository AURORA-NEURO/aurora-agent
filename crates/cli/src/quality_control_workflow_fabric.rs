...
impl QualityWorkflowRun {
    pub fn validate(&self) -> Result<(), QualityWorkflowError> {
        ...
        for values in [
            ...
            if !canonical(values) {
                return Err(QualityWorkflowError::Invalid(
                    "quality workflow ordering is not canonical".into(),
                ));
            }
        }
        let required = self
        ...
        let states = self
            ...
            .iter()
            .chain(self.pending_observation_order.iter())
            .chain(self.quarantined_observation_order.iter())
            .chain(self.blocked_observation_order.iter())
            .cloned()
            .collect::<Vec<_>>();
        if required.len() != self.required_observation_order.len()
            || states.len() != required.len()
            || states.iter().cloned().collect::<BTreeSet<_>>() != required
            || self
                .missing_modality_order
                .iter()
                .any(|modality| !self.required_modality_order.contains(modality))
        {
            return Err(QualityWorkflowError::Invalid(
                "quality observation or modality states do not partition the plan".into(),
            ));
        }
        for digest in [
            ...
            if digest.as_str().len() != 64 {
                return Err(QualityWorkflowError::Invalid(
                    "quality workflow digest is invalid".into(),
                ));
            }
        }
        if self.artifact.content_type != "application/vnd.aurora.quality-workflow-run+json" {
            return Err(QualityWorkflowError::Invalid(
                "quality workflow artifact type is invalid".into(),
            ));
        }
        if self.effect_receipts.iter().any(|effect| {
            !effect.starts_with("retain:quality-workflow:")
                && !effect.starts_with("quarantine:quality-workflow:")
                && effect != "block:unsafe-release"
        }) {
            return Err(QualityWorkflowError::Invalid(
                "quality workflow effect is outside retention/quarantine gate".into(),
            ));
        }
        Ok(())
    }
    ...
    pub fn digest(&self) -> Result<ContentHash, QualityWorkflowError> {
        ...
        ContentHash::of_value(
            &serde_json::to_value(self)
                .map_err(|error| QualityWorkflowError::Artifact(error.to_string()))?,
        )
        .map_err(|error| QualityWorkflowError::Artifact(error.to_string()))
    }
}
...
pub fn assure(draft: &QualityWorkflowDraft) -> Result<QualityWorkflowRun, QualityWorkflowError> {
    if draft.request_id.trim().is_empty()
        || draft.run_id.trim().is_empty()
        || draft.scope.trim().is_empty()
        || draft.semantic_profile.trim().is_empty()
        || draft.required_observation_order.is_empty()
        || !canonical(&draft.required_observation_order)
        || draft.required_modality_order.is_empty()
        || !canonical(&draft.required_modality_order)
        || draft.stage_order
            != STAGES
                .iter()
                .map(|value| (*value).to_string())
    ...
    let required = draft
        ...
        .cloned()
        .collect::<BTreeSet<_>>();
    let modalities = draft
        .required_modality_order
        .iter()
        .cloned()
        .collect::<BTreeSet<_>>();
    if required.len() != draft.required_observation_order.len()
        || modalities.len() != draft.required_modality_order.len()
    {
        return Err(QualityWorkflowError::Invalid(
            "required quality observations or modalities are duplicated".into(),
        ));
    }
    ...
    for observation in &draft.observations {
        if observation.observation_id.trim().is_empty()
            || !required.contains(&observation.observation_id)
            ...
            || observation.baseline_milli.is_none()
            || observation.provenance_digest.is_none()
            || observation.semantic_profile != draft.semantic_profile
            || observation.replay_identity != draft.replay_identity
            || !observation.local_data
            || !canonical(&observation.omissions)
            || !canonical(&observation.uncertainty)
        {
            return Err(QualityWorkflowError::Invalid("quality observation identity, modality, baseline, provenance, profile, replay, locality, or annotations are invalid".into()));
        }
        observed_modalities.insert(observation.modality.clone());
        for item in &observation.omissions {
            omissions.insert(format!("observation:{}:{item}", observation.observation_id));
        }
        for item in &observation.uncertainty {
            uncertainty.insert(format!("observation:{}:{item}", observation.observation_id));
        }
        ...
        match observation.evidence_state {
            ...
            EvidenceState::Unknown | EvidenceState::Speculative => {
                pending.insert(observation.observation_id.clone());
            ...
            {
                passed.insert(observation.observation_id.clone());
            }
            _ => {
                quarantined.insert(observation.observation_id.clone());
                omissions.insert(format!(
                    "observation:{}:threshold-or-closure",
                    observation.observation_id
                ));
            }
        }
    ...
    for id in required.difference(&seen) {
        ...
        omissions.insert(format!("missing-observation:{id}"));
        uncertainty.insert(format!("missing-observation:{id}"));
    }
    let missing_modality = modalities
        .difference(&observed_modalities)
        .cloned()
        .collect::<BTreeSet<_>>();
    for modality in &missing_modality {
        omissions.insert(format!("missing-modality:{modality}"));
        uncertainty.insert(format!("missing-modality:{modality}"));
    }
    ...
    let checkpoint = json!({"request_id":draft.request_id,"run_id":draft.run_id,"stage_order":draft.stage_order,"required_observation_order":draft.required_observation_order,"passed_observation_order":passed_order,"pending_observation_order":pending_order,"quarantined_observation_order":quarantined_order,"blocked_observation_order":blocked_order,"replay_identity":draft.replay_identity});
    let checkpoint_digest = ContentHash::of_value(&checkpoint)
        .map_err(|error| QualityWorkflowError::Artifact(error.to_string()))?;
    let workflow_digest = ContentHash::of_value(&json!({"checkpoint_digest":checkpoint_digest,"disposition":disposition,"semantic_profile":draft.semantic_profile})).map_err(|error| QualityWorkflowError::Artifact(error.to_string()))?;
    let artifact = TypedResearchArtifact::from_payload(
        format!("cli-quality-workflow:{}", draft.request_id),
        "application/vnd.aurora.quality-workflow-run+json",
        &checkpoint,
        Vec::<SemanticLoss>::new(),
        vec![ProvenanceLink {
            source_id: draft.run_id.clone(),
            relation: "quality-workflow-checkpoint".into(),
            digest: checkpoint_digest.clone(),
        }],
    )
    .map_err(|error| QualityWorkflowError::Artifact(error.to_string()))?;
    let effect_receipts = if disposition == "qualified" {
        vec![format!("retain:quality-workflow:{}", draft.request_id)]
    } else if disposition == "quarantine" {
        vec![format!("quarantine:quality-workflow:{}", draft.request_id)]
    } else {
        vec!["block:unsafe-release".into()]
    };
    let run = QualityWorkflowRun {
        ...
        required_observation_order: draft.required_observation_order.clone(),
        required_modality_order: draft.required_modality_order.clone(),
        passed_observation_order: passed_order,
        pending_observation_order: pending_order,
        quarantined_observation_order: quarantined_order,
...
mod tests {
    use super::*;
    fn hash() -> ContentHash {
        ContentHash::of_bytes(b"cli-quality-workflow")
    }
    fn observation(id: &str, state: EvidenceState, modality: &str) -> QualityObservation {
        QualityObservation {
            observation_id: id.into(),
            study_id: "study-a".into(),
            modality: modality.into(),
            semantic_profile: "qc:v1".into(),
            metric: "signal-to-noise".into(),
            observed_milli: 900,
            threshold_milli: 700,
            baseline_milli: Some(800),
    ...
    fn draft() -> QualityWorkflowDraft {
        QualityWorkflowDraft {
            request_id: "request:cli-quality".into(),
            run_id: "run:cli-quality".into(),
            scope: "organoid-qc".into(),
            semantic_profile: "qc:v1".into(),
            required_observation_order: vec!["obs-a".into(), "obs-b".into()],
            required_modality_order: vec!["imaging".into(), "omics".into()],
            observations: vec![
                observation("obs-a", EvidenceState::Supported, "imaging"),
                observation("obs-b", EvidenceState::Proven, "omics"),
            ],
            stage_order: STAGES.iter().map(|value| (*value).to_string()).collect(),
            replay_identity: hash(),
            policy_allow: true,
    ...
    #[test]
    fn qualified_qc_retains_receipt() {
        let run = assure(&draft()).unwrap();
        assert_eq!(run.disposition, "qualified");
        assert_eq!(run.passed_observation_order, vec!["obs-a", "obs-b"]);
    }
    #[test]
    fn unknown_qc_quarantines() {
        let mut value = draft();
        value.observations[0].evidence_state = EvidenceState::Unknown;
        let run = assure(&value).unwrap();
        assert_eq!(run.disposition, "quarantine");
        assert!(run.pending_observation_order.contains(&"obs-a".into()));
    }
    #[test]
    fn missing_modality_is_quarantine() {
        let mut value = draft();
        value.observations.pop();
        let run = assure(&value).unwrap();
        assert_eq!(run.disposition, "quarantine");
        assert!(run.missing_modality_order.contains(&"omics".into()));
    }
    #[test]
    fn contradiction_quarantines_and_retains_negative() {
        let mut value = draft();
