...
pub const CONTRACT_VERSION: &str =
    "cli-federated-continual-interpretation-visualization-interoperability-gateway/1.0";
    ...
    pub fn validate(&self) -> Result<(), InterpretationGatewayError> {
        ...
        if partitions.iter().any(|id| !panels.contains(id))
            ...
            || self
                .missing_panel_order
                .iter()
                .any(|id| panels.contains(id))
            || self.ranked_order.iter().collect::<BTreeSet<_>>() != panels
        {
            return Err(invalid(
                "interpretation panel states do not partition observed panels",
            ));
        }
        for value in [
        ...
        self.artifact
            .validate_metadata()
            .map_err(|error| InterpretationGatewayError::Artifact(error.to_string()))?;
        ...
        if self.disposition == "qualified" {
            if self.effect_receipts.len() != 1
                || !self.effect_receipts[0].starts_with("exchange:interpretation-artifact:")
            {
                return Err(invalid(
                    "qualified interpretation exchange effect is invalid",
                ));
            }
        } else if self.effect_receipts != ["block:unsafe-release"] {
            return Err(invalid(
                "non-qualified interpretation exchange must block release",
            ));
        }
        Ok(())
    }
...
pub fn assure_interpretation_exchange(
    ...
    let ranked_order = panels
        .iter()
        .map(|panel| panel.panel_id.clone())
        .collect::<Vec<_>>();
    let mut panel_order = ranked_order.clone();
    panel_order.sort();
    let panel_map = panels
        .iter()
    ...
    let required = request
        ...
        .cloned()
        .collect::<BTreeSet<_>>();
    let missing_panel_order = required
        .iter()
        .filter(|id| !panel_map.contains_key(*id))
        .cloned()
        .collect::<Vec<_>>();
    let studies = request
    ...
    let modalities = request
        ...
        .cloned()
        .collect::<BTreeSet<_>>();
    let mut exchanged = BTreeSet::new();
    ...
    for panel in &panels {
        if panel.negative_result {
            negative.insert(format!("{}:negative-result", panel.panel_id));
        }
        omissions.extend(
            panel
                .omissions
                .iter()
                .map(|item| format!("{}:{item}", panel.panel_id)),
        );
        uncertainty.extend(
            panel
                .uncertainty
        ...
        if panel.evidence_state == EvidenceState::Contradicted {
            blocked.insert(panel.panel_id.clone());
            contradiction.insert(format!("{}:contradicted-evidence", panel.panel_id));
            continue;
        }
        if matches!(
            panel.evidence_state,
            EvidenceState::Unknown | EvidenceState::Speculative
        ) {
            unresolved.insert(panel.panel_id.clone());
            uncertainty.insert(format!("{}:evidence-unresolved", panel.panel_id));
            continue;
        }
        let panel_studies = panel.study_order.iter().cloned().collect::<BTreeSet<_>>();
        let panel_modalities = panel
            .modality_order
            .iter()
            .cloned()
            .collect::<BTreeSet<_>>();
        let complete = panel.label.trim() != ""
            && panel.semantic_profile == request.semantic_profile
            && panel.replay_identity == request.replay_identity
            && panel.source_digest_valid()
            && panel.provenance_digest.is_some()
            && studies.is_subset(&panel_studies)
            && modalities.is_subset(&panel_modalities)
            && panel.comparable
            && panel.omissions.is_empty()
            && panel.uncertainty.is_empty()
            && panel.local_data
            && panel.permitted
            && panel.interpretation_score_milli >= 600;
        if complete
            && matches!(
                panel.evidence_state,
        ...
        {
            exchanged.insert(panel.panel_id.clone());
        } else {
            unresolved.insert(panel.panel_id.clone());
            if panel.provenance_digest.is_none() {
                omissions.insert(format!("{}:provenance-missing", panel.panel_id));
            }
            if !studies.is_subset(&panel_studies) {
                omissions.insert(format!(
                    "{}:required-study-coverage-incomplete",
                    panel.panel_id
                ));
            }
            if !modalities.is_subset(&panel_modalities) {
                omissions.insert(format!(
                    "{}:required-modality-coverage-incomplete",
                    panel.panel_id
                ));
            }
            if panel.interpretation_score_milli < 600 {
                uncertainty.insert(format!(
                    "{}:interpretation-threshold-not-met",
                    panel.panel_id
                ));
            }
            if !panel.local_data || !panel.permitted {
                blocked.insert(panel.panel_id.clone());
                unresolved.remove(&panel.panel_id);
                omissions.insert(format!("{}:locality-or-permission-denied", panel.panel_id));
            }
        }
    }
    for id in &missing_panel_order {
        omissions.insert(format!("{id}:required-panel-missing"));
    }
    let missing_study_order = request
        .required_study_order
        .iter()
        .filter(|study| !panels.iter().any(|panel| panel.study_order.contains(study)))
        .cloned()
        .collect::<Vec<_>>();
    let missing_modality_order = request
        ...
        .filter(|modality| {
            !panels
                .iter()
                .any(|panel| panel.modality_order.contains(modality))
        })
        .cloned()
        .collect::<Vec<_>>();
    ...
    let disposition = if global_block {
        "blocked"
    } else if missing_panel_order.is_empty()
    ...
    let exchanged_order = exchanged.into_iter().collect::<Vec<_>>();
    ...
    let effect_receipts = if disposition == "qualified" {
        vec![format!(
            "exchange:interpretation-artifact:{}",
            request.request_id
        )]
    } else {
    ...
    let artifact = TypedResearchArtifact::from_payload(
        format!("cli-interpretation:{}", request.request_id),
        "application/vnd.aurora.interactive-interpretation+json",
        &payload,
        Vec::new(),
        vec![ProvenanceLink {
            source_id: format!("federation:{}", request.federation_id),
            relation: "derived-from-local-interpretation-manifest".into(),
            digest: request.replay_identity.clone(),
        }],
    )
    .map_err(|error| InterpretationGatewayError::Artifact(error.to_string()))?;
    let envelope = InterpretationGatewayEnvelope {
        schema_version: RESEARCH_CONTRACT_SCHEMA_VERSION.into(),
...
impl InterpretationPanel {
    fn source_digest_valid(&self) -> bool {
        digest(&self.result_digest) && digest(&self.visualization_digest)
    }
}

fn validate_request(
    request: &InterpretationGatewayRequest,
) -> Result<(), InterpretationGatewayError> {
    if request.request_id.trim().is_empty()
        ...
        || request.panels.is_empty()
        || !canonical(&request.required_panel_order)
        ...
        || !request.aggregate_only
    {
        return Err(invalid("interpretation request identity, federation, requirements, budget, locality, or boundary is invalid"));
    }
    let mut seen = BTreeSet::new();
    for panel in &request.panels {
        if panel.panel_id.trim().is_empty()
            || panel.label.trim().is_empty()
            || !seen.insert(panel.panel_id.clone())
            || panel.study_order.is_empty()
            || panel.modality_order.is_empty()
            || !canonical(&panel.study_order)
            || !canonical(&panel.modality_order)
            || panel.interpretation_score_milli > 1000
            || !digest(&panel.result_digest)
            || !digest(&panel.visualization_digest)
            || panel
                .provenance_digest
                .as_ref()
                .is_some_and(|value| !digest(value))
            || !digest(&panel.replay_identity)
            || panel.semantic_profile.trim().is_empty()
            || !canonical(&panel.omissions)
            || !canonical(&panel.uncertainty)
        {
            return Err(invalid(format!(
                "panel {} is malformed or duplicated",
                panel.panel_id
            )));
        }
    }
...
mod tests {
    ...
    fn panel(id: &str, state: EvidenceState, score: u16) -> InterpretationPanel {
        InterpretationPanel {
            panel_id: id.into(),
            ...
            interpretation_score_milli: score,
            result_digest: hash(&format!("result-{id}")),
            visualization_digest: hash(&format!("viz-{id}")),
    ...
    fn request(panels: Vec<InterpretationPanel>) -> InterpretationGatewayRequest {
        InterpretationGatewayRequest {
            request_id: "request-1".into(),
            federation_id: "federation-1".into(),
            source_institution: "site-a".into(),
            target_institution: "site-b".into(),
            purpose: "interpretation-exchange".into(),
            semantic_profile: "preclinical-neural-organoid".into(),
            required_panel_order: vec!["panel-a".into()],
    ...
    #[test]
    fn manifest_is_a2() {
        let m = interpretation_interoperability_manifest();
        assert_eq!(m.autonomy_tier, AutonomyTier::A2);
        m.validate().unwrap();
    }
    #[test]
    fn supported_panel_exchanges() {
        let e = assure_interpretation_exchange(&request(vec![panel(
            "panel-a",
            EvidenceState::Supported,
            900,
        )]))
        .unwrap();
        assert_eq!(e.disposition, "qualified");
        e.validate().unwrap();
    }
    #[test]
    fn missing_modality_unresolved() {
        let mut q = request(vec![panel("panel-a", EvidenceState::Supported, 900)]);
        q.required_modality_order.push("spatial".into());
        q.required_modality_order.sort();
        let e = assure_interpretation_exchange(&q).unwrap();
        assert_eq!(e.disposition, "unresolved");
    }
    #[test]
    fn unknown_and_contradiction_retained() {
        let mut q = request(vec![
            panel("panel-a", EvidenceState::Unknown, 900),
            panel("panel-b", EvidenceState::Contradicted, 900),
        ]);
        q.required_panel_order = vec!["panel-a".into()];
        let e = assure_interpretation_exchange(&q).unwrap();
        assert!(e.unresolved_order.contains(&"panel-a".into()));
        assert!(e.blocked_order.contains(&"panel-b".into()));
    }
    #[test]
    fn adversarial_blocks() {
        let mut q = request(vec![panel("panel-a", EvidenceState::Supported, 900)]);
        q.adversarial_events = vec!["poisoned-panel".into()];
        let e = assure_interpretation_exchange(&q).unwrap();
        assert_eq!(e.effect_receipts, vec!["block:unsafe-release"]);
    }
    #[test]
    fn duplicate_rejected() {
        let q = request(vec![
            panel("panel-a", EvidenceState::Supported, 900),
            panel("panel-a", EvidenceState::Supported, 800),
        ]);
        assert!(matches!(
            assure_interpretation_exchange(&q),
            Err(InterpretationGatewayError::Invalid(_))
        ));
    }
    #[test]
    fn ranking_deterministic() {
        let a = assure_interpretation_exchange(&request(vec![
            panel("panel-b", EvidenceState::Supported, 700),
            panel("panel-a", EvidenceState::Supported, 900),
        ]))
        .unwrap();
        let b = assure_interpretation_exchange(&request(vec![
            panel("panel-a", EvidenceState::Supported, 900),
            panel("panel-b", EvidenceState::Supported, 700),
        ]))
        .unwrap();
