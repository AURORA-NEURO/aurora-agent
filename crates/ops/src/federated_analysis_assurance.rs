...
impl FederatedAnalysisReceipt {
    pub fn validate(&self) -> Result<(), FederatedAnalysisError> {
        ...
        {
            return Err(FederatedAnalysisError::Invalid(
                "analysis identity, locality, candidates, decisions, or effects are incomplete"
                    .into(),
            ));
        }
        for values in [
        ...
        if self.rank_order.len() != self.analysis_order.len()
            || BTreeSet::from_iter(self.rank_order.iter().cloned())
                != BTreeSet::from_iter(self.analysis_order.iter().cloned())
            || self
                .decisions
                .iter()
                .zip(&self.analysis_order)
                .any(|(decision, id)| &decision.analysis_id != id)
        {
            return Err(FederatedAnalysisError::Invalid(
                "analysis ranking or decisions do not match candidates".into(),
            ));
        }
        ...
        {
            return Err(FederatedAnalysisError::Invalid(
                "analysis dispositions do not partition candidates".into(),
            ));
        }
        if self.effect_receipts.iter().any(|effect| {
            !effect.starts_with("qualify:analysis:") && effect != "block:unsafe-release"
        }) {
            return Err(FederatedAnalysisError::Invalid(
                "analysis effect is outside the release gate".into(),
            ));
        }
        self.artifact
            .validate_metadata()
            .map_err(|error| FederatedAnalysisError::Artifact(error.to_string()))
    }
}
...
pub fn assure(
...
) -> Result<FederatedAnalysisReceipt, FederatedAnalysisError> {
    ...
    {
        return Err(FederatedAnalysisError::Invalid("analysis identity, quorum, capacity, checkpoint, candidates, locality, or boundary is invalid".into()));
    }
    let mut candidates = request.candidates.clone();
    candidates.sort_by(|a, b| a.analysis_id.cmp(&b.analysis_id));
    let analysis_order = candidates
        .iter()
        .map(|candidate| candidate.analysis_id.clone())
        .collect::<Vec<_>>();
    if analysis_order.iter().any(|id| id.trim().is_empty())
        || analysis_order
            .windows(2)
            .any(|window| window[0] == window[1])
    {
        return Err(FederatedAnalysisError::Invalid(
            "analysis identifiers must be unique and non-empty".into(),
        ));
    }
    let origins = candidates
        ...
        .collect::<BTreeSet<_>>();
    if origins.len() < request.required_origin_quorum as usize
        || origins.iter().any(|origin| origin.trim().is_empty())
    {
        return Err(FederatedAnalysisError::Invalid(
            "declared analysis origin quorum is not available".into(),
        ));
    }
    ...
    for candidate in &candidates {
        ...
        if candidate.independent_site_count < candidate.required_site_quorum {
            pending.insert("independent-site-quorum".into());
            omissions.insert(format!(
                "{}:sites={}/{}",
                id, candidate.independent_site_count, candidate.required_site_quorum
            ));
        }
        let score = candidate.baseline_delta_milli - candidate.uncertainty_width_milli as i64
            + candidate.independent_site_count.min(20) as i64 * 100
            + if candidate.evidence_state == EvidenceState::Proven {
                20000
            } else if candidate.evidence_state == EvidenceState::Supported {
                10000
            } else {
                0
            };
        ...
        let disposition = if !failed.is_empty() {
            ...
            qualified.push(id.clone());
            "qualified"
        };
        if disposition == "blocked" {
            semantic_loss.push(SemanticLoss {
                field: format!("analysis:{}", id),
                reason: "analysis attestation failed one or more release gates".into(),
    ...
    let mut rank_order = analysis_order.clone();
    rank_order.sort_by(|a, b| scores[b].cmp(&scores[a]).then_with(|| a.cmp(b)));
    let admission = if !global_failed.is_empty() || !blocked.is_empty() {
        AnalysisAdmission::Blocked
    } else if !unresolved.is_empty() {
        AnalysisAdmission::Unresolved
    } else if qualified.is_empty() {
        AnalysisAdmission::Blocked
    } else {
        AnalysisAdmission::Qualified
    };
    let checkpoint_digest = ContentHash::of_value(&json!({"federation_id": request.federation_id, "checkpoint_seq": request.checkpoint_seq, "analysis_order": analysis_order, "origin_order": origins})).map_err(|error| FederatedAnalysisError::Artifact(error.to_string()))?;
    let control_digest = ContentHash::of_value(&json!({"admission": admission, "rank_order": rank_order, "decisions": decisions, "semantic_loss": semantic_loss})).map_err(|error| FederatedAnalysisError::Artifact(error.to_string()))?;
    let payload = json!({"schema_version": RESEARCH_CONTRACT_SCHEMA_VERSION, "contract_version": CONTRACT_VERSION, "feature_id": FEATURE_ID, "request_id": request.request_id, "federation_id": request.federation_id, "purpose": request.purpose, "semantic_profile": request.semantic_profile, "admission": admission, "analysis_order": analysis_order, "rank_order": rank_order, "decisions": decisions, "checkpoint_digest": checkpoint_digest, "control_digest": control_digest, "replay_identity": request.replay_identity, "boundary": PRECLINICAL_BOUNDARY});
    let artifact = TypedResearchArtifact::from_payload(
        format!("federated-analysis-assurance:{}", request.request_id),
        "application/vnd.aurora.qualified-analysis-result+json",
        &payload,
        semantic_loss.clone(),
        vec![ProvenanceLink {
            source_id: request.federation_id.clone(),
            relation: "federated-analysis-assurance".into(),
            digest: control_digest.clone(),
        }],
    )
    .map_err(|error| FederatedAnalysisError::Artifact(error.to_string()))?;
    let effect_receipts = if admission == AnalysisAdmission::Qualified {
        vec![format!("qualify:analysis:{}", request.federation_id)]
    } else {
...
mod tests {
    use super::*;
    fn hash() -> ContentHash {
        ContentHash::of_bytes(b"federated-analysis")
    }
    fn candidate(id: &str, origin: &str, state: EvidenceState) -> AnalysisCandidate {
        AnalysisCandidate {
            analysis_id: id.into(),
            origin: origin.into(),
            ...
            baseline_delta_milli: 5000,
            uncertainty_width_milli: 100,
            independent_site_count: 3,
            required_site_quorum: 2,
            policy_allow: true,
    ...
    fn request() -> FederatedAnalysisRequest {
        FederatedAnalysisRequest {
            ...
            checkpoint_seq: 1,
            candidates: vec![
                candidate("a1", "site-a", EvidenceState::Supported),
                candidate("a2", "site-b", EvidenceState::Proven),
            ],
            policy_allow: true,
            protected_closure: true,
            signed_approval: true,
    ...
    #[test]
    fn manifest_is_a1_and_operator_facing() {
    ...
    fn qualified_analysis_is_deterministic() {
        ...
        assert_eq!(receipt.admission, AnalysisAdmission::Qualified);
        assert_eq!(receipt.rank_order, vec!["a2", "a1"]);
    }
    #[test]
    fn unknown_or_site_gap_is_unresolved() {
        let mut value = request();
        value.candidates[0].evidence_state = EvidenceState::Unknown;
        value.candidates[0].independent_site_count = 1;
        let receipt = assure(&value).unwrap();
        assert_eq!(receipt.admission, AnalysisAdmission::Unresolved);
    }
    #[test]
    fn contradiction_and_policy_block() {
        ...
        let receipt = assure(&value).unwrap();
        assert_eq!(receipt.admission, AnalysisAdmission::Blocked);
        assert!(receipt
            .negative_evidence
            .iter()
    ...
    #[test]
    fn locality_and_approval_fail_closed() {
        let mut value = request();
