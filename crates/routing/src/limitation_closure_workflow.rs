//! Prospective high-throughput limitation-closure workflow fabric.
//!
//! Atlas feature: `AFA-routing-P26-F15`.  The fabric turns caller-declared limitations and closure
...
pub const FEATURE_ID: &str = "AFA-routing-P26-F15";
pub const CONTRACT_VERSION: &str = "routing-prospective-limitation-closure-workflow-fabric/1.0";
pub const INPUT_SCHEMA: &str = "LimitationClosureWorkflowRequest5@1";
pub const OUTPUT_SCHEMA: &str = "LimitationClosureWorkflowReceipt7@1";
pub const CONTENT_TYPE: &str = "application/vnd.aurora.routing-limitation-closure-receipt-7+json";
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct LimitationClosureWorkflowRequest5 {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct LimitationClosureWorkflowReceipt7 {
    pub schema_version: String,
...
fn invalid(message: impl Into<String>) -> LimitationClosureError {
    LimitationClosureError::Invalid(message.into())
}
...
impl LimitationClosureWorkflowReceipt7 {
    pub fn validate(&self) -> Result<(), LimitationClosureError> {
        ...
        self.artifact
            .validate_metadata()
            .map_err(|e| LimitationClosureError::Artifact(e.to_string()))
    }
...
pub fn compile_limitation_closure_workflow(
    q: &LimitationClosureWorkflowRequest5,
) -> Result<LimitationClosureWorkflowReceipt7, LimitationClosureError> {
    validate_request(q)?;
    ...
    let receipt = LimitationClosureWorkflowReceipt7 {
...
mod tests {
    ...
    fn q(items: Vec<Limitation6>) -> LimitationClosureWorkflowRequest5 {
    ...
    fn qualified() {
        assert_eq!(
            compile_limitation_closure_workflow(&q(vec![lim("lim:1", EvidenceState::Supported)]))
                .unwrap()
                .disposition,
            LimitationClosureDisposition::Qualified
        )
    ...
    fn unknown() {
        assert_eq!(
            compile_limitation_closure_workflow(&q(vec![lim("lim:1", EvidenceState::Unknown)]))
                .unwrap()
                .disposition,
            LimitationClosureDisposition::Unresolved
        )
    ...
    fn contradiction() {
        assert_eq!(
            compile_limitation_closure_workflow(&q(vec![lim(
                "lim:1",
    ...
    fn missing() {
        assert_eq!(
            compile_limitation_closure_workflow(&q(vec![lim("other", EvidenceState::Supported)]))
                .unwrap()
                .disposition,
            LimitationClosureDisposition::Blocked
        )
    ...
    fn negative() {
        ...
        assert_eq!(
            compile_limitation_closure_workflow(&q(vec![x]))
                .unwrap()
    ...
    #[test]
    fn manifest() {
        limitation_closure_workflow_manifest().validate().unwrap()
    }
}
