...
fn standard_adapters() -> Vec<MissionEvaluatorAdapter> {
    vec![
        adapter("world.observation_integrity", "world_and_ingestion", &["world state", "ingestion", "observations"], &["observation", "evaluation"], "Compare declared world observations with retained source and acquisition metadata.", &["fiber_compile", "observed_world_declare", "source_provenance_audit"], &["/ok", "/valid", "/digest"]),
        adapter("decision.context_coverage", "decision_context", &["decision context", "evidence coverage", "information acquisition"], &["observation", "evaluation"], "Inspect whether a decision context names required evidence dimensions and unresolved acquisition obligations.", &["context_bundle", "epistemic_value", "decision_cell_audit"], &["/ok", "/coverage", "/obligations"]),
        ...
        adapter("benchmark.portfolio_coverage", "benchmark_pack_portfolio", &["benchmark", "packs", "coverage"], &["evaluation", "release"], "Inspect benchmark portfolio coverage, difficulty, and missingness declarations.", &["benchmark_pack_audit", "atlas_surface_audit", "pack_manifest_check"], &["/coverage", "/missing", "/digest"]),
        ...
        adapter("mutation.causal_identification", "mutation_and_causal_discovery", &["mutation", "causal discovery", "intervention"], &["evaluation", "observation"], "Inspect whether mutation or intervention evidence is separated from causal interpretation and holdout status.", &["mutation_audit", "causal_discovery", "lab_holdout_audit"], &["/causal", "/holdout", "/evidence"]),
        adapter("bioeval.reference_contract", "bioevaluation_reference_contracts", &["bioevaluation", "reference standard", "metamorphic testing"], &["evaluation", "release"], "Inspect reference, metamorphic, and re-execution contracts for bounded biological evaluation.", &["bioeval_reference_audit", "bioeval_metamorphic_run", "evaluation_reproduction_check"], &["/reference", "/relations", "/reexecution"]),
        adapter("biology.observation_consistency", "biological_domains", &["biology", "biological observation", "assay"], &["observation", "evaluation"], "Compare biological observations across declared modalities without inferring clinical meaning.", &["biocapability_evidence_audit", "biological_claim_check", "modality_compare"], &["/evidence", "/dimensions", "/status"]),
        adapter("biolang.query_contract", "biological_ir_and_query", &["biological IR", "BioQL", "query"], &["observation", "evaluation"], "Inspect query schema, typed projection, and source binding closure for a biological IR request.", &["biolang_compile", "biolang_query_check", "biolang_schema_audit"], &["/schema", "/bindings", "/errors"]),
        adapter("foundation.schema_integrity", "foundation_contracts", &["foundation", "schema", "determinism"], &["observation", "release"], "Inspect version, digest, determinism, and refusal fields at a foundational contract boundary.", &["governance_schema_check", "schema_validate", "contract_audit"], &["/schema", "/digest", "/valid"]),
        ...
        adapter("oncoworlds.assay_fidelity", "oncoworlds_models_and_assays", &["oncology models", "assays", "fidelity"], &["evaluation", "observation"], "Compare model and assay fidelity axes with their declared evidence and split conditions.", &["onco_model_audit", "assay_fidelity_check", "onco_response_assess"], &["/fidelity", "/evidence", "/split"]),
        adapter("oncoworlds.clonal_history", "oncoworlds_clonal_evolution", &["clonal evolution", "lineage", "tumour population"], &["observation", "evaluation"], "Inspect clonal history continuity, sampling limits, and unresolved lineage transitions.", &["clonal_history_audit", "lineage_audit", "worldline_view"], &["/lineage", "/transitions", "/missing"]),
        adapter("oncoworlds.shift_equity", "oncoworlds_shift_and_equity", &["distribution shift", "equity", "cohort"], &["evaluation", "operational"], "Inspect shift strata, cohort representation, and transport caveats without converting them into a fairness conclusion.", &["shift_audit", "representation_audit", "cohort_comparability_check"], &["/strata", "/coverage", "/transport"]),
        ...
        adapter("influence.abstract_soundness", "influence_bounds_and_abstract_analysis", &["influence bounds", "abstract analysis", "perturbation"], &["evaluation", "release"], "Inspect declared soundness bounds and omitted regions in an abstract influence analysis.", &["influence_analyze", "abstract_bound_check", "perturbation_audit"], &["/bounds", "/omissions", "/sound"]),
...
mod tests {
    ...
    fn search_is_conjunctive_digest_bound_and_stable() {
        ...
        assert!(first.matches[0]
            .adapter
            .candidate_tools
            .contains(&"assay_fidelity_check".into()));
    }
