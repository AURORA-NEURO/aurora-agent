...
pub fn verify_mission_evidence_bundle(bundle: &Value) -> Result<Value, EvidenceBundleError> {
    ...
    without_digest
        .as_object_mut()
        .expect("bundle object was checked above")
    ...
    let trace_included = export
        .get("trace_included")
        .and_then(Value::as_bool)
        .unwrap_or(export_include_trace);
    let trace_contract_valid = trace_included == export_include_trace
        && match object.get("trace") {
            Some(Value::Null) => !trace_included,
            Some(Value::Array(rows)) => {
                rows.iter().all(Value::is_object) && (trace_included || rows.is_empty())
            }
            _ => false,
        };
