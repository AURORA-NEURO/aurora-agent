//! Federated continual context-compilation researcher workbench.
//!
//! Atlas feature: `AFA-bioworlds-P03-F20`. Only permitted aggregate artifacts may
...
pub enum FederatedContextWorkbenchVerdict {
    Qualified,
    Unresolved,
    Blocked,
}
...
pub struct FederatedContextWorkbenchReceipt {
    ...
    pub goal: String,
    pub semantic_profile: String,
    pub verdict: FederatedContextWorkbenchVerdict,
    pub institution_order: Vec<String>,
...
pub fn federated_continual_context_research_workbench_manifest() -> CapabilityManifest {
    CapabilityManifest {
        schema_version: RESEARCH_CONTRACT_SCHEMA_VERSION.into(),
        capability_id: FEATURE_ID.into(),
        version: CONTRACT_VERSION.into(),
        owner_crate: "bioworlds".into(),
        consumers: [
        ...
        .into(),
        behavior: "renders an omission-aware federated continual DecisionQuery workbench and certifies only semantically aligned, fresh, provenance-complete, replayable, permitted aggregate context".into(),
        value: "gives preclinical researchers a typed CertifiedDecisionSection with visible omissions and denial evidence while preserving institution-local raw data".into(),
        inputs: vec![TypedPort {
            name: "decision_query".into(),
            schema: "DecisionQuery4@1".into(),
            required: true,
        }],
        outputs: vec![TypedPort {
            name: "certified_decision_section".into(),
            schema: "CertifiedDecisionSection5@1".into(),
            required: true,
        }],
        effects: [
        ...
        .into(),
        permissions: ["view:federated-context-workbench".into()].into(),
...
pub fn compile_federated_continual_context_workbench(
    ...
    let verdict = if !global_open || !blocked.is_empty() {
        FederatedContextWorkbenchVerdict::Blocked
    } else if !unknown.is_empty() || qualified.len() < request.minimum_quorum as usize {
        FederatedContextWorkbenchVerdict::Unresolved
    } else {
        FederatedContextWorkbenchVerdict::Qualified
    };
    ...
    let artifact = TypedResearchArtifact::from_payload(
        format!(
            "bioworlds-federated-continual-context-research-workbench:{}",
            request.request_id
        ),
        CONTENT_TYPE,
        &payload,
    ...
    let receipt = FederatedContextWorkbenchReceipt {
        ...
        effect_receipts: if matches!(verdict, FederatedContextWorkbenchVerdict::Qualified) {
            vec![format!(
                "view:federated-context-workbench:{}",
                request.request_id
            )]
        } else {
...
mod tests {
    ...
    fn request() -> FederatedContextWorkbenchRequest {
        ...
        let peer = |institution: &str| FederatedContextWorkbenchPeer {
    ...
    fn manifest_is_a1() {
        assert_eq!(
            federated_continual_context_research_workbench_manifest().autonomy_tier,
            AutonomyTier::A1
        );
    }
    #[test]
    fn complete_is_qualified() {
        assert_eq!(
            compile_federated_continual_context_workbench(&request())
                .unwrap()
                .verdict,
            FederatedContextWorkbenchVerdict::Qualified
        );
    ...
    fn missing_peer_is_unresolved() {
        ...
        value.peers.pop();
        assert_eq!(
            compile_federated_continual_context_workbench(&value)
                .unwrap()
                .verdict,
            FederatedContextWorkbenchVerdict::Unresolved
        );
    ...
    #[test]
    fn digest_is_stable() {
        let receipt = compile_federated_continual_context_workbench(&request()).unwrap();
