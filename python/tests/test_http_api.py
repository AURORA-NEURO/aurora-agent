...
class FakeApiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path == "/healthz":
            ...
            self._send(200, {"ok": True, "schema": "bioprism-devplat-cross-domain-artifact-audit/0.1", "workflow": "artifact_registry_cross_store_audit", "consistent": True, "bounded": True, "truncated": False, "stores": {}, "coverage": {}, "artifact_kind_counts": {"domain_report": 1}, "findings": [], "execution": "not_started", "guarantees": [], "does_not_claim": ["the three stores were read in one atomic transaction"]})
        ...
        elif self.path.startswith("/v1/operations/gate-reviews"):
            review_id = "e" * 64
            acceptance = {"gate_digest": "d" * 64, "reviewer": "operator", "rationale": "reviewed", "group_ids": ["biological_domains"], "accepted_gates": {"biological_domains": ["catalogue", "observed_activity", "transport_completion", "evaluation_evidence", "safety_evidence", "release_evidence"]}, "review_id": review_id}
            review = {"review_id": review_id, "event_id": 4, "request_id": "review-1", "acceptance": acceptance, "gate_digest": "d" * 64, "group_ids": ["biological_domains"], "evidence": [], "replay": f"/v1/operations/gate-reviews?review_id={review_id}", "readiness_claimed": False}
            self._send(200, {"ok": True, "workflow": "operations_gate_reviews", "schema": "bioprism-operations-gate-reviews/0.1", "review_id": review_id, "found": True, "page": {"events": [{"id": 4, "event_type": "operations.gate_review.accepted", "subject": "operations_gate_review", "request_id": "review-1", "payload": {"review_id": review_id}}], "after": 0, "next_after": 4, "oldest": 1, "newest": 4, "gap": False, "dropped_events": 0}, "reviews": [review], "review_count": 1, "readiness_claimed": False, "guarantees": ["durable"], "non_claims": ["approval"]})
        elif self.path.startswith("/v1/operations/gates"):
            self._send(200, {"ok": True, "workflow": "operations_domain_gates", "schema": "bioprism-operations-domain-gates/0.1", "gate_digest": "d" * 64, "gate_digest_scope": "tool_evidence_projection_without_gate_digest", "event_cursor": {"after": 0, "next_after": 1, "oldest": 1, "newest": 1, "gap": False, "dropped_events": 0, "returned_events": 1}, "groups": [{"id": "biological_domains", "status": "available", "domains": ["oncology"], "declared_tool_count": 1, "advertised_tool_count": 1, "missing_tool_count": 0, "missing_tools": [], "fully_advertised": True, "gate_state": "insufficient_evidence", "readiness_claimed": False, "gates": {"catalogue": {"state": "pass", "missing_tool_count": 0}, "observed_activity": {"state": "observed", "event_count": 1, "tool_count": 1, "tools": ["modality_catalog"]}, "transport_completion": {"state": "observed", "event_count": 1, "tool_count": 1, "tools": ["modality_catalog"], "refused_event_count": 0, "refused_tool_count": 0, "refused_tools": []}, "evaluation_evidence": {"state": "missing", "event_count": 0, "tool_count": 0, "tools": []}, "safety_evidence": {"state": "missing", "event_count": 0, "tool_count": 0, "tools": []}, "release_evidence": {"state": "missing", "event_count": 0, "tool_count": 0, "tools": []}}, "last_event_id": 1, "evidence_scope": "requested_event_page_only"}], "summary": {"group_count": 1, "returned_groups": 1, "tool_events_scanned": 1, "attributed_tool_events": 1, "unattributed_tool_events": 0, "completed_tool_events": 1, "refused_tool_events": 0, "evaluation_evidence_events": 0, "safety_evidence_events": 0, "release_evidence_events": 0, "groups_blocked_catalogue": 0, "groups_insufficient_evidence": 1, "groups_review_required": 0, "readiness_claimed": False}, "gate_policy": {"required_gates": ["catalogue", "observed_activity", "transport_completion", "evaluation_evidence", "safety_evidence", "release_evidence"], "decision_rule": "all gates need observed evidence", "readiness_claimed": False}, "guarantees": ["separate evidence channels"], "non_claims": ["scientific validity"], "links": {"operations_domains": "/v1/operations/domains"}})
...
class HttpApiClientTests(unittest.TestCase):
    ...
    def test_http_health_tools_events_and_structured_errors(self) -> None:
        ...
        claims = client.mission_claim_lineage("async-1")
        self.assertIsInstance(claims, MissionClaimLineage)
        self.assertTrue(claims.claim_lineage["claims"][0]["claimable"])
        with self.assertRaises(ArgumentError):
        ...
        provider_request = CiProviderNormalizationRequest(
            {"workflow": "contracts"},
            "github_actions",
            {"run": {"id": 9001, "conclusion": "success"}, "jobs": [{"name": "tests", "conclusion": "success"}]},
        )
        provider_result = client.ci_provider_normalize(provider_request)
        self.assertEqual(
            provider_result["mcp"]["result"]["provider"],
            "github_actions",
        )
        self.assertEqual(
        ...
        receipt_result = client.developer_delivery_receipt(receipt_request)
        self.assertEqual(
            receipt_result["mcp"]["result"]["receipt_id"],
            "receipt-http-1",
        )
        self.assertEqual(
            receipt_result["mcp"]["result"]["delivery"]["release_request"]["id"],
            "delivery-http-1",
        )
        verified_result = client.developer_delivery_receipt_verify(
            DeveloperDeliveryReceiptVerificationRequest(
                {"receipt_id": "receipt-http-1", "receipt_digest": "a" * 64},
                {"workflow": "developer_delivery_audit"},
            )
        )
        self.assertEqual(
            verified_result["mcp"]["result"]["receipt"]["receipt_id"],
            "receipt-http-1",
        )
        provenance = client.execution_provenance_audit(
            ExecutionProvenanceRequest({"plan": {"mission_id": "mission-http"}})
        )
        self.assertEqual(
            provenance["mcp"]["result"]["mission"]["plan"]["mission_id"],
            "mission-http",
        )
        self.assertEqual(
        ...
        review = client.create_operations_gate_review(
            OperationsGateReviewRequest(
                ...
                "reviewed",
                ("biological_domains",),
                {"biological_domains": ("catalogue", "observed_activity", "transport_completion", "evaluation_evidence", "safety_evidence", "release_evidence")},
            )
        )
        self.assertEqual(len(review.review_id), 64)
        reviews = client.operations_gate_reviews(review_id=review.review_id)
        self.assertIsInstance(reviews, OperationsGateReviews)
        self.assertEqual(reviews.reviews[0].review_id, review.review_id)
        ...
        self.assertEqual(handoff.groups[0].route_need_id, "domain-group:biological_domains")
        self.assertTrue(handoff.execution_prerequisites["required"])
    ...
    def test_http_arguments_and_async_facade(self) -> None:
        ...
        async def run() -> None:
            ...
            self.assertEqual(trace.events[-1].event, "mission.completed")
            claims = await client.mission_claim_lineage("async-1")
            self.assertTrue(claims.claim_lineage["claims"][0]["claimable"])
            ...
            gates = await client.operations_domain_gates(limit=2)
            self.assertEqual(gates.summary["groups_insufficient_evidence"], 1)
            reviews = await client.operations_gate_reviews(review_id="e" * 64)
