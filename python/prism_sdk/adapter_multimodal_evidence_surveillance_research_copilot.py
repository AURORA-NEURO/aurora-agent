...
class MultimodalEvidenceSurveillanceResearchCopilotReceipt:
    ...
    feature_id: str = ADAPTER_MULTIMODAL_EVIDENCE_SURVEILLANCE_RESEARCH_COPILOT_FEATURE_ID
    contract_version: str = ADAPTER_MULTIMODAL_EVIDENCE_SURVEILLANCE_RESEARCH_COPILOT_CONTRACT_VERSION
    schema_version: str = RESEARCH_CONTRACT_SCHEMA_VERSION
    raw_data_local: bool = True
    boundary: str = PRECLINICAL_BOUNDARY

    def validate(self) -> None:
        if self.schema_version != RESEARCH_CONTRACT_SCHEMA_VERSION or self.contract_version != ADAPTER_MULTIMODAL_EVIDENCE_SURVEILLANCE_RESEARCH_COPILOT_CONTRACT_VERSION or self.feature_id != ADAPTER_MULTIMODAL_EVIDENCE_SURVEILLANCE_RESEARCH_COPILOT_FEATURE_ID:
...
def run_multimodal_evidence_surveillance_research_copilot(*, request_id: str, agent_id: str, semantic_profile: str, required_studies: Sequence[str], required_modalities: Sequence[str], declared_tools: Sequence[str], requested_tool: str, max_tool_calls: int, dry_run: bool, approval_reference: str | None, approval_granted: bool, observations: Sequence[MultimodalCopilotEvidenceObservation], min_relevance_score: int = 0, policy_allow: bool = True, protected_closure: bool = True, raw_data_local: bool = True, replay_identity: str) -> MultimodalEvidenceSurveillanceResearchCopilotReceipt:
    ...
    qualified_set = {"schema_version": RESEARCH_CONTRACT_SCHEMA_VERSION, "set_id": f"qualified-evidence-multimodal-copilot:{request_id}", "semantic_profile": semantic_profile, "study_order": list(study_order), "modality_order": list(modality_order), "selected_order": list(selected_order), "selected_digests": [digest_map[source] for source in selected_order], "incomparable_order": list(incomparable_order), "missing_cell_order": list(missing_order), "negative_order": list(negative_order), "omissions": list(omissions_order), "uncertainty": list(uncertainty_order), "evidence_state": "supported" if disposition == "completed" else "unknown", "tool_mode": "dry_run" if dry_run else "bounded_invocation", "boundary": PRECLINICAL_BOUNDARY}; artifact = {"content_hash": research_artifact_digest(qualified_set), "media_type": "application/vnd.aurora.qualified-evidence-set3+json"}
