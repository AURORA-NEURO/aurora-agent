"""Python parity surface for AFA-adapter-P02-F06.

The engine is deliberately local and deterministic.  It ranks typed candidate
metadata for multiple imaging and omics studies, never moves raw source payloads, and emits
...
from .research_contracts import (
    ADAPTER_MULTIMODAL_RETRIEVAL_SYNTHESIS_CONTRACT_MODEL_CONTRACT_VERSION,
...
FEATURE_ID = ADAPTER_MULTIMODAL_RETRIEVAL_SYNTHESIS_CONTRACT_MODEL_FEATURE_ID
CONTRACT_VERSION = ADAPTER_MULTIMODAL_RETRIEVAL_SYNTHESIS_CONTRACT_MODEL_CONTRACT_VERSION
INPUT_SCHEMA = "ScopedRetrievalQuery@1"
OUTPUT_SCHEMA = "EvidenceSynthesis1@1"


@dataclass(frozen=True)
class MultimodalRetrievalSynthesisCandidate:
    evidence_id: str
    study_id: str
    modality: str
...
@dataclass(frozen=True)
class MultimodalRetrievalSynthesisContractModelReceipt:
    request_id: str
    query_id: str
    contract_id: str
    schema_profile: str
    ...
    def validate(self) -> None:
        if (
            ...
            raise ResearchContractError("multimodal retrieval contract model identity, output, locality, candidates, or effects are incomplete")
        for values in (
            ...
            if tuple(sorted(set(values))) != values:
                raise ResearchContractError("multimodal retrieval contract model ordering is not canonical")
        if set(self.selected_order) | set(self.omitted_order) != set(self.candidate_order):
            raise ResearchContractError("multimodal retrieval contract model states do not partition candidates")
        for value in (
            self.replay_identity, self.synthesis_digest, self.contract_digest,
            self.artifact.get("content_hash"),
        ):
            if not isinstance(value, str) or not re.fullmatch(r"[0-9a-f]{64}", value):
                raise ResearchContractError("multimodal retrieval contract model digest is invalid")
        if any(
            not effect.startswith("compute:multimodal-retrieval-contract-model:")
            and effect != "block:unsafe-release"
            for effect in self.effect_receipts
        ):
            raise ResearchContractError("multimodal retrieval contract model effect is outside local computation gate")


def run_multimodal_retrieval_synthesis_contract_model(
    *,
    request_id: str,
    query_id: str,
    requester: str,
    ...
    comparability_profile: str,
    max_results: int,
    candidates: Sequence[MultimodalRetrievalSynthesisCandidate],
    contract_id: str,
    algorithm_version: str,
    ...
    if (
        ...
        or not intent.strip() or len(study_ids) < 2 or len(required_modalities) < 2
    ...
    selected: list[MultimodalRetrievalSynthesisCandidate] = []
    omitted: list[MultimodalRetrievalSynthesisCandidate] = []
    ...
    receipt = MultimodalRetrievalSynthesisContractModelReceipt(
        request_id=request_id, query_id=query_id, contract_id=contract_id,
        ...
        effect_receipts=(f"compute:multimodal-retrieval-contract-model:{contract_id}",),
        artifact={"content_hash": research_artifact_digest(payload),
                  "media_type": "application/vnd.aurora.multimodal-retrieval-synthesis-contract-model+json"},
    )
    receipt.validate()
    return receipt
