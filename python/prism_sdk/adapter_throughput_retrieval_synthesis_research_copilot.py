"""Python parity surface for AFA-adapter-P02-F11.
...
FEATURE_ID = ADAPTER_THROUGHPUT_RETRIEVAL_SYNTHESIS_RESEARCH_COPILOT_FEATURE_ID
CONTRACT_VERSION = ADAPTER_THROUGHPUT_RETRIEVAL_SYNTHESIS_RESEARCH_COPILOT_CONTRACT_VERSION
...
@dataclass(frozen=True)
class ThroughputRetrievalSynthesisResearchCopilotReceipt:
    request_id: str
    query_id: str
    copilot_id: str
    agent_id: str
    ...
    def validate(self) -> None:
        if (
            ...
            raise ResearchContractError("throughput retrieval research copilot identity, batch admission, output, locality, candidates, or effects are incomplete")
        for values in (
            ...
            if tuple(sorted(set(values))) != values:
                raise ResearchContractError("throughput retrieval research copilot ordering is not canonical")
        if set(self.selected_order) | set(self.omitted_order) != set(self.candidate_order):
            raise ResearchContractError("throughput retrieval research copilot states do not partition candidates")
        for value in (
            ...
            if not isinstance(value, str) or not re.fullmatch(r"[0-9a-f]{64}", value):
                raise ResearchContractError("throughput retrieval research copilot digest is invalid")
        if any(
            not effect.startswith("compute:throughput-retrieval-research-copilot:")
            and not effect.startswith("approval-required:")
            and effect != "block:unsafe-release"
            for effect in self.effect_receipts
        ):
            raise ResearchContractError("throughput retrieval research copilot effect is outside bounded computation or approval gate")
...
def run_throughput_retrieval_synthesis_research_copilot(
    *,
    request_id: str,
    query_id: str,
    requester: str,
    ...
    checkpoint_digest: str,
    approval_token: str = "",
    agent_id: str = "agent:throughput-retrieval-copilot",
    ...
    receipt = ThroughputRetrievalSynthesisResearchCopilotReceipt(
        request_id=request_id, query_id=query_id, copilot_id=copilot_id,
        agent_id=agent_id, recommendation_mode=recommendation_mode,
        approval_required=approval_required,
        ...
        effect_receipts=(
            ...
            else f"compute:throughput-retrieval-research-copilot:{copilot_id}",
        ),
        artifact={"content_hash": research_artifact_digest(payload),
                  "media_type": "application/vnd.aurora.throughput-retrieval-synthesis-research-copilot+json"},
    )
    receipt.validate()
    return receipt
