"""Workflow-protocol parity for AFA-worldgen-P01-F15."""
...
class ThroughputEvidenceSurveillanceWorkflowReceipt:
    ...
    schema_version: str = RESEARCH_CONTRACT_SCHEMA_VERSION; contract_version: str = WORLDGEN_THROUGHPUT_EVIDENCE_SURVEILLANCE_WORKFLOW_FABRIC_CONTRACT_VERSION; feature_id: str = WORLDGEN_THROUGHPUT_EVIDENCE_SURVEILLANCE_WORKFLOW_FABRIC_FEATURE_ID; raw_data_local: bool = True; boundary: str = PRECLINICAL_BOUNDARY

    def validate(self) -> None:
        if self.schema_version != RESEARCH_CONTRACT_SCHEMA_VERSION or self.contract_version != WORLDGEN_THROUGHPUT_EVIDENCE_SURVEILLANCE_WORKFLOW_FABRIC_CONTRACT_VERSION or self.feature_id != WORLDGEN_THROUGHPUT_EVIDENCE_SURVEILLANCE_WORKFLOW_FABRIC_FEATURE_ID or self.boundary != PRECLINICAL_BOUNDARY or not self.raw_data_local or not self.request_id.strip() or not self.workflow_id.strip() or not self.agent_id.strip() or not self.batch_id.strip() or self.checkpoint_seq <= 0 or self.stage_order != CANONICAL_STAGES or not self.effect_receipts:
...
def schedule_throughput_evidence_surveillance_workflow(*, request_id: str, workflow_id: str, checkpoint_id: str, request: Mapping[str, Any], requested_stage_order: Sequence[str], budget_units: int, replay_identity: str, boundary: str = PRECLINICAL_BOUNDARY) -> ThroughputEvidenceSurveillanceWorkflowReceipt:
    ...
    disposition = copilot.disposition; blocked_order = ("stage:release",) if disposition == "blocked" else (); payload = {"schema_version": RESEARCH_CONTRACT_SCHEMA_VERSION, "contract_version": WORLDGEN_THROUGHPUT_EVIDENCE_SURVEILLANCE_WORKFLOW_FABRIC_CONTRACT_VERSION, "feature_id": WORLDGEN_THROUGHPUT_EVIDENCE_SURVEILLANCE_WORKFLOW_FABRIC_FEATURE_ID, "workflow_id": workflow_id, "batch_id": request["batch_id"], "stage_order": list(CANONICAL_STAGES), "plan_order": list(plan_order), "completed_order": list(completed_order), "compensation_order": list(compensation_order), "candidate_order": list(copilot.candidate_order), "selected_order": list(copilot.selected_order), "unresolved_order": list(copilot.unresolved_order), "denied_order": list(copilot.denied_order), "overflow_order": list(copilot.overflow_order), "replay_identity": replay_identity, "copilot_run_digest": copilot_run_digest, "checkpoint_digest": checkpoint_digest, "workflow_digest": workflow_digest, "omissions": list(omissions_order), "uncertainty": list(copilot.uncertainty), "negative_evidence": list(copilot.negative_evidence), "boundary": PRECLINICAL_BOUNDARY}; artifact = {"schema_version": RESEARCH_CONTRACT_SCHEMA_VERSION, "artifact_id": f"worldgen-throughput-evidence-workflow:{workflow_id}", "content_type": "application/vnd.aurora.worldgen.throughput-research-workflow+json", "content_hash": research_artifact_digest(payload), "semantic_loss": [], "provenance": [], "boundary": PRECLINICAL_BOUNDARY}
