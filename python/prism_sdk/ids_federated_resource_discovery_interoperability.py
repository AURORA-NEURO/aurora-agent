"""Python parity for ``AFA-ids-P05-F24``.
...
FEATURE_ID = "AFA-ids-P05-F24"
CONTRACT_VERSION = "ids-federated-continual-resource-discovery-interoperability/1.0"
INPUT_SCHEMA = "ResourceNeed4@1"
OUTPUT_SCHEMA = "QualifiedResourceSet6@1"
CONTENT_TYPE = "application/vnd.aurora.qualified-resource-set-6+json"
MAX_ENDPOINTS = 4096
MAX_RESULTS = 256
...
class QualifiedResourceSet6:
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            raise ResearchContractError("ids resource interoperability identity, locality, protocol, peers, or effects are incomplete")
        fields = ("endpoint_order", "qualified_order", "unresolved_order", "blocked_order", "missing_capability_order", "peer_order", "qualified_peer_order", "missing_peer_order", "omissions", "uncertainty", "negative_evidence", "migration_notes", "effect_receipts")
        if any(not _canonical(value.get(field, ())) for field in fields):
            raise ResearchContractError("ids resource interoperability ordering is not canonical")
        ...
        if endpoint_order != classified or len(endpoint_order) != len(value["endpoint_order"]):
            raise ResearchContractError("ids resource endpoint dispositions do not partition candidates")
        ...
        if peer_order != qualified_peers | missing_peers or len(qualified_peers) + len(missing_peers) != len(peer_order):
            raise ResearchContractError("ids resource peer dispositions do not partition peers")
        ...
        if len(resources) != len(value["qualified_order"]) or any(row.get("resource_id") != resource_id for row, resource_id in zip(resources, value["qualified_order"])):
            raise ResearchContractError("ids resource qualified rows do not match qualified order")
        ...
        if artifact.get("content_type") != CONTENT_TYPE or artifact.get("boundary") != PRECLINICAL_BOUNDARY or artifact.get("content_hash") != value.get("selection_digest"):
            raise ResearchContractError("ids resource artifact metadata or digest is inconsistent")
        ...
        if not all(_digest(item) for item in digests):
            raise ResearchContractError("ids resource digest is invalid")
        if any(not effect.startswith("exchange:permitted-artifacts:") and effect != "block:unsafe-release" for effect in value["effect_receipts"]):
            raise ResearchContractError("ids resource effect is outside the interoperability gate")


def interoperability_manifest() -> dict[str, Any]:
    return {
        ...
        "effects": ["exchange:permitted-artifacts"],
        "permissions": ["connect:approved-endpoints"],
        "autonomy_tier": "A2",
        "boundary": PRECLINICAL_BOUNDARY,
    }


def interoperate_resources(
    ...
    if (
        ...
        raise ResearchContractError("ids resource request identity, capabilities, bounds, locality, peers, endpoints, replay, or boundary is invalid")
    ...
    if any(not row.get("resource_id") or not row.get("endpoint_id") or not row.get("origin") or not all(_digest(row.get(key)) for key in ("artifact_digest", "provenance_digest", "replay_identity")) for row in endpoint_rows) or len(set(endpoint_ids)) != len(endpoint_ids):
        raise ResearchContractError("ids resource endpoint identity, uniqueness, or digests are invalid")
    ...
    if any(not row.get("peer_id") or not row.get("origin") or not _digest(row.get("summary_digest")) for row in peer_rows) or len(set(peer_ids)) != len(peer_ids):
        raise ResearchContractError("ids resource peer identity, uniqueness, or digest is invalid")
    qualified_peers = set()
    missing_peers = set()
    uncertainty = set()
    ...
    global_block = not all(request.get(key) is True for key in ("policy_allow", "protected_closure", "signed_approval", "federation_approved", "raw_data_local", "aggregate_only"))
