"""Python parity surface for AFA-adapter-P02-F10.
...
FEATURE_ID = ADAPTER_MULTIMODAL_RETRIEVAL_SYNTHESIS_RESEARCH_COPILOT_FEATURE_ID
CONTRACT_VERSION = ADAPTER_MULTIMODAL_RETRIEVAL_SYNTHESIS_RESEARCH_COPILOT_CONTRACT_VERSION
...
@dataclass(frozen=True)
class MultimodalRetrievalSynthesisResearchCopilotReceipt:
    request_id: str
    query_id: str
    copilot_id: str
    agent_id: str
    ...
    def validate(self) -> None:
        if (
            ...
            raise ResearchContractError("local retrieval research copilot identity, output, locality, candidates, or effects are incomplete")
        for values in (
            ...
            if tuple(sorted(set(values))) != values:
                raise ResearchContractError("local retrieval research copilot ordering is not canonical")
        if set(self.selected_order) | set(self.omitted_order) != set(self.candidate_order):
            raise ResearchContractError("local retrieval research copilot states do not partition candidates")
        for value in (
            ...
            if not isinstance(value, str) or not re.fullmatch(r"[0-9a-f]{64}", value):
                raise ResearchContractError("local retrieval research copilot digest is invalid")
        if any(
            not effect.startswith("compute:multimodal-retrieval-research-copilot:")
            and not effect.startswith("approval-required:")
            and effect != "block:unsafe-release"
            for effect in self.effect_receipts
        ):
            raise ResearchContractError("local retrieval research copilot effect is outside local computation gate")


def run_multimodal_retrieval_synthesis_research_copilot(
    *,
    request_id: str,
    query_id: str,
    requester: str,
    ...
    comparability_digest: str,
    approval_token: str = "",
    agent_id: str = "agent:multimodal-retrieval-copilot",
    ...
    disposition = (
        "blocked" if not policy_allow or not protected_closure_satisfied or not approval_token.strip()
    ...
    receipt = MultimodalRetrievalSynthesisResearchCopilotReceipt(
        request_id=request_id, query_id=query_id, copilot_id=copilot_id,
        agent_id=agent_id, recommendation_mode=recommendation_mode,
        approval_required=approval_required,
        ...
        effect_receipts=(
            ...
            else f"compute:multimodal-retrieval-research-copilot:{copilot_id}",
        ),
        artifact={"content_hash": research_artifact_digest(payload),
                  "media_type": "application/vnd.aurora.multimodal-retrieval-synthesis-research-copilot+json"},
    )
    receipt.validate()
    return receipt
