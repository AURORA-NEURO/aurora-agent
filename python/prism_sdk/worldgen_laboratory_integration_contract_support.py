"""Versioned laboratory integration contract negotiation for Worldgen P11 F05-F08."""
from __future__ import annotations
from dataclasses import dataclass
from typing import Any
from .research_contracts import PRECLINICAL_BOUNDARY, RESEARCH_CONTRACT_SCHEMA_VERSION, ResearchContractError, research_artifact_digest
CONTENT_TYPE="application/vnd.aurora.worldgen.laboratory-integration-contract-receipt+json"
@dataclass(frozen=True)
class InstrumentContractRequest:
...
@dataclass(frozen=True)
class InstrumentContractReceipt:
    value:dict[str,Any]
    def validate(self)->None:
        v,a=self.value,self.value.get("artifact",{})
        if not(v.get("schema_version")==RESEARCH_CONTRACT_SCHEMA_VERSION and v.get("boundary")==PRECLINICAL_BOUNDARY and a.get("boundary")==PRECLINICAL_BOUNDARY and a.get("content_type")==CONTENT_TYPE and v.get("raw_data_local") is True and v.get("aggregate_only") is True and v.get("request_id") and v.get("consumer") and v.get("producer") and v.get("field_order") and v.get("effect_receipts")==["none:laboratory_integration-contract-validation"] and all(isinstance(v.get(k),str) and len(v[k])==64 and all(c in "0123456789abcdef" for c in v[k]) for k in ("replay_identity","contract_digest")) and a.get("content_hash")==v.get("contract_digest")): raise ResearchContractError("laboratory integration contract identity, fields, locality, digests, or effects are incomplete")
        for key in ("field_order","retained_field_order","missing_field_order","omitted_field_order","semantic_loss_order","effect_receipts"):
            ...
            if vals!=tuple(sorted(set(vals))): raise ResearchContractError("laboratory integration contract vectors are not canonical")
        ...
        if len(fields)!=len(v["field_order"]) or parts!=fields: raise ResearchContractError("laboratory integration contract fields do not partition")
...
def manifest(*,feature_id:str,contract_version:str,input_schema:str,scale:str,autonomy_tier:str)->dict[str,Any]:
    return {"schema_version":RESEARCH_CONTRACT_SCHEMA_VERSION,"capability_id":feature_id,"version":contract_version,"owner_crate":"worldgen","consumers":["laboratory_integration steward","preclinical researcher","downstream laboratory_integration consumer"],"behavior":f"negotiate a versioned typed laboratory integration contract for {scale}","value":"makes laboratory_integration schema compatibility, semantic loss, locality, and permissions explicit before reuse","input_schema":input_schema,"output_schema":"InstrumentContractReceipt1@1","effects":["none:laboratory_integration-contract-validation","block:unsafe-release"],"permissions":["negotiate:laboratory_integration-contract"],"determinism":"byte_stable","autonomy_tier":autonomy_tier,"boundary":PRECLINICAL_BOUNDARY,"contract_version":contract_version}
def negotiate(request:InstrumentContractRequest,*,feature_id:str,contract_version:str,scale:str,require_federation:bool=False)->InstrumentContractReceipt:
    if not(request.request_id.strip() and request.consumer.strip() and request.producer.strip() and request.namespace.strip() and request.semantic_profile.strip() and request.negotiated_version.strip() and request.field_order and tuple(request.field_order)==tuple(sorted(set(request.field_order))) and request.boundary==PRECLINICAL_BOUNDARY and request.raw_data_local and request.aggregate_only and len(request.replay_identity)==64 and all(c in "0123456789abcdef" for c in request.replay_identity) and (not require_federation or request.policy_allow)): raise ResearchContractError("laboratory integration contract request is invalid")
