"""Parity implementation for AFA-oncoworlds-P01-F11."""
...
FEATURE_ID = "AFA-oncoworlds-P01-F11"
CONTRACT_VERSION = "oncoworlds-prospective-high-throughput-evidence-surveillance-research-copilot/1.0"
INPUT_SCHEMA = "EvidenceFeed3@1"
OUTPUT_SCHEMA = "QualifiedEvidenceSet3@1"


@dataclass(frozen=True)
class OncoworldsEvidenceObservation:
    source_id: str
    sequence: int
    digest: str | None
...
@dataclass(frozen=True)
class OncoworldsEvidenceSurveillanceCopilotReceipt:
...
def run_oncoworlds_prospective_evidence_surveillance_copilot(*, request_id: str, agent_id: str, batch_id: str, checkpoint_seq: int, capacity: int, declared_tools: Sequence[str], requested_tool: str, max_tool_calls: int, dry_run: bool, approval_reference: str | None, approval_granted: bool, observations: Sequence[OncoworldsEvidenceObservation], min_relevance_score: int = 0, policy_allow: bool = True, protected_closure: bool = True, raw_data_local: bool = True, replay_identity: str) -> OncoworldsEvidenceSurveillanceCopilotReceipt:
    ...
    receipt = OncoworldsEvidenceSurveillanceCopilotReceipt(request_id, agent_id, batch_id, checkpoint_seq, capacity, disposition, candidate, selected_order, unresolved_order, denied_order, overflow_order, replay_identity, queue_digest, checkpoint_digest, evidence_digest, provenance_digest, run_digest, omissions_order, uncertainty_order, negative_order, tool_receipts, ("block:unsafe-release",) if disposition == "blocked" else (f"dry-run:bounded-tool:{agent_id}",) if dry_run else (f"invoke:declared-tool:{agent_id}",), qualified, artifact); receipt.validate(); return receipt

def oncoworlds_prospective_evidence_surveillance_copilot_manifest() -> dict[str, Any]:
    return {
        "schema_version": RESEARCH_CONTRACT_SCHEMA_VERSION,
        "capability_id": FEATURE_ID,
        "version": CONTRACT_VERSION,
        "owner_crate": "oncoworlds",
        "consumers": ["preclinical neuroscientist", "MCP host", "queue schema steward"],
        "behavior": "runs bounded prospective EvidenceFeed3 batches with checkpointed ordering, explicit overflow, omission, negative evidence, and declared tool effects",
        "value": "turns OncoWorlds evidence surveillance into resumable, policy-auditable research work",
        "input_schema": INPUT_SCHEMA,
        "output_schema": OUTPUT_SCHEMA,
        "effects": ["read_local_data", "execute_local_computation", "write_local_artifact"],
        "permissions": ["invoke:declared-tools", "read:local-evidence"],
        "autonomy_tier": "a2",
...
__all__ = [
    ...
    "OncoworldsEvidenceObservation", "OncoworldsEvidenceSurveillanceCopilotReceipt",
