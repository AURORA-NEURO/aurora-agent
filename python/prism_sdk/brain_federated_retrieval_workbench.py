"""Python parity contract for the federated retrieval researcher workbench."""
...
class BrainFederatedRetrievalWorkbenchReceipt:
    request_id: str
    workspace_id: str
    federation_id: str
    institution_id: str
    ...
    feature_id: str = FEDERATED_RETRIEVAL_WORKBENCH_FEATURE_ID
    contract_version: str = FEDERATED_RETRIEVAL_WORKBENCH_CONTRACT_VERSION
    schema_version: str = RESEARCH_CONTRACT_SCHEMA_VERSION
    raw_data_local: bool = True
    boundary: str = PRECLINICAL_BOUNDARY

    def validate(self) -> None:
        if self.schema_version != RESEARCH_CONTRACT_SCHEMA_VERSION or self.feature_id != FEDERATED_RETRIEVAL_WORKBENCH_FEATURE_ID or self.contract_version != FEDERATED_RETRIEVAL_WORKBENCH_CONTRACT_VERSION:
            raise ResearchContractError("federated workbench schema, feature, or version mismatch")
        if self.boundary != PRECLINICAL_BOUNDARY or not self.raw_data_local or not self.request_id.strip() or not self.workspace_id.strip() or not self.federation_id.strip() or not self.institution_id.strip() or not self.purpose.strip() or not self.endpoint.strip() or len(self.study_order) < 2 or len(self.modality_order) < 2 or not self.view_order or not self.panel_order or not self.action_receipts or not self.candidate_order or not self.effect_receipts or self.budget_units <= 0:
            raise ResearchContractError("federated workbench identity, coverage, views, panels, retrieval, locality, budget, or effects are incomplete")
        if any(value not in self.candidate_order for value in (*self.ranked_order, *self.qualified_order, *self.blocked_order, *self.unknown_order)):
            raise ResearchContractError("federated workbench state is not covered by candidates")
        ...
        if tuple(sorted(set(self.aggregate_order))) != self.aggregate_order:
            raise ResearchContractError("federated aggregate ordering is not canonical")
        for value in (self.comparability_digest, self.envelope_digest, self.synthesis_digest, self.workbench_digest, self.replay_identity, self.artifact.get("content_hash"), *self.aggregate_order):
            if not isinstance(value, str) or not re.fullmatch(r"[0-9a-f]{64}", value):
                raise ResearchContractError("federated workbench digest is invalid")
        if any(not effect.startswith("view:local-federated-retrieval-artifacts:") and effect != "block:unsafe-release" for effect in self.effect_receipts):
            raise ResearchContractError("federated workbench effect is not read-only")

    def digest(self) -> str:
        self.validate()
        return research_artifact_digest({
            "schema_version": self.schema_version, "contract_version": self.contract_version, "feature_id": self.feature_id,
            "request_id": self.request_id, "workspace_id": self.workspace_id, "federation_id": self.federation_id, "institution_id": self.institution_id, "purpose": self.purpose, "endpoint": self.endpoint, "study_order": list(self.study_order), "modality_order": list(self.modality_order), "disposition": self.disposition,
