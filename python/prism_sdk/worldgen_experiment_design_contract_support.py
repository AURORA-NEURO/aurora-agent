"""Versioned experiment design contract negotiation for Worldgen P08 F05-F08."""
from __future__ import annotations
from dataclasses import dataclass
from typing import Any
from .research_contracts import PRECLINICAL_BOUNDARY, RESEARCH_CONTRACT_SCHEMA_VERSION, ResearchContractError, research_artifact_digest
CONTENT_TYPE="application/vnd.aurora.worldgen.experiment_design-contract-receipt+json"
@dataclass(frozen=True)
class ExperimentDesignContractRequest:
...
@dataclass(frozen=True)
class ExperimentDesignContractReceipt:
    value:dict[str,Any]
    def validate(self)->None:
        v,a=self.value,self.value.get("artifact",{})
        if not(v.get("schema_version")==RESEARCH_CONTRACT_SCHEMA_VERSION and v.get("boundary")==PRECLINICAL_BOUNDARY and a.get("boundary")==PRECLINICAL_BOUNDARY and a.get("content_type")==CONTENT_TYPE and v.get("raw_data_local") is True and v.get("aggregate_only") is True and v.get("request_id") and v.get("consumer") and v.get("producer") and v.get("field_order") and v.get("effect_receipts")==["none:experiment_design-contract-validation"] and all(isinstance(v.get(k),str) and len(v[k])==64 and all(c in "0123456789abcdef" for c in v[k]) for k in ("replay_identity","contract_digest")) and a.get("content_hash")==v.get("contract_digest")): raise ResearchContractError("experiment design contract identity, fields, locality, digests, or effects are incomplete")
        for key in ("field_order","retained_field_order","missing_field_order","omitted_field_order","semantic_loss_order","effect_receipts"):
            ...
            if vals!=tuple(sorted(set(vals))): raise ResearchContractError("experiment design contract vectors are not canonical")
        ...
        if len(fields)!=len(v["field_order"]) or parts!=fields: raise ResearchContractError("experiment design contract fields do not partition")
...
def manifest(*,feature_id:str,contract_version:str,input_schema:str,scale:str,autonomy_tier:str)->dict[str,Any]:
    return {"schema_version":RESEARCH_CONTRACT_SCHEMA_VERSION,"capability_id":feature_id,"version":contract_version,"owner_crate":"worldgen","consumers":["experiment_design steward","preclinical researcher","downstream experiment_design consumer"],"behavior":f"negotiate a versioned typed experiment design contract for {scale}","value":"makes experiment_design schema compatibility, semantic loss, locality, and permissions explicit before reuse","input_schema":input_schema,"output_schema":"ExperimentDesignContractReceipt1@1","effects":["none:experiment_design-contract-validation","block:unsafe-release"],"permissions":["negotiate:experiment_design-contract"],"determinism":"byte_stable","autonomy_tier":autonomy_tier,"boundary":PRECLINICAL_BOUNDARY,"contract_version":contract_version}
def negotiate(request:ExperimentDesignContractRequest,*,feature_id:str,contract_version:str,scale:str,require_federation:bool=False)->ExperimentDesignContractReceipt:
    ...
    payload={"schema_version":RESEARCH_CONTRACT_SCHEMA_VERSION,"contract_version":contract_version,"feature_id":feature_id,"request_id":request.request_id,"consumer":request.consumer,"producer":request.producer,"namespace":request.namespace,"semantic_profile":request.semantic_profile,"negotiated_version":request.negotiated_version,"compatibility":compatibility,"disposition":disposition,"field_order":sorted(fields),"retained_field_order":sorted(retained),"missing_field_order":sorted(missing),"omitted_field_order":sorted(omitted),"semantic_loss_order":sorted(loss),"replay_identity":request.replay_identity,"effect_receipts":["none:experiment_design-contract-validation"],"raw_data_local":True,"aggregate_only":True,"boundary":PRECLINICAL_BOUNDARY}
    d=research_artifact_digest(payload); payload["contract_digest"]=d; payload["artifact"]={"artifact_id":f"worldgen-experiment_design-contract:{request.request_id}","content_type":CONTENT_TYPE,"content_hash":d,"boundary":PRECLINICAL_BOUNDARY}; receipt=ExperimentDesignContractReceipt(payload); receipt.validate(); return receipt
__all__=["CONTENT_TYPE","ExperimentDesignContractRequest","ExperimentDesignContractReceipt","manifest","negotiate"]
