"""Python parity surface for AFA-adapter-P02-F09.

The engine is deliberately local and deterministic.  It ranks typed candidate
metadata for one preclinical study, never moves raw source payloads, and emits
...
FEATURE_ID = ADAPTER_LOCAL_RETRIEVAL_SYNTHESIS_RESEARCH_COPILOT_FEATURE_ID
CONTRACT_VERSION = ADAPTER_LOCAL_RETRIEVAL_SYNTHESIS_RESEARCH_COPILOT_CONTRACT_VERSION
INPUT_SCHEMA = "ScopedRetrievalQuery@1"
OUTPUT_SCHEMA = "EvidenceSynthesis1@1"


@dataclass(frozen=True)
class LocalRetrievalSynthesisCandidate:
    evidence_id: str
    study_id: str
    modality: str
...
@dataclass(frozen=True)
class LocalRetrievalSynthesisResearchCopilotReceipt:
    request_id: str
    query_id: str
    copilot_id: str
    agent_id: str
    ...
    def validate(self) -> None:
        ...
        if any(
            not effect.startswith("compute:local-retrieval-research-copilot:")
...
def run_local_retrieval_synthesis_research_copilot(
    *,
    request_id: str,
    query_id: str,
    requester: str,
    ...
    copilot_id: str,
    algorithm_version: str,
    agent_id: str = "agent:local-retrieval-copilot",
    ...
    selected: list[LocalRetrievalSynthesisCandidate] = []
    omitted: list[LocalRetrievalSynthesisCandidate] = []
    ...
    receipt = LocalRetrievalSynthesisResearchCopilotReceipt(
        request_id=request_id, query_id=query_id, copilot_id=copilot_id,
        agent_id=agent_id, recommendation_mode=recommendation_mode,
        approval_required=approval_required,
        ...
        effect_receipts=(f"compute:local-retrieval-research-copilot:{copilot_id}",),
        artifact={"content_hash": research_artifact_digest(payload),
                  "media_type": "application/vnd.aurora.local-retrieval-synthesis-research-copilot+json"},
    )
    receipt.validate()
    return receipt
