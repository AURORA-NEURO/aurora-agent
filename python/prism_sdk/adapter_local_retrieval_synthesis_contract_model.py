"""Python parity surface for AFA-adapter-P02-F05.
...
FEATURE_ID = ADAPTER_LOCAL_RETRIEVAL_SYNTHESIS_CONTRACT_MODEL_FEATURE_ID
CONTRACT_VERSION = ADAPTER_LOCAL_RETRIEVAL_SYNTHESIS_CONTRACT_MODEL_CONTRACT_VERSION
...
@dataclass(frozen=True)
class LocalRetrievalSynthesisContractModelReceipt:
    request_id: str
    query_id: str
    contract_id: str
    schema_profile: str
    ...
    def validate(self) -> None:
        if (
            ...
            raise ResearchContractError("local retrieval contract model identity, output, locality, candidates, or effects are incomplete")
        for values in (
            ...
            if tuple(sorted(set(values))) != values:
                raise ResearchContractError("local retrieval contract model ordering is not canonical")
        if set(self.selected_order) | set(self.omitted_order) != set(self.candidate_order):
            raise ResearchContractError("local retrieval contract model states do not partition candidates")
        for value in (
            self.replay_identity, self.synthesis_digest, self.contract_digest,
            self.artifact.get("content_hash"),
        ):
            if not isinstance(value, str) or not re.fullmatch(r"[0-9a-f]{64}", value):
                raise ResearchContractError("local retrieval contract model digest is invalid")
        if any(
            not effect.startswith("compute:local-retrieval-contract-model:")
            and effect != "block:unsafe-release"
            for effect in self.effect_receipts
        ):
            raise ResearchContractError("local retrieval contract model effect is outside local computation gate")


def run_local_retrieval_synthesis_contract_model(
    *,
    request_id: str,
    query_id: str,
    requester: str,
    ...
    if (
        ...
        or not candidates or not contract_id.strip() or schema_profile != INPUT_SCHEMA
    ...
    receipt = LocalRetrievalSynthesisContractModelReceipt(
        request_id=request_id, query_id=query_id, contract_id=contract_id,
        ...
        effect_receipts=(f"compute:local-retrieval-contract-model:{contract_id}",),
        artifact={"content_hash": research_artifact_digest(payload),
                  "media_type": "application/vnd.aurora.local-retrieval-synthesis-contract-model+json"},
    )
    receipt.validate()
    return receipt
