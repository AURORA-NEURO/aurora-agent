"""Python parity surface for AFA-adapter-P02-F21."""
...
FEATURE_ID = "AFA-adapter-P02-F21"
CONTRACT_VERSION = "adapter-local-retrieval-synthesis-interoperability-gateway/1.0"
...
class LocalRetrievalSynthesisInteroperabilityGatewayReceipt:
...
def render_local_retrieval_synthesis_interoperability_gateway(*, protocol: Mapping[str, Any], workbench_kwargs: Mapping[str, Any], requested_input_schema: str = INPUT_SCHEMA, requested_output_schema: str = OUTPUT_SCHEMA, migration_policy: str = "additive-minor-with-loss-receipt", semantic_loss_budget: int = 4, boundary: str = PRECLINICAL_BOUNDARY) -> LocalRetrievalSynthesisInteroperabilityGatewayReceipt:
    ...
    receipt = LocalRetrievalSynthesisInteroperabilityGatewayReceipt(request_id=request_id, endpoint_id=endpoint_id, negotiated_version=negotiated, disposition=disposition, input_schema=requested_input_schema, output_schema=requested_output_schema, migration_policy=migration_policy, semantic_loss_budget=semantic_loss_budget, capability_order=offered, artifact_digest_order=artifacts, semantic_loss_order=tuple(loss), omissions=omissions, uncertainty=uncertainty, workbench_digest=wb.workbench_digest, protocol_digest=protocol_digest, effect_receipts=(effect,), artifact={"content_hash": research_artifact_digest({**payload, "protocol_digest": protocol_digest}), "media_type": "application/vnd.aurora.local-retrieval-synthesis-interoperability+json"})
    receipt.validate(); return receipt


__all__ = ["FEATURE_ID", "CONTRACT_VERSION", "TARGET_PROTOCOL_VERSION", "LocalRetrievalSynthesisInteroperabilityGatewayReceipt", "render_local_retrieval_synthesis_interoperability_gateway"]
