"""Python mirror of the brain evidence-surveillance receipt."""
...
class BrainEvidenceSurveillanceReceipt:
    request_id: str
    study_id: str
    scope: str
    disposition: str
    ...
    feature_id: str = BRAIN_EVIDENCE_SURVEILLANCE_FEATURE_ID
    contract_version: str = BRAIN_EVIDENCE_SURVEILLANCE_CONTRACT_VERSION
    schema_version: str = RESEARCH_CONTRACT_SCHEMA_VERSION
    raw_data_local: bool = True
    boundary: str = PRECLINICAL_BOUNDARY

    def validate(self) -> None:
        if self.schema_version != RESEARCH_CONTRACT_SCHEMA_VERSION or self.feature_id != BRAIN_EVIDENCE_SURVEILLANCE_FEATURE_ID or self.contract_version != BRAIN_EVIDENCE_SURVEILLANCE_CONTRACT_VERSION:
            raise ResearchContractError("brain surveillance schema, feature, or version mismatch")
        if self.boundary != PRECLINICAL_BOUNDARY or not self.raw_data_local or not self.request_id.strip() or not self.study_id.strip() or not self.scope.strip() or not self.candidate_order or len(self.relevance_order) != len(self.candidate_order) or not self.effect_receipts:
            raise ResearchContractError("evidence identity, ranking, locality, relevance, or effects are incomplete")
        if self.disposition not in {"qualified", "partial", "unknown", "blocked"}:
            raise ResearchContractError("brain surveillance disposition is unknown")
        if any(value not in self.candidate_order for value in (*self.qualified_order, *self.blocked_order, *self.unknown_order)):
            raise ResearchContractError("evidence state is not covered by candidate order")
        for values in (self.candidate_order, self.qualified_order, self.blocked_order, self.unknown_order, self.source_order, self.modality_order, self.omissions, self.uncertainty, self.negative_evidence, self.effect_receipts):
            if tuple(sorted(set(values))) != values:
                raise ResearchContractError("brain surveillance ordering is invalid")
        for values in (self.semantic_order, self.artifact_order, self.provenance_order):
            if tuple(sorted(set(values))) != values:
                raise ResearchContractError("brain surveillance digest ordering is invalid")
        ...
        if any(not isinstance(value, str) or not re.fullmatch(r"[0-9a-f]{64}", value) for value in digests):
            raise ResearchContractError("brain surveillance digest is invalid")
        if self.qualified_order and any(not effect.startswith("read:local-research-artifacts:") for effect in self.effect_receipts):
            raise ResearchContractError("qualified evidence requires a local-read receipt")
        if not self.qualified_order and self.effect_receipts != ("block:unsafe-release",):
            raise ResearchContractError("empty evidence result must be explicitly blocked")
