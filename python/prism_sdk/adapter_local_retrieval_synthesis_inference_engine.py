"""Python parity surface for AFA-adapter-P02-F01.
...
FEATURE_ID = ADAPTER_LOCAL_RETRIEVAL_SYNTHESIS_INFERENCE_ENGINE_FEATURE_ID
CONTRACT_VERSION = ADAPTER_LOCAL_RETRIEVAL_SYNTHESIS_INFERENCE_ENGINE_CONTRACT_VERSION
...
@dataclass(frozen=True)
class LocalRetrievalSynthesisInferenceEngineReceipt:
    request_id: str
    query_id: str
    engine_id: str
    ...
    def validate(self) -> None:
        if (
            ...
            raise ResearchContractError("local retrieval engine identity, output, locality, candidates, or effects are incomplete")
        for values in (
            ...
            if tuple(sorted(set(values))) != values:
                raise ResearchContractError("local retrieval engine ordering is not canonical")
        if set(self.selected_order) | set(self.omitted_order) != set(self.candidate_order):
            raise ResearchContractError("local retrieval engine states do not partition candidates")
        for value in (
            self.replay_identity, self.synthesis_digest, self.engine_digest,
            self.artifact.get("content_hash"),
        ):
            if not isinstance(value, str) or not re.fullmatch(r"[0-9a-f]{64}", value):
                raise ResearchContractError("local retrieval engine digest is invalid")
        if any(
            not effect.startswith("compute:local-retrieval-engine:")
            and effect != "block:unsafe-release"
            for effect in self.effect_receipts
        ):
            raise ResearchContractError("local retrieval engine effect is outside local computation gate")


def run_local_retrieval_synthesis_inference_engine(
    *,
    request_id: str,
    query_id: str,
    requester: str,
    ...
    receipt = LocalRetrievalSynthesisInferenceEngineReceipt(
        request_id=request_id, query_id=query_id, engine_id=engine_id,
        algorithm_version=algorithm_version, requested_output=requested_output,
