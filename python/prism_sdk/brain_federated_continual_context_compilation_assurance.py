"""Federated continual context-compilation assurance parity contract."""
...
class FederatedContextAssurancePeer:
    institution_id: str
    artifact_id: str
    context_digest: str
    section_digest: str
...
@dataclass(frozen=True)
class FederatedContextAssuranceReceipt:
    request_id: str
    federation_id: str
    purpose: str
    scope: str
    ...
    feature_id: str = FEDERATED_CONTINUAL_CONTEXT_COMPILATION_ASSURANCE_FEATURE_ID
    contract_version: str = FEDERATED_CONTINUAL_CONTEXT_COMPILATION_ASSURANCE_CONTRACT_VERSION
    ...
    def validate(self) -> None:
        if (
            ...
            or self.feature_id != FEDERATED_CONTINUAL_CONTEXT_COMPILATION_ASSURANCE_FEATURE_ID
            or self.contract_version != FEDERATED_CONTINUAL_CONTEXT_COMPILATION_ASSURANCE_CONTRACT_VERSION
        ):
            raise ResearchContractError("federated assurance schema, feature, or version mismatch")
        if (
            ...
            raise ResearchContractError("federated assurance identity, quorum, locality, aggregate-only, or effects are incomplete")
        for values in (
        ...
        if classified != set(self.candidate_order):
            raise ResearchContractError("federated assurance outcomes do not partition candidates")
        ...
        if any(not effect.startswith("assurance:federated-context:") and effect != "block:unsafe-release" for effect in self.effect_receipts):
            raise ResearchContractError("federated assurance effect is outside the governed release gate")
...
def assure_federated_continual_context_compilation(*, request_id: str, federation_id: str, purpose: str, scope: str, goal: str, semantic_profile: str, institution_ids: Sequence[str], peers: Sequence[FederatedContextAssurancePeer], minimum_quorum: int, replay_identity: str, policy_allow: bool = True, protected_closure: bool = True, raw_data_local: bool = True, aggregate_only: bool = True, signed_approval: bool = True) -> FederatedContextAssuranceReceipt:
    if not request_id.strip() or not federation_id.strip() or not purpose.strip() or not scope.strip() or not goal.strip() or not semantic_profile.strip() or len(institution_ids) < 2 or minimum_quorum < 1 or not re.fullmatch(r"[0-9a-f]{64}", replay_identity) or not raw_data_local or not aggregate_only:
        raise ResearchContractError("federated assurance identity, quorum, replay, locality, aggregate-only, or boundary is invalid")
    ...
    peer_map: dict[str, FederatedContextAssurancePeer] = {}
    for peer in peers:
