"""Python parity surface for AFA-adapter-P02-F07.
...
FEATURE_ID = ADAPTER_THROUGHPUT_RETRIEVAL_SYNTHESIS_CONTRACT_MODEL_FEATURE_ID
CONTRACT_VERSION = ADAPTER_THROUGHPUT_RETRIEVAL_SYNTHESIS_CONTRACT_MODEL_CONTRACT_VERSION
...
@dataclass(frozen=True)
class ThroughputRetrievalSynthesisContractModelReceipt:
    request_id: str
    query_id: str
    contract_id: str
    schema_profile: str
    ...
    def validate(self) -> None:
        if (
            ...
            raise ResearchContractError("throughput retrieval contract model identity, output, locality, candidates, or effects are incomplete")
        for values in (
            ...
            if tuple(sorted(set(values))) != values:
                raise ResearchContractError("throughput retrieval contract model ordering is not canonical")
        if set(self.selected_order) | set(self.omitted_order) != set(self.candidate_order):
            raise ResearchContractError("throughput retrieval contract model states do not partition candidates")
        ...
        for value in (
            ...
            if not isinstance(value, str) or not re.fullmatch(r"[0-9a-f]{64}", value):
                raise ResearchContractError("throughput retrieval contract model digest is invalid")
        if any(
            not effect.startswith("compute:throughput-retrieval-contract-model:")
            and effect != "block:unsafe-release"
            for effect in self.effect_receipts
        ):
            raise ResearchContractError("throughput retrieval contract model effect is outside local computation gate")


def run_throughput_retrieval_synthesis_contract_model(
    *,
    request_id: str,
    query_id: str,
    requester: str,
    ...
    receipt = ThroughputRetrievalSynthesisContractModelReceipt(
        request_id=request_id, query_id=query_id, contract_id=contract_id,
        ...
        effect_receipts=(f"compute:throughput-retrieval-contract-model:{contract_id}",),
        artifact={"content_hash": research_artifact_digest(payload),
                  "media_type": "application/vnd.aurora.throughput-retrieval-synthesis-contract-model+json"},
    )
    receipt.validate()
    return receipt
