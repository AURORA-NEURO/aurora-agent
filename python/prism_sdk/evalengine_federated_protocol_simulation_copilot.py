"""Federated continual protocol simulation research copilot for ``AFA-evalengine-P10-F12``."""
...
FEATURE_ID = "AFA-evalengine-P10-F12"
CONTRACT_VERSION = "evalengine-federated-continual-protocol-simulation-research-copilot/1.0"
INPUT_SCHEMA = "ProtocolDraft4@1"
OUTPUT_SCHEMA = "ProtocolSimulationReport3@1"
...
class EvalengineProtocolSimulationReport:
    ...
    def validate(self) -> None:
        ...
        if any(not effect.startswith("invoke:declared-tool:evalengine-protocol-simulation:") and effect != "block:unsafe-release" for effect in self.effect_receipts): raise ResearchContractError("federated protocol effect is outside verification gate")
...
def assure_evalengine_protocol(*, request: Mapping[str, Any]) -> EvalengineProtocolSimulationReport:
    ...
    envelope = _hash({"federation_id":request["federation_id"],"purpose":request["purpose"],"protocol_digest":request["protocol_digest"],"peer_order":peer_ids,"qualified_peer_order":qpeer,"replay_identity":request["replay_identity"],"semantic_profile":request["semantic_profile"]}); payload = {"schema_version":RESEARCH_CONTRACT_SCHEMA_VERSION,"contract_version":CONTRACT_VERSION,"feature_id":FEATURE_ID,"request_id":request["request_id"],"federation_id":request["federation_id"],"purpose":request["purpose"],"protocol_schema":request["protocol_schema"],"semantic_profile":request["semantic_profile"],"required_step_order":required_steps,"observed_step_order":sorted(observed_steps),"missing_step_order":missing_steps,"violation_order":sorted(violations),"peer_order":peer_ids,"qualified_peer_order":qpeer,"unresolved_peer_order":upeer,"blocked_peer_order":bpeer,"replay_identity":request["replay_identity"],"peer_envelope_digest":envelope,"disposition":disposition,"boundary":PRECLINICAL_BOUNDARY}; verdict = _hash(payload); artifact={"schema_version":RESEARCH_CONTRACT_SCHEMA_VERSION,"artifact_id":f"evalengine-protocol-simulation:{request['request_id']}","content_type":"application/vnd.aurora.protocol-simulation-report+json","content_hash":verdict,"semantic_loss":[],"provenance":[{"source_id":request["federation_id"],"relation":"federated-protocol-simulation-assurance","digest":verdict}],"boundary":PRECLINICAL_BOUNDARY}; checks=tuple(sorted({"schema-version","step-closure","peer-provenance","peer-semantic-profile","peer-replay-identity","peer-quorum","policy-boundary","negative-evidence-retention"})); report=EvalengineProtocolSimulationReport(RESEARCH_CONTRACT_SCHEMA_VERSION,CONTRACT_VERSION,FEATURE_ID,str(request["request_id"]),str(request["federation_id"]),str(request["purpose"]),str(request["protocol_schema"]),str(request["semantic_profile"]),disposition,tuple(required_steps),tuple(sorted(observed_steps)),tuple(missing_steps),tuple(sorted(violations)),tuple(peer_ids),tuple(qpeer),tuple(upeer),tuple(bpeer),tuple(sorted(set(str(item) for item in request.get("adversarial_events", [])))),tuple(sorted(omissions)),tuple(sorted(uncertainty)),tuple(sorted(negative)),str(request["protocol_digest"]),str(request["replay_identity"]),str(request.get("provenance_digest")) if request.get("provenance_digest") else None,envelope,verdict,(f"invoke:declared-tool:evalengine-protocol-simulation:{request['request_id']}",) if disposition=="qualified" else ("block:unsafe-release",),artifact,True,PRECLINICAL_BOUNDARY); report.validate(); return report

__all__ = ["FEATURE_ID","CONTRACT_VERSION","INPUT_SCHEMA","OUTPUT_SCHEMA","EvalengineProtocolSimulationReport","assure_evalengine_protocol","evalengine_protocol_simulation_copilot_manifest"]


def evalengine_protocol_simulation_copilot_manifest() -> dict[str, Any]:
    return {"schema_version": RESEARCH_CONTRACT_SCHEMA_VERSION, "capability_id": FEATURE_ID, "version": CONTRACT_VERSION, "owner_crate": "evalengine", "consumers": ["preclinical neuroscientist", "protocol simulator reviewer", "federation steward"], "behavior": "preflights caller-supplied protocol state-machine summaries and emits bounded declared-tool receipts without executing protocols", "value": "makes protocol simulation, recovery, negative evidence, and federation gates auditable before any laboratory effect", "input_schema": INPUT_SCHEMA, "output_schema": OUTPUT_SCHEMA, "effects": ["invoke:declared-tool", "block:unsafe-release"], "permissions": ["invoke:declared-tools"], "autonomy_tier": "A2", "boundary": PRECLINICAL_BOUNDARY}
