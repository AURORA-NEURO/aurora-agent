"""Python parity for ``AFA-ids-P07-F26`` multimodal QC assurance."""
from __future__ import annotations
import hashlib,json,re
from dataclasses import dataclass
from typing import Any,Mapping,Sequence
from .research_contracts import PRECLINICAL_BOUNDARY,RESEARCH_CONTRACT_SCHEMA_VERSION,ResearchContractError
FEATURE_ID="AFA-ids-P07-F26"; CONTRACT_VERSION="ids-multimodal-quality-control-assurance-harness/1.0"; INPUT_SCHEMA="QualityControlBatch4@1"; OUTPUT_SCHEMA="QualityControlReport8@1"; CONTENT_TYPE="application/vnd.aurora.quality-control-report-8+json"
def _hash(v:Any)->str:return hashlib.sha256(json.dumps(v,sort_keys=True,separators=(",",":"),ensure_ascii=False).encode()).hexdigest()
def _digest(v:Any)->bool:return isinstance(v,str) and re.fullmatch(r"[0-9a-f]{64}",v) is not None
def _ordered(v:list[str])->bool:return tuple(v)==tuple(sorted(set(v)))
@dataclass(frozen=True)
class QualityControlReport8:
    value:dict[str,Any]
    ...
    def validate(self)->None:
        ...
        if any(not e.startswith("manage:local-capability:") and e!="block:unsafe-release" for e in v["effect_receipts"]):raise ResearchContractError("quality effect is outside governed gate")
def quality_control_manifest()->dict[str,Any]:return {"schema_version":RESEARCH_CONTRACT_SCHEMA_VERSION,"capability_id":FEATURE_ID,"version":CONTRACT_VERSION,"owner_crate":"ids","consumers":["quality scientist","computational biologist","research administrator"],"behavior":"evaluates bounded multimodal metric summaries against typed thresholds and modality closure","value":"prevents failed, missing, or unmeasured QC evidence from silently entering a research workflow","input_schema":INPUT_SCHEMA,"output_schema":OUTPUT_SCHEMA,"effects":["manage:local-capability"],"permissions":["read:local-research-artifacts"],"autonomy_tier":"A1","boundary":PRECLINICAL_BOUNDARY}
def assure_quality_control(request:Mapping[str,Any],observations:Sequence[Mapping[str,Any]])->QualityControlReport8:
