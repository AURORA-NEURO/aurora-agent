"""Python parity contract for the throughput retrieval researcher workbench."""
...
class BrainThroughputRetrievalWorkbenchReceipt:
    request_id: str
    workspace_id: str
    batch_id: str
    partition: str
    ...
    feature_id: str = THROUGHPUT_RETRIEVAL_WORKBENCH_FEATURE_ID
    contract_version: str = THROUGHPUT_RETRIEVAL_WORKBENCH_CONTRACT_VERSION
    schema_version: str = RESEARCH_CONTRACT_SCHEMA_VERSION
    raw_data_local: bool = True
    boundary: str = PRECLINICAL_BOUNDARY

    def validate(self) -> None:
        if self.schema_version != RESEARCH_CONTRACT_SCHEMA_VERSION or self.feature_id != THROUGHPUT_RETRIEVAL_WORKBENCH_FEATURE_ID or self.contract_version != THROUGHPUT_RETRIEVAL_WORKBENCH_CONTRACT_VERSION:
            raise ResearchContractError("throughput workbench schema, feature, or version mismatch")
        if self.boundary != PRECLINICAL_BOUNDARY or not self.raw_data_local or not self.request_id.strip() or not self.workspace_id.strip() or not self.batch_id.strip() or not self.partition.strip() or not self.view_order or not self.panel_order or not self.action_receipts or not self.candidate_order or self.checkpoint_seq <= 0 or self.budget_units <= 0 or not self.effect_receipts:
            raise ResearchContractError("throughput workbench identity, queue, checkpoint, views, panels, budget, locality, or effects are incomplete")
        ...
        for values in (self.view_order, self.panel_order, self.action_receipts, self.candidate_order, self.ranked_order, self.qualified_order, self.blocked_order, self.unknown_order, self.omissions, self.uncertainty, self.negative_evidence, self.effect_receipts):
            if tuple(sorted(set(values))) != values:
                raise ResearchContractError("throughput workbench ordering is not canonical")
        for value in (self.queue_digest, self.synthesis_digest, self.workbench_digest, self.replay_identity, self.artifact.get("content_hash")):
            if not isinstance(value, str) or not re.fullmatch(r"[0-9a-f]{64}", value):
                raise ResearchContractError("throughput workbench digest is invalid")
        if any(not effect.startswith("view:local-throughput-retrieval-artifacts:") and effect != "block:unsafe-release" for effect in self.effect_receipts):
    ...
    def digest(self) -> str:
        ...
        return research_artifact_digest({
            ...
            "request_id": self.request_id, "workspace_id": self.workspace_id, "batch_id": self.batch_id, "partition": self.partition, "disposition": self.disposition,
